...
/** DB 검사규칙 symptom_reports_report_type_check 와 같은 값 집합. 여기 없는 값은 저장이 거부된다. */
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const report: SymptomReport = {
      ...
      // DB 검사규칙(symptom_reports_report_type_check)이 허용하는 값만 넣는다.
      // 기존 기본값 "self" 는 이 목록에 없어 환자 화면 제출이 «전부» 저장 실패했다(응답은 saved:false).
