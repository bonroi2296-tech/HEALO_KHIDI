...
// 🛑 여기 20 이면 코디가 실제 환자 한 벌을 대신 올릴 때 스무 장째에서 막힌다.
//    실제 규모: 유방암 케이스 한 벌이 러시아어 원본 21 + 영문 번역 23 + 영상 7 + 의뢰서 1 = 52장,
//    다른 케이스는 문서만 68장이었다. 20 은 «한 벌»의 절반도 못 받는 수다.
//
// ⚠️ 접수 창구(referralSubmit.ts)의 100 과 «같은 수»가 아니다 — 재는 것이 다르다.
//    그쪽 100 = «한 번 제출에 담을 수 있는 장수». 여기 = «이 문의에 쌓인 전부».
//    같은 수로 두면 환자가 68장을 내고 온 문의에 코디가 왓츠앱으로 받은 걸 더 올리다 막힌다
//    (2026-09-08 독립 리뷰). 그래서 접수 상한의 두 배로 «여유»를 둔다.
//    이 창구는 직원 인증이 필요하고 봇이 못 부르므로, 여유를 두는 쪽의 위험이 더 작다.
