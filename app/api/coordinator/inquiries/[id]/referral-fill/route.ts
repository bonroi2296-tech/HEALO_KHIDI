...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const [key, raw] of Object.entries(incoming)) {
      ...
      // 진단명 칸에도 같은 것이 들어온다 — #316 은 「Z04 Обследование и наблюдение…」
      // (기타 목적의 검사 및 관찰)이 병명 자리에 저장돼 있었다.
      if (key === "diagnosisNameRaw" && startsWithEncounterCode(String(raw))) continue;
