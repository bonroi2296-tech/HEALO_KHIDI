...
const CONTENT = {
  ko: {
    ...
    kept: [
      ...
      "상담 때 올리신 검사 파일은 진료기록이라 남습니다. 다만 파일 이름에 성함이 들어 있으면 그 글자를 지웁니다.",
      "문의·상담 기록은 이름·연락처·이메일을 지운 «익명» 상태로 남습니다. 병원에 전달된 진료 관련 기록이라 법령상 보존의무가 걸리며, 남은 내용만으로는 회원님을 알아볼 수 없습니다.",
  ...
  en: {
    ...
    kept: [
      ...
      "Test files you uploaded stay on the care record. If your name appears in a file name, that name is removed.",
      "Inquiry and consultation records remain, but with your name, contact number and email removed. They are care records already sent to a hospital and the law requires them to be kept; what is left cannot identify you.",
  ...
  ru: {
    ...
    kept: [
      ...
      "Загруженные вами файлы обследований остаются в записи о лечении. Если ваше имя есть в названии файла, оно удаляется.",
      "Записи об обращениях и консультациях остаются, но без вашего имени, телефона и электронной почты. Это записи о лечении, уже переданные в больницу, их хранение требует закон; по оставшимся данным вас нельзя опознать.",
  ...
  kz: {
    ...
    kept: [
      ...
      "Сіз жүктеген тексеру файлдары емдеу жазбасында қалады. Файл атауында аты-жөніңіз болса, ол өшіріледі.",
      "Өтініш пен кеңес жазбалары қалады, бірақ аты-жөніңіз, телефоныңыз және поштаңыз өшіріледі. Бұл ауруханаға жіберілген емдеу жазбалары, оларды сақтауды заң талап етеді; қалған мәліметтер бойынша сізді тану мүмкін емес.",
  ...
  zh: {
    ...
    kept: [
      ...
      "您上传的检查文件属于诊疗记录会保留。若文件名中含有您的姓名，该姓名将被清除。",
      "咨询与问诊记录会保留，但姓名、联系电话和邮箱将被清除。这些是已发送至医院的诊疗相关记录，法律要求保存；仅凭剩余内容无法识别您本人。",
  ...
  ja: {
    ...
    kept: [
      ...
      "アップロードされた検査ファイルは診療記録として残ります。ファイル名にお名前が含まれる場合、その部分は削除されます。",
      "お問い合わせ・相談の記録は残りますが、氏名・連絡先・メールアドレスは消去されます。病院へ送られた診療関連の記録で法令上の保存義務があり、残る内容だけではお客様を特定できません。",
