...
5. **차트(그래프·도넛·막대)를 넣을 거면 그리기 «전»에 `dataviz` 스킬을 연다.** 이 문서엔 차트 규칙이 없다.
   2026-09-10 실측: 맨손으로 만든 차트 3개 중 **2개가 그 스킬의 안티패턴**이었다 —
   ①2조각 도넛(단일 비율)은 차트가 아니라 **숫자 카드(`B.stat`)** 다 ②부분-전체는 도넛이 아니라 **가로 누적 막대**가 기본형이다.
   색은 눈대중하지 말고 검증기를 돌린다: `node <스킬>/scripts/validate_palette.js "<hex,...>" --mode light`.
   ⚠️ 우리 차트는 «강조 1 + 회색»이라 categorical 검사 2건은 범위 밖이다. 봐야 할 건 **대비 WARN** 하나이고,
   막대마다 직접 라벨을 달면 해소된다.
6. **뽑고 나서 눈으로 본다** (§3).
