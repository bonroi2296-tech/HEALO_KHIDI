...
# 메시지에 «무엇이 바뀌었는지»를 넣는다 (2026-09-10).
# 왜: 한 세션에서만 네 번, 어시가 이유를 길게 적어 커밋하려는 «직전»에 이 훅이 먼저 낚아채
#     「작업 자동 저장」이라는 맹탕 메모로 올려버렸다. 여러 세션이 같은 본판을 공유하니
#     이 경합 자체는 못 없앤다 — 대신 «나중에 되짚을 수 있게» 파일명이라도 남긴다.
#     (근본 해결은 어시가 파일을 만들자마자 바로 커밋하는 것이다. 훅이 할 일이 아니다.)
_n=$(git diff --cached --name-only | wc -l | tr -d ' ')
_f=$(git diff --cached --name-only | head -3 | sed 's|.*/||' | paste -sd' ' - 2>/dev/null)
[ "$_n" -gt 3 ] && _f="${_f} 외 $((_n - 3))개"
git commit -m "chore: 작업 자동 저장 (${ts}) — ${_f:-변경분}" >/dev/null 2>&1 || exit 0
