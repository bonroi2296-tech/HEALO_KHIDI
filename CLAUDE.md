...
- **쉽게 설명 + 선택지는 "누르는 버튼"으로 (PO가 직접 요청 — 어기지 마라)**: 비개발자가 바로 이해하게 비유·일상어로 풀어라. 그리고 **PO가 뭔가 고르거나 결정해야 할 땐, 텍스트로 A/B/C 늘어놓지 말고 반드시 `AskUserQuestion` 도구로 "눌러서 고르는 버튼"을 띄워라.** PO는 타이핑 대신 버튼 클릭으로 답하길 원함. (텍스트 선택지 나열 = 위반)

---

## ⚠️ 자동 운영 규칙 — PO 번거로움 최소화 (2026-06-22 PO 요청·어기지 마라)

**PO가 직접 봐야 할 건 딱 3가지뿐: ①돈 나가는 것 ②화면에 보이는 변경(UI·카피) ③되돌리기 어려운 것(DB·보안·삭제).** 그 외는 어시스턴트가 알아서 + 자동검사로 닫는다. 아래는 PO가 반복 확인 안 하게 하는 고정 규칙.

1. **저위험 PR은 CI 통과 시 자동 머지** — 타입·테스트·가드·문서·비시각 변경이고 CI 초록이면 PO 머지버튼 기다리지 말고 어시스턴트가 머지(가능하면 GitHub auto-merge 설정). **단 위 3가지(돈·보이는 화면·되돌리기 어려움)에 해당하면 PO 확인/프리뷰 먼저.** AI 챗 응답 카피 변경처럼 사용자에게 보이는 것도 ②에 포함 → 확인.
