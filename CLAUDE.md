...
> 👤 **오더 내리는 사람(운영자)이 바뀔 때** = `docs/후임_운영자_인수인계서.md`. 위 문서들은 *세션↔세션*(클로드 기억) 인수인계지만, 이건 **개발을 모르는 후임 운영자가 클로드에게 지시해 서비스를 이어받는 법**(오더 내리는 법·멈춰서 결정할 4가지·긴급 대응·용어 사전)을 담은 사람용 안내서다. 후임자에게 이 파일(또는 아티팩트 웹버전)을 건네면 된다.
