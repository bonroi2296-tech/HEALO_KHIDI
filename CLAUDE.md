...
- **쉽게 설명 + 선택지로 줘라 (PO가 직접 요청 — 어기지 마라)**: 비개발자가 바로 이해하게 비유·일상어로 풀고, 무엇을 결정/검토해야 하는지는 **열린 질문 대신 고른 선택지(예: A/B/C, ✅/❌, "이거 할까요 / 나중에")로 제시**. PO는 긴 서술보다 "뭘 누르면 되는지·뭘 고르면 되는지"를 원함. 결정이 필요하면 AskUserQuestion 적극 사용.
