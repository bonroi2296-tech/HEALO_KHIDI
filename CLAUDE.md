...
1. **저위험 PR은 CI 통과 시 자동 머지(프로덕션까지)** — 타입·테스트·가드·문서·비시각 변경 + **저위험 UI(오타·번역·작은 문구·색 미세조정·명백한 버그 수정)**까지 CI 초록이면 PO 머지버튼 안 기다리고 어시스턴트가 머지·배포.
   **⚠️ 2026-07-28 갱신 — 「즉시 머지」가 아니라 「대기열에 넣기」다**: 자동머지 대상이라고 판단했으면
   `gh pr edit <번호> --add-label merge-queued` 로 라벨만 달아라. 실제 머지는 하루 한 번
   오후 3시 창구(`Batch Merge` 워크플로)가 모아서 한다. 빌드비 때문 — 상세는 「빌드 & 배포」.
   급한 것(장애·보안·PO 요청)만 예외로 즉시 머지. **큰 UI/플로우/카피 톤 변경**은 막지 말고 **만들어서 프리뷰 링크를 남겨라 → PO가 나중에 비동기로 검토**(머지 전 자연스러운 검토 기회). 멈춰 세우는 건 ①돈 ②되돌리기 어려움뿐.
...
- **Production: 「모아서 배포」 (2026-07-28 PO 결정).** PR 이 끝나면 **바로 머지하지 말고
  `merge-queued` 라벨만 달아라.** 하루 한 번 **KST 오후 3시**에 `Batch Merge` 워크플로가
  모아서 머지하고, 프로덕션 빌드는 **배치의 마지막 커밋 1건만** 돈다.
  - 창구가 하루 하나뿐이라 **PR 이 최대 24시간 대기**한다. 그래서 급한 것
    (**장애·보안·PO가 지금 보자고 한 것**)은 라벨 없이 그냥 즉시 머지해라 — 참지 말 것.
  - 지금 당장 창구를 열려면: `gh workflow run "Batch Merge (모아서 머지)"`
