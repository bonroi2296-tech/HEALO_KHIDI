...
## 코딩 원칙 — ponytail 미니멀룰 (상시 적용)

> 출처: ponytail (github.com/DietrichGebert/ponytail, MIT). PO 요청으로 레포에 룰 내장 — 로컬·클라우드 세션 모두 적용. "가장 좋은 코드는 안 짠 코드."

**코드 작성 전, 첫 번째로 해당되는 규칙에서 멈춘다:**
1. **YAGNI** — 애초에 만들 필요가 있나?
2. **표준 라이브러리** — 이미 거기 있나?
3. **플랫폼 기본 기능** — 네이티브로 되나?
4. **기존 의존성** — 이미 설치돼 있나?
5. **간결성** — 한 줄로 되나?
6. **최소 구현** — 그제서야 가장 작은 동작 코드.

**핵심**: 요청 안 한 추상화·보일러플레이트 금지 / 새 의존성 지양 / 추가보다 삭제·영리함보다 단순함 / 파일 최소 / 의도적 단순화는 `ponytail:` 주석으로 한계 표기.

**⚠️ 미니멀룰 예외(절대 양보 금지 — 이 의료 플랫폼에선 특히 중요)**: 신뢰경계 입력검증 · 데이터 손실 방지 에러처리 · **보안** · 접근성 · 명시적으로 요청된 기능. (= 보안/검증/암호화는 "간결함"을 이유로 줄이지 않는다.)

**테스트**: 비자명한 로직은 작은 실행 검증 1개(assert 데모 또는 프레임워크 없는 최소 테스트). 한 줄 함수는 불필요.
