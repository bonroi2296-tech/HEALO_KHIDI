...
| # | 업체 | 역할 | 처리 방법 | 비고 |
|---|---|---|---|---|
| 1 | **Supabase** | DB·인증·스토리지(환자정보 본체) | 조직 설정 → Legal/Compliance(또는 Security)에서 **DPA 검토·서명**. 보안문서(SOC2 등)도 다운로드. | 가장 중요(데이터 본체). 데이터는 서울 리전. HIPAA BAA는 상위 플랜 애드온일 수 있음 — 필요 시 문의. |
