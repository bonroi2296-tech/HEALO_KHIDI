...
## 2026-07-28 완성도 감사 ② (범위: diff — 콘텐츠 편집기 「저장 전 미리보기」 신설, 반성문 #139 방어)
- 스캔: DoD-2(성공 실측) · DoD-3(문서-현실 드리프트) · DoD-7(시각 완결성 — 확인창을 모바일 375px·데스크톱 1280px 두 폭에서)
- **발견 1건 → 수정: DoD-3 드리프트.** 사용설명서가 「**항목 이름**과 이전→새 문구를 훑어보라」고 하는데, 사전 키(`intakeLabels.cancer.*` 등)는 사람이 읽는 이름이 없어 화면엔 **키 경로만** 뜬다. → 「어떤 항목인지(이름이 있으면 이름, 없으면 항목 코드)」로 문구 교정 + 반성문 #139 의 기능 서술도 실제(라벨 병기·목록 얼림·줄바꿈 보존)에 맞게 보강.
- 무발견(실측 근거):
