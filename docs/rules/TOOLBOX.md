...
## 4. 🔌 붙일 후보 (2026-09-10 조사, PO 「내일 다 붙여준다」)

> **결론부터: 크롬 개발자도구와 캔바 둘만 먼저 붙인다.** 다섯 개를 한꺼번에 붙이면
> 위 §1 의 「플러그인 12개 3개월 0회」가 그대로 재현된다. 붙이는 건 하루, 쓰는 습관은 아니다.
> 전부 **공식 문서 원문에서 도구 목록을 직접 확인**한 것이다(검색 요약 아님).

| 순위 | 무엇 | 도구 수 | 설치 | 무엇이 바뀌나 |
|---|---|---|---|---|
| 1 | **크롬 개발자도구**(구글 공식) | 50+ | Node LTS + 크롬 | 라이트하우스·콘솔·네트워크·성능추적·화면녹화가 한 세션에서 |
| 2 | **캔바**(공식 AI 커넥터) | 32 | 로그인만 | 디자인 생성·**편집**·PPTX 내보내기. **무료 요금제로 됨** |
| 3 | GA4(구글 공식) | 7 | 파이썬+gcloud (손 감) | 유입·이탈·전환을 어시가 직접 읽음. **읽기 전용** |
| 4 | 슬랙(공식) | 미확인 | 로그인 | 메시지 읽기·쓰기·검색. 도큐사인은 **슬랙 앱**이라 이게 전제 |
| 5 | 피그마 | 20+ | 중간 | 선택 기반 작업은 **데스크톱 앱이 켜져 있어야** 함 |
| 보류 | 구글 광고(구글 공식) | 3 | 개발자 토큰 승인 필요 | **읽기만**. 입찰·정지·소재 생성 불가. 광고 집행 시작할 때 |
