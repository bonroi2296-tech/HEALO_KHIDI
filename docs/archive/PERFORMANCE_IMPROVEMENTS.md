...
#### 변경 파일 목록
- `next.config.js` (신규 생성)
- `src/App.jsx`
- `src/lib/logger.ts` (신규 생성)

#### 변경 이유
- **Lighthouse 항목**: `unused-javascript` (약 794KiB 절감 목표)
...
- `lucide-react`와 `@supabase/supabase-js`의 tree-shaking 최적화 (`optimizePackageImports`)
...
```javascript
// next.config.js: vendor 청크 분리
splitChunks: {
  cacheGroups: {
    react: { test: /react|react-dom|react-router-dom/, name: 'react-vendor' },
    supabase: { test: /@supabase/, name: 'supabase-vendor' },
    ai: { test: /@ai-sdk|ai/, name: 'ai-vendor' },
    googleMaps: { test: /@react-google-maps/, name: 'google-maps-vendor' },
  }
}
```
...
- Google Analytics 스크립트는 이미 `strategy="afterInteractive"`로 설정되어 있음
- CSS는 Next.js가 자동으로 최적화하지만, 폰트 최적화 명시적 활성화
- SWC minifier 사용으로 빌드 시간 및 번들 크기 최적화

#### 주요 변경 내용
```javascript
// next.config.js
optimizeFonts: true,  // 폰트 최적화
swcMinify: true,      // SWC minifier 사용
```
...
- `src/lib/logger.ts` (신규 생성)
- `src/App.jsx`
- `src/lib/data/treatments.js`
- `src/lib/data/hospitals.js`
- `src/legacy-pages/TreatmentDetailPage.jsx`
- `next.config.js`

#### 변경 이유
- **Lighthouse 항목**: `errors-in-console` (0점 → 100점 목표), `valid-source-maps` 개선
- 프로덕션 환경에서 `console.error` 제거로 브라우저 콘솔 오류 감소
- `productionBrowserSourceMaps: true`로 소스맵 생성하여 디버깅 가능성 유지

#### 주요 변경 내용
```typescript
// src/lib/logger.ts: 프로덕션 환경에서 console.error 조건부 처리
export const logError = (context: string, error: any, details?: Record<string, any>) => {
  if (isDev) {
    console.error(`[${context}]`, error, details || '');
  } else {
    // 프로덕션: 에러 추적만 (필요시 Sentry 등 연동 가능)
  }
};
```

```javascript
// 모든 console.error를 logError로 교체
// src/App.jsx, src/lib/data/*.js 등
logError("context", error);
```

```javascript
// next.config.js
productionBrowserSourceMaps: true,  // 소스맵 생성
```
