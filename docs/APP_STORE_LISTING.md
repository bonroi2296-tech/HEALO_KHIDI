...
- ⬜ **남은 앱 설정 과제 (진행률 6/11)**: 콘텐츠 등급(IARC 설문) · 타겟층 · **데이터 보안**(설문, 답안=`APP_STORE_REVIEW_ANSWERS.md`) · 스토어 등록정보 → 그 다음 AAB 업로드(내부 테스트).
  - ⚠️ **URL 을 손으로 찍지 마라** — `app-content/government-app` 같은 추측 경로는 앱 목록으로 튕긴다. 대시보드 「할 일 보기」의 링크를 클릭할 것.
