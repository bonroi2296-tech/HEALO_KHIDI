...
## 6. 콜드 아웃리치 플레이북 (해외 파트너 발굴용)

> DNS 인증(1~5순위)은 **스팸함행을 막는다.** 이 플레이북은 **낯선 도메인이라 무시·삭제당하는 걸 막는다.** 둘 다 있어야 해외 메일링이 먹힌다.
> 대상: CIS(러시아·카자흐 등) 병원·에이전시·의사 = 환자 소개 파트너.

### 전제 — 우리 콜드메일은 "대량 블라스트"가 아니라 "표적 1:1"
초기 규모(하루 10~30통, 실제 담당자에게 개인화)에선 **개인화 1:1 메일이 스팸도 안 되고 답장률도 높다.** 대량 발송 툴(Instantly 등)은 아직 살 필요 없음 — 오버스펙.

### A. 발신 주소
- **지금(저볼륨)**: `partners@healwith.co.kr` 같은 전용 주소 하나로. 볼륨 낮고 품질 높으면 본 도메인 위험 낮음 → 서브도메인 분리는 아직 오버(Zoho 무료는 도메인 1개라 서브도메인 추가도 번거로움).
- ⚠️ **미래(하루 100통+ 대량)**: 그때 별도 저가 도메인(예 `healwith-partners.com`) 사서 격리 + 워밍업 + 전용 툴. 본 도메인 평판 보호.

### B. 워밍업 (제일 중요)
- 새 발신 주소는 첫 주 하루 5~10통 → 2주차 15~20 → 점진 증량. **하루아침에 대량 = 스팸 확정.**
- 시작 며칠은 내부/지인과 몇 통 주고받아 "정상 계정" 신호부터 만들기.

### C. 명단 품질 (반송률이 평판 직격)
- 실제 존재하는 담당자 주소만. 추측/스크래핑 남발 X.
- 발송 전 이메일 검증으로 죽은 주소 거르기 — **반송률 2~3% 넘으면 평판 급락.**
- 오래된·구매 명단 금지(스팸 트랩 = 방치된 미끼 주소에 걸리면 즉시 블랙리스트).

### D. 메일 내용 (스팸필터 + 사람 둘 다 통과)
- **러시아어로 써라.** 타겟이 러시아·카자흐면 영어보다 신뢰·응답 다 높음.
- 개인화 첫 줄(그 병원/사람 구체 언급). 템플릿 티나면 스팸+무시.
- 첫 메일엔 **링크 1개·이미지 최소·첨부 지양**(많으면 스팸점수↑).
- 스팸 유발 단어 회피(무료, 100%, 지금 당장, $$$ 등).
- 발신 표시명 = 실명/직책 + healwith. 하단에 **실제 서명**(이름·직책·회사·주소·전화) + 수신거부 한 줄.

### E. 신뢰 신호 — "무시당함"의 진짜 해법
낯선 도메인이 무시당하는 걸 막는 건 인증(DMARC)만이 아니라 **첫 메일에서 "우리가 진짜 정부지원 정식 기관"임을 보여주는 것**:
- KHIDI(한국보건산업진흥원) 정부지원과제
- 외국인환자 **유치업 등록번호 A-2026-01-02-06761** + SGI 보증보험 1억
- 실제 병원 파트너·홈페이지(https://healwith.co.kr)
→ 인증 = 스팸함 방지, 신뢰신호 = 무시 방지. 역할이 다름.

### F. 팔로업 & 측정
- 팔로업은 3~4일 간격 **2회까지.** 그 이상 = 스팸신고 유발.
- 지표: 반송률<3%, 스팸신고<0.1%, 답장률. 반송 높으면 명단부터 정리.
...
### 진단 로그 (2026-07-13 실측)
- 제보 출처: Assel이 `assel@healwith.co.kr`에서 러시아권으로 발송 시 "안 간다".
- **assel@ 우편함 전수 확인: 보낸 메일 0통.** 대기열·스팸함도 0. 유일한 흔적 = 7/3 미발송 초안 1개(gmail 수신자, 제목 없음). → "보냈는데 반송"이 아니라 **발송 시도 자체가 성공한 적 없음**.
- 계정 접근 문제 정황: 7/13 PO가 비밀번호 리셋 후에야 로그인 가능 (Assel이 계정을 제대로 못 쓰고 있었을 가능성).
- **테스트 발송 (7/13 11:37, assel@ → bonroi2296@gmail.com): 성공, Gmail 받은편지함 정상 도착 (스팸 아님).** → 계정 발송 기능·도메인 평판 모두 정상.
- **mail-tester.com 정밀검사 (7/13 11:52, 러시아어 실전형 아웃리치 본문으로): 10/10 만점.**
  - SPF PASS / DKIM PASS(zmail) / DMARC PASS. SpamAssassin 점수 -0.2(우수).
  - Zoho 발신 IP `136.143.188.54`(sender4-of-o54.zoho.com): 블랙리스트 없음 + Mailspike **화이트리스트** "Very Good reputation(+4)". rDNS 정상.
  - → **발송 인프라·인증·IP평판·콘텐츠 전부 결백 확정.** "Zoho라서 러시아에 못 보낸다"는 가설 기각.
- 남은 유일 용의자 = **Assel의 발송 시도 자체** (계정 로그인/사용 문제로 발송이 아예 안 됐을 가능성). Assel 답변 대기: ①어느 계정에서 보냈나 ②보내기 시 에러 문구·스크린샷 ③assel@ 로그인 가능했나. + (선택) 실제 .ru 주소로 교차 테스트.
