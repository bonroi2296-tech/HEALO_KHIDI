...
GLOBAL = [
    ...
    # 서비스명은 healwith 하나(2026-08-20 PO 결정 — 옛 이름은 상표권 이슈로 병기도 금지).
    # 머리말·꼬리말까지 iter_paragraphs 가 훑으므로 여기 두면 전 문서에 먹는다.
    ("HEALO 플랫폼", "healwith 플랫폼"),
    ("HEALO Platform", "healwith Platform"),
    ("HEALO-2026-DOC", "healwith-2026-DOC"),
    ("HEALO-REQ", "healwith-REQ"),
    ("HEALO-FSD", "healwith-FSD"),
    ("HEALO-EVAL", "healwith-EVAL"),
    ("HEALO-KHIDI-2026", "healwith-KHIDI-2026"),
    ("HEALO 배포본", "healwith 배포본"),
    ("HEALO (2026년", "healwith (2026년"),
    ("HEALO는", "healwith 는"),
    ("HEALO 는", "healwith 는"),
    ("HEALO 현재", "healwith 현재"),
    ("플랫폼 HEALO", "플랫폼 healwith"),
    ("(HEALO DB)", "(healwith DB)"),
    ("(HEALO)", "(healwith)"),
    ("HEALO 코드베이스", "healwith 코드베이스"),
    ("HEALO 평가 매트릭스", "healwith 평가 매트릭스"),
    ("HEALO 시스템", "healwith 시스템"),
    # 기준일 — 2026-08-19/20 판을 2026-09-06 으로
    ("2026년 8월 19일", "2026년 9월 6일"),
    ("2026년 8월 20일", "2026년 9월 6일"),
    ("(8월 19일)", "(9월 6일)"),
    ("(8월 20일)", "(9월 6일)"),
]
...
REPLACEMENTS = {
    ...
    "08_테스트결과서.docx": [
        ...
        ('27건(2026-04 수동) · 자동 스모크 97건(2026-08-20 실측)',
         f'27건(2026-04 수동) · 자동 스모크 {F.QUALITY["smoke_tests"]}건(2026-09-06 --list)'),
        ...
        ('5.2 자동화 시험 보강 내역 (2026-04 착수 점검 지적사항 해소) (2026-04 지적 → 2026-08-20 해소 내역)',
        ...
        ('5.2 자동화 테스트 미흡 영역 (2026-04 지적 → 2026-08-19 해소 내역) (2026-04 지적 → 2026-08-19 해소 내역) (2026-04 지적 → 2026-08-19 해소 내역) (2026-04 지적 → 2026-08-19 해소 내역)',
         "5.2 자동화 시험 보강 내역 (2026-04 착수 점검 지적사항 해소)"),
        ("5.3 테스트 자동화 진행 상황 (2026-08-19 기준)", "5.3 테스트 자동화 진행 상황 (2026-09-06 기준)"),
        ("«수동 확인»", "수동 확인"),
        ...
        ("게스트 토큰 인테이크", "링크로 문의 접수·진행상황"),
        ("게스트 링크 접속 → 로그인 없이 인테이크 폼 접근 가능", "링크 접속 → 로그인 없이 문의 접수·진행상황(/claim) 접근 가능"),
        ("통번역 3,543건(시험 방 제외 3,277건)", "통역 자막 3,735건(시험 방 제외 3,396건, 2026-09-06)"),
        ("5-Step 인테이크 완료", "문의 접수(1단계+2단계) 완료"),
        ("Step 3에서 PDF 파일 첨부", "2단계에서 PDF 파일 첨부"),
        # ↓ 2026-09-06 재측정
        ("48개 파일 152건", f"{F.QUALITY['e2e_files']}개 파일 {F.QUALITY['e2e_tests']}건"),
        ("146개 파일 1,289건", f"{F.QUALITY['unit_files']}개 파일 {F.QUALITY['unit_tests']:,}건"),
        ("146개 파일 / 1,289건", f"{F.QUALITY['unit_files']}개 파일 / {F.QUALITY['unit_tests']:,}건"),
        ("48개 파일 / 152건", f"{F.QUALITY['e2e_files']}개 파일 / {F.QUALITY['e2e_tests']}건"),
        ("전건 통과 (2026-08-20 실행, 32.7초)", F.QUALITY["unit_result"]),
        ("Critical: 0건, High: 0건, Moderate: 2건 (운영 의존성 기준, 2026-08-20 실행)", "Critical: 0건, High: 0건, Moderate: 3건 (2026-09-06 실행. 셋 다 dev 도구 경로라 실서비스 번들에 안 실린다)"),
        ("E2E 테스트는 3개 사용자 역할(환자/코디네이터/관리자) 기준", "E2E 테스트는 6개 역할 세션(환자·코디네이터·관리자·해외 에이전시·해외 의료기관·국내 병원) 기준. 운영 Supabase 가 아니라 «검사 전용» Supabase 프로젝트를 본다"),
        ("로컬 개발 서버 + 운영 Supabase (별도 Staging DB 없음)", "로컬 개발 서버 + 운영 Supabase(단위·수동) / 자동 E2E 는 GitHub Actions + 검사 전용 Supabase 프로젝트(2026-08-30 확인)"),
        ("Vitest + 운영 Supabase", "Vitest + 운영 Supabase(단위) · Playwright + 검사 전용 Supabase(E2E)"),
        ("동일 IP에서 60회/분 초과 요청 → 429 Too Many Requests 반환", "동일 IP 에서 관문별 상한(기본 20회/분, 접수 5회/분) 초과 요청 → 429 반환. 인스턴스별 메모리가 아니라 공용 저장소로 센다(check:ratelimit-scope)"),
        ("2.2 인테이크 플로우", "2.2 문의 접수 플로우"),
        ("(회원가입~인테이크~화상상담)", "(회원가입~문의 접수~화상상담)"),
        ("→ 인테이크 DB 저장 확인", "→ inquiries·cancer_patient_intakes 저장 확인"),
        ("인테이크 제출 후 DB에 성명 평문 저장 없음 확인", "문의 접수 후 DB에 성명 평문 저장 없음 확인"),
        ("인테이크 제출 → 코디네이터 이메일 알림 수신 확인", "문의 접수 → 코디네이터 메일·종 알림 수신 확인(시험 문의는 제외)"),
        ("인테이크 제출 후 DB 직접 조회", "문의 접수 후 DB 직접 조회"),
        ("인테이크 E2E | 5건", "문의 접수 E2E | 5건"),
        ("2026-08-20 실측. 실패 2건은 의뢰서 접수 뒤 완료 화면을 기다리는 검사로, 다시 돌리면 실패 건수가 달라지고(2건→1건) 자동 검사 서버에서는 통과한다. 접수 자체는 운영DB 저장까지 확인했다",
         "2026-08-20 실측(실패 2건은 의뢰서 접수 뒤 완료 화면 대기 검사 — 자동 검사 서버에서는 통과). 2026-09-06 본판 자동 E2E 최근 40회 중 성공 32·취소 8·실패 0. 매 실행 1건씩 흔들리던 원인은 개발 서버가 힙 한계로 재시작하는 것(2026-08-31 규명)"),
        ("성능 자동화: Lighthouse CI 파이프라인 미설정 (예정)", "성능·접근성 자동화: audit-live.yml 이 매주 월요일 axe-core(WCAG 2.1 AA)·Lighthouse 를 돌린다. 로그인 뒤 화면 22개는 8/17~8/31 측정 실패 → 9/06 수리, 9/07 첫 재측정"),
        ("[예정] 부하 테스트 (100 동시 접속)", "[예정] 부하 테스트 (100 동시 접속): 실서비스 상태 확인 주소로 동시 100건은 100/100 성공(2026-08-20). 쓰기 포함 본격 부하 시험은 유치 확대 시점"),
        ("[예정] 외부 침투 테스트", "[진행] 외부 침투 테스트 대신 해커 관점 자체 감사 9라운드(2026-09-05 #1615: 공개 엔드포인트·내부정보 노출·설문 레이스·PII 접속 기록) + anon RPC 회수(#1622). 외부 전문가 의뢰는 미결"),
        ("실 사용자 기반 Core Web Vitals 측정 | Vercel Analytics", "실 사용자 기반 Core Web Vitals 측정 | Vercel Analytics 는 켜져 있지 않다(2026-09-05 실측). Sentry 성능 + Lighthouse 주간 실측으로 대체"),
        ("완료 (성능 수치 갱신 예정)", "완료 (2026-09-06 재측정 반영)"),
    ],
    "09_산출물목록.docx": [
        ...
        ("Vercel 배포 설정: crons, rewrites, headers", f"Vercel 배포 설정: 정기 실행 {len(F.CRONS)}종 · 보안 머리값 · 빌드 창구 판정"),
        ("Vercel 배포 설정: 정기 실행 9종 · 보안 머리값 2종 · 빌드 창구 판정", f"Vercel 배포 설정: 정기 실행 {len(F.CRONS)}종 · 보안 머리값 · 빌드 창구 판정"),
        ("migrations/ (154개 파일)", f"migrations/ ({F.ROUTE_STATS['migrations']}개 파일)"),
        ("2026년 8월 | 중간보고", "2026년 8~9월 | 중간보고"),
        ("Phase A 5건 + Phase B 4건 + 기술·운영 문서 6건 = 15건", "Phase A 7건 + Phase B 4건 + 기술·운영 문서 6건 + 운영 대장 7종 = 24건"),
        ("14개 기능그룹 상세 명세, 구현 현황(완료96%/부분4%) (~30p)", "21개 기능그룹·기능 ID 41개 상세 명세 + 남은 일 부록, 구현 현황(완료 95%/부분 5%) (~45p)"),
        ("페르소나 4종, FR-01~28, NFR 6범주, 외부시스템 9종, 용어 28개 (~22p)", "페르소나 4종, FR-01~37 + 추적표, NFR 23개(값마다 실측일), 외부시스템 15종, 용어 28개 (~26p)"),
        ("lo-fi 와이어프레임 10종(P-01~P-10) + 화면별 목적·구성요소·데이터·연결 (~23p)", "lo-fi 와이어프레임 11종(P-01~P-11) + 시스템 구성도 + 화면별 목적·구성요소·데이터·연결 (~25p)"),
        ...
        ("01-1_화면설계서.docx (화면 10종 lo-fi 설계)", "01-1_화면설계서.docx (화면 11종 lo-fi 설계 + 시스템 구성도)"),
    ...
    "06_사용자매뉴얼.docx": [
        ...
        # ↓ 2026-09-06: 옛 낱말·구조 정리
        ('1.2 인테이크 폼 작성', '1.2 문의 접수(1단계·2단계)'),
        ('  1.2 인테이크 폼 작성 | 4', '  1.2 문의 접수(1단계·2단계) | 4'),
        ('2.3 인테이크 검토 및 병원 배정', '2.3 문의 검토 및 병원 배정'),
        ('  2.3 인테이크 검토 및 병원 배정 | 10', '  2.3 문의 검토 및 병원 배정 | 10'),
        ('제 3장. 의료진 사용 안내 (Doctor Guide)', '제 3장. 국내 의료기관 담당자 사용 안내 (Hospital Guide)'),
        ('제 3장. 의료진 사용 안내 | 13', '제 3장. 국내 의료기관 담당자 사용 안내 | 13'),
        ('3.1 파트너 병원 포털 접속', '3.1 의료기관 포털 접속'),
        ('  3.1 파트너 병원 포털 접속 | 13', '  3.1 의료기관 포털 접속 | 13'),
        ('회원가입 없이도 [게스트 상담] 기능으로 인테이크 폼 작성 가능: 링크를 받은 경우 바로 접속',
         '회원가입 없이도 문의를 접수할 수 있다. 접수하면 진행상황 링크(/claim/…)를 그 채널로 돌려주며, 링크만으로 진행·소견·서류를 본다'),
        ('Step별 입력 내용', '1단계·2단계 입력 내용'),
        ('Step 1 | 기본 정보 | 성명, 연락처(WhatsApp), 국가, 생년월일', '1단계 | 접수 | 이름 · 연락 수단(왓츠앱·텔레그램·메일) · 국적 · 선호 언어 · 암종·병기 · 하고 싶은 말 · 동의(필수 4 + 선택 1)'),
        ('Step 2 | 암 정보 | 암종, 병기, 현재 치료 상태, 기왕력', '2단계 | 임상 정보 | 진단일 · 현재 치료 상태 · 기왕력 · 병원에 전할 요청(/inquiry/intake, 문의번호+접근 토큰)'),
        ('Step 3 | 의료기록 업로드 | CT/MRI/혈액검사 등 파일 첨부 (PDF·JPEG·PNG, 최대 10개·건당 200MB)', '2단계 | 의료기록 | CT/MRI/혈액검사 등 첨부(PDF·JPG·PNG·Word·DICOM·ZIP/RAR·음성 메모, 건당 200MB)'),
        ('Step 4 | 희망 치료 | 치료 방법, 예산, 입국 가능 시기, 동행자', '2단계 | 희망 사항 | 치료 방법 · 예산 · 입국 가능 시기 · 동행자'),
        ('Step 5 | 개인정보 동의·제출 | 개인정보 처리 동의, 의료 정보 동의 후 제출', '— | 자동 저장 | 2단계는 0.4초마다 자동 저장되며 나중에 이어서 쓸 수 있다'),
        ('인테이크 항목 검토: 암종, 병기, 업로드 의료 기록 확인', '접수 내용 검토: 암종·병기·첨부 의료 기록·AI 케이스 브리프 확인'),
        ('인테이크 폼 제출 완료: 접수 확인 이메일', '문의 접수 완료: 접수 확인 이메일(시험 문의엔 보내지 않는다)'),
        ('상세 페이지에서: 인테이크 정보, 의료 기록, 채팅 이력, 예약 현황 확인', '상세 페이지에서: 접수 정보, 의료 기록, 채팅 이력, 예약 현황, 소견, 병원 의뢰서 확인'),
        ('진료 의견서는 법적 의료 문서가 아니며 참고 의견으로만 사용된다. 정식 진료는 입국 후 대면 진료에서 진행.',
         '소견은 법적 의료 문서가 아니며 참고 의견으로만 사용된다. 정식 진료는 입국 후 대면 진료에서 진행. 환자는 /claim/… 진행상황 화면에서 코디가 공개한 공식 문서로만 본다.'),
    ],
    "07_관리자매뉴얼.docx": [
        ("/admin/intake/[id] 에서", "/admin/inquiries 에서"),
        ("3.1 인테이크 배정 프로세스", "3.1 문의 배정 프로세스"),
        ("인테이크 제출 → 코디네이터 첫 연락 시간 (목표: 24시간 이내)", "문의 접수 → 코디네이터 첫 연락 시간 (목표: 24시간 이내. 7일 무동작은 「식은 문의」 크론이 잡는다)"),
        ("HEALO v1.0 (2026년 4월 기준)", "HEALO (2026년 8월 19일 기준)"),
        ("2026-04-30", "2026-08-19"),
        ...
        # 2026-09-06 정정: 아래 두 줄이 먼저 걸려 L358 의 kpi-dashboard 규칙이 영영 못 걸렸다(한 절 안에 주소 두 개).
        ("/admin/statistics 접속", "[홈 › KHIDI 리포트] 메뉴 클릭 (/admin/khidi/kpi-dashboard)"),
        ("/admin/khidi/conversion 접속", "[홈 › KHIDI 리포트] 메뉴 클릭 (/admin/khidi/kpi-dashboard)"),
        ("[화면: /admin/statistics 페이지]", "[화면: /admin/khidi/kpi-dashboard 페이지: KHIDI 리포트]"),
        ...
        ("2026-08-19", "2026-08-20"),
        # ↓ 2026-09-06 실측 정정(코드 대조)
        ("상태 변경: 신규접수 → 검토중 → 병원배정 → 상담완료 → 입원대기 → 사후관리",
         "문의 상태(inquiries.status)는 pending·received·completed·blocked·normalized·error 6종이고, 유치 결과는 별도 칸 outcome(admitted 유치 확정 / lost 이탈·종료 / 비어 있음 진행 중)에 적는다. 케이스 «단계»는 case_status_history 가 6단계로 따로 기록한다"),
        ("화상상담 내용은 녹화·저장하지 않는다. 개인정보 보호 정책 준수.",
         "녹화는 기본 꺼짐(CONSULT_RECORDING_ENABLED). 켜면 90일 보관 뒤 크론이 자동 파기한다. 자막·번역 기록은 암호화 저장된다."),
        ("환자 유치 건수 | status = '입원대기' 이상으로 변경된 환자 건수", "환자 유치 건수 | inquiries.outcome = 'admitted' (코디 「유치 확정」 또는 병원 포털 「치료 확정」). 시험 문의 제외"),
        ("status = '입원대기' 이상으로 변경된 환자 건수", "inquiries.outcome = 'admitted' (코디 「유치 확정」 또는 병원 포털 「치료 확정」). 시험 문의 제외"),
        ("consultation_sessions 테이블 completed 건수", "영상 사전상담 완료 세션 + 환자에게 전달된 소견(case_opinions.released_at) 합산. 시험 문의·시험 방 제외 — 정의는 src/lib/khidi/kpi.ts"),
        ("inquiries.country 기준 집계", "inquiries.nationality 기준 집계(normalizeNationality 로 표기 통일)"),
        ("/admin/inquiries 에서 [코디네이터 배정] 클릭 후 담당자 선택",
         "코디 화면 [의뢰·케이스/병원배정]에서 문의를 케이스로 전환하고 담당·병원을 정한다(관리자 문의 화면에는 배정 단추가 없다)"),
        ("코디네이터에게 배정 알림 이메일 자동 발송", "케이스 이력에 남고 담당자 종 알림이 간다"),
        ("GitHub main 브랜치 + GitLab 자동 미러 (ops/gitlab-mirror 워크플로우)",
         "GitHub main 브랜치 + GitLab 자동 미러 (.github/workflows/gitlab-mirror.yml 「Mirror to GitLab」)"),
        ("[리포트 다운로드] 클릭: CSV 또는 PDF 형식", "[내려받기] 클릭: CSV 형식(PDF 출력은 없다)"),
        ("Analytics 탭: 방문자 수, 페이지별 응답 시간(Core Web Vitals) 확인",
         "Analytics 탭: 켜져 있지 않다(2026-09-05 실측 API 404). 방문자 분석은 GA4·얀덱스 메트리카, 오류는 Sentry(bonroi.sentry.io), 생사는 UptimeRobot"),
        ("Vercel 배포 트리거: 하루 한 번(KST 15시) Daily Deploy 창구가 main 을 production 브랜치로 밀 때만 빌드된다. main 에 합쳤다고 바로 실서비스에 나가지 않는다.",
         "Vercel 배포 트리거: 하루 한 번(KST 15시) Vercel 예약(/api/cron/daily-deploy)이 main 최신 커밋을 실서비스로 내보낸다. main 에 합쳤다고 바로 나가지 않으며, 나갔는지는 /api/health 의 commit 으로 판정한다(긴급은 커밋 제목 [deploy])."),
    ],
}
...
PLAN_MAP_ROWS = [
    ...
    ("", "UI/UX 설계서", "01-1_화면설계서.docx (화면 11종 lo-fi 설계 + 시스템 구성도)", "충족"),
...
def add_appendix(doc, kind):
    ...
    if kind == "user":
        sec("코디네이터 메뉴 16개 (본문에서 다루지 않은 것 포함)")
        para(doc, "정본은 app/coordinator/layout.jsx. 본문 제2장은 문의함·케이스·상담 일정·메시지 네 메뉴만 다룬다.", size=9)
        table(doc, ["메뉴", "경로", "무엇을 하나"], [
            ("대시보드", "/coordinator", "대기 문의·오늘/예정 상담·긴급 알림 카드"),
            ("음성 정리", "/coordinator/voice", "왓츠앱·텔레그램 음성 메모를 AI 가 받아쓴 것을 읽는다(문의 생성 전에도)"),
            ("문의함", "/coordinator/inbox", "전체 / 추가 정보 필요 / 매칭 준비 완료 / 종료 탭 · 「N일째 정체」·「환자 새 글」 배지"),
            ("AI 상담 리드", "/coordinator/chat", "AI 대화 중 문의로 승격된 것·연락처 확보 현황"),
            ("의뢰·케이스/병원배정", "/coordinator/cases", "문의를 케이스로 전환, 병원 배정, 케이스 브리프"),
            ("상담 일정", "/coordinator/consultations", "상담 생성(환자·시각) → 초대 링크 → [상담 완료](실적 판정)"),
            ("유치 전환", "/coordinator/conversion", "깔때기 · 유치 확정 / 종료(환자가 안 온다고 함) 결과 입력"),
            ("사후관리·만족도", "/coordinator/satisfaction", "설문 응답·평균, 사후관리 차수"),
            ("파트너 발굴", "/coordinator/partners", "에이전시·병원 발굴 기록"),
            ("메시지", "/coordinator/messages", "웹·텔레그램·왓츠앱·에이전시 대화, 추천 답장"),
            ("비자 트래킹", "/coordinator/visa", "비자 신청 진행·서류"),
            ("견적", "/coordinator/cost-estimates", "견적 작성·이력, 환자 정식 견적 요청 확인"),
            ("증상 알림", "/coordinator/alerts", "사후관리 환자 위험도·침묵 경보"),
            ("콘텐츠 편집", "/coordinator/content", "화면 문구·치료법·암종·FAQ·병원 소개 6개 언어 직접 수정"),
            ("개선 요청함", "/coordinator/requests", "쓰다가 불편한 것을 그 자리에서 적는다"),
            ("설정", "/coordinator/settings", "언어·알림"),
        ])
        sec("환자 화면 빠른 메뉴 6종 + 진행상황 링크")
        table(doc, ["메뉴", "경로", "무엇을 하나"], [
            ("내 상담", "/patient/consultations", "잡힌 상담·대기실 입장"),
            ("의료 문서", "/patient/documents", "문서 올리기·보기(200MB)"),
            ("암 치료 가이드", "/education", "암 5종 가이드 6개 언어(공개 화면과 같다)"),
            ("증상 기록", "/patient/symptoms", "귀국 뒤 증상 입력 → 위험도 판정·담당자 알림"),
            ("재진 예약", "/patient/rebooking", "재방문 필요 판정 뒤 예약·견적·비자 재이용"),
            ("비자 가이드", "/visa", "C-3-3 / G-1-10 안내·서류"),
            ("진행상황(가입 불필요)", "/claim/<토큰>", "접수 시 받은 링크로 6단계 진행·소견·서류·추가 자료. 환자 글은 코디에게 배지로"),
            ("AI 건강 상담 · 달력 · 견적 · 계정", "/patient/chat · /patient/calendar · /patient/cost-estimates · /patient/account", "로그인 뒤 부가 화면"),
        ])
        sec("해외 에이전시·해외 의료기관 포털 (본문 미수록)")
        table(doc, ["계층", "경로", "할 수 있는 것"], [
            ("해외 에이전시", "/agency", "환자 대신 의뢰 · 케이스 6단계 진행 확인 · 코디와 케이스 메신저 · 공개된 소견 번역본 열람"),
            ("해외 의료기관", "/clinic", "위 전부 + 검사결과·영상·소견 업로드(경과 기록) — 사후관리 ICT ④"),
        ])
        para(doc, "계정은 관리자가 [에이전시·클리닉] 메뉴에서 상대 업무메일을 ID 로 발급한다. 자율 가입은 없다.", size=9)

    if kind == "admin":
        sec("관리자 메뉴 7그룹 (정본 = app/admin/_components/AdminNav.jsx)")
        table(doc, ["그룹", "화면"], [
            ("홈", "대시보드 · KHIDI 리포트(kpi-dashboard · north-star · conversion · satisfaction · evidence) · 문의 통계 · 광고 예산"),
            ("상담·문의", "문의·케이스 받은함 · 케이스 관리 · AI 채팅 · Human Agent 채널 · 화상 상담 · 협진 의뢰 · 후속 리마인더 · 환자 대화 · 견적 · 비자 · 증상 알림"),
            ("파트너·회원", "직원(코디) 계정 · 에이전시 · 클리닉 · 파트너 발굴 · 제휴 병원 · 병원 진료의뢰 · 환자 회원 · 데이터 삭제 요청(법정 의무 — 세 곳 동시 삭제)"),
            ("콘텐츠", "문구 편집기 · 환자 교육자료 · AI 지식베이스(RAG 문서 올리기)"),
            ("AI 품질", "자가시험(ai-regression) · 품질 판정(ai-quality) · 피드백 · 모델 벤치마크 · 에이전트 분석 · 사용량"),
            ("시스템", "개선 요청함 · 감사로그(admin_audit_logs 9,433행) · 외부 서비스 사용량 · 알림 관리 · 브랜딩 설정 · 관측(Sentry)"),
            ("비활성 화면", "메뉴에서 숨겼지만 주소로는 열리는 9종(대량 Import · 데이터 보강 · 크롤 · 플레이북 · 치료·암종·의료진·지점 · AI 피드백). npm run check:dead-screens 가 매달 실DB 와 대조"),
        ])
    ...
    if kind == "test":
        ...
        table(doc, ["구분", "규모", "결과"], [
            ...
            ("스모크 E2E(신청서마다)", f"{F.QUALITY['smoke_files']}개 파일 / {F.QUALITY['smoke_tests']}건", "자동 실행"),
            ("전체 E2E", f"{F.QUALITY['e2e_files']}개 파일 / {F.QUALITY['e2e_tests']}건",
             "본판 합침 뒤 + 매일 밤 실서비스"),
            ("의존성 취약점", "npm audit", F.QUALITY["npm_audit"]),
            ("AI 자가시험", "50문항 · 주 2회", F.AI_QUALITY["regression_latest"]),
        ])
        sec("자동 검사 항목")
        table(doc, ["검사", "내용", "주기"], F.QUALITY["ci_gates"])
        sec("검사 워크플로")
        table(doc, ["워크플로", "주기", "하는 일"], F.WORKFLOWS)

    if kind in ("admin", "test"):
        sec("정기 자동 실행 (정본 = vercel.json)")
        para(doc, "Vercel 예약이 깨운다. 깃허브 예약은 이 저장소에서 하루 7회만 돌아 시각이 중요한 일에는 쓰지 않는다.", size=9)
        table(doc, ["경로", "주기(UTC)", "하는 일"], F.CRONS)

    if kind in ("admin", "user"):
        sec("운영 데이터 규모")
        table(doc, ["항목", "규모"], F.DATA_VOLUME)

    if kind in ("admin", "test", "inventory"):
        sec("남은 일 · 기술 부채")
        para(doc, "「완료」 뒤에 남은 것. 고치면 _facts.py GAPS 에서 지운다.", size=9)
        table(doc, ["영역", "무엇이 남았나", "어디를 보면 되나"], F.GAPS)

    sec("근거자료")
    table(doc, ["구분", "출처", "확인 내용"], F.PROVENANCE + [
        ("설정", "vercel.json crons · .github/workflows/*.yml", "정기 실행·검사 워크플로"),
        ("실행", f"npx vitest run · npx playwright test --list · npm audit ({F.AS_OF})", "시험 규모·취약점"),
    ])
...
def main():
    ...
    if os.path.exists(path):
        ...
        hit += set_cells(doc, "npm 의존성 보안",
                         {2: "1회", 3: F.QUALITY["npm_audit"]})
        hit += set_cells(doc, "성능 측정 (Lighthouse)", {1: "6항목"})
        hit += set_cells(doc, "npm audit", {1: "2026-09-06"})
        if hit:
            doc.save(path)
            changed.append(f"08_테스트결과서.docx: 측정 결과 {hit}줄 갱신")

    # 0-2-1) 06 문의 접수 표 — 옛 5-Step 행을 칸 단위로 다시 쓴다(문단 교체로는 못 고친다)
    path = os.path.join(HERE, "06_사용자매뉴얼.docx")
    if os.path.exists(path):
        doc = Document(path)
        hit = set_cells(doc, "Step 1", {0: "1단계", 1: "접수", 2: "이름 · 연락 수단(왓츠앱·텔레그램·메일) · 국적 · 선호 언어 · 암종·병기 · 하고 싶은 말 · 동의(필수 4 + 선택 1)"})
        hit += set_cells(doc, "Step 2", {0: "2단계", 1: "임상 정보", 2: "진단일 · 현재 치료 상태 · 기왕력 · 병원에 전할 요청 (/inquiry/intake — 문의번호+접근 토큰)"})
        hit += set_cells(doc, "Step 3", {0: "2단계", 1: "의료기록", 2: "CT/MRI/혈액검사 등 첨부 (PDF·JPG·PNG·Word·DICOM·ZIP/RAR·음성 메모, 건당 200MB)"})
        hit += set_cells(doc, "Step 4", {0: "2단계", 1: "희망 사항", 2: "치료 방법 · 예산 · 입국 가능 시기 · 동행자"})
        hit += set_cells(doc, "Step 5", {0: "—", 1: "자동 저장", 2: "2단계는 0.4초마다 자동 저장되며 나중에 이어서 쓸 수 있다"})
        if hit:
            doc.save(path)
            changed.append(f"06_사용자매뉴얼.docx: 접수 표 {hit}줄 갱신")

    # 0-3) 09 요약표 — 칸을 넘는 문자열 교체는 안 걸린다(2026-09-06 발견: 「13/17 인데 완성도 100%」가 그대로 남아 있었다)
    path = os.path.join(HERE, "09_산출물목록.docx")
    if os.path.exists(path):
        doc = Document(path)
        hit = set_cells(doc, "Phase A 산출물", {1: "7건", 2: "7건 (100%)", 3: "2026-09-06 재실측 갱신(01·01-1·02·10·11·EVAL·00_INDEX)"})
        hit += set_cells(doc, "Phase B 산출물", {1: "4건", 2: "4건 (100%)", 3: "중간·최종 보고서는 KHIDI 지정 양식이라 본 목록에서 제외"})
        hit += set_cells(doc, "기술·운영 문서", {1: "13건", 2: "13건 (100%)", 3: "코드베이스 내 관리(기술 6 + 운영 대장 7)"})
        hit += set_cells(doc, "전체 합계", {1: "24건", 2: "24건 완료", 3: "완성도 100% (2026-09-06)"})
        if hit:
            doc.save(path)
            changed.append(f"09_산출물목록.docx: 요약표 {hit}줄 갱신")
