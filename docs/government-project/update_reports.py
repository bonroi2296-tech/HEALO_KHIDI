...
CWV_ROWS = [
    ("LCP (최대 콘텐츠 페인트)", "2.5초 이하", "모바일 5.31초 / 데스크톱 1.12초",
     "각 5회 반복 측정의 중앙값. 데스크톱 충족, 모바일 미달. 다만 모바일 값은 저속 4G·CPU "
     "4배 감속을 «가정해 계산한» 값이다. 같은 저사양 기기 조건에서 회선만 정상이면 0.68초, "
     "감속 없이 실제로 재면 0.42초로, 실제 이용자가 겪는 지연과는 차이가 크다. "
     "원인은 규명되어 있다 — 웹글꼴(Pretendard)을 내려받는 동안 첫 화면 글자 블록이 최종 "
     "모양으로 다시 그려지기 때문이며, 글꼴 요청을 차단하면 스타일·레이아웃 계산이 842ms 에서 "
     "434ms 로 줄어든다. 2026. 7. 27. 개선안 5종(미리받기·차단형 로딩·히어로 시스템 글꼴 등)을 "
     "각 4회 비교 측정했으나 글꼴을 아예 쓰지 않는 것 외에는 효과가 없었고, 이는 사이트 "
     "타이포그래피 전체를 포기하는 값이라 채택하지 않았다. 상시 관찰 대상"),
    ("CLS (레이아웃 안정성)", "0.1 이하", "모바일 0.045 / 데스크톱 0.008",
     "수정 전 데스크톱 0.51(5회 중 4회 발생)·모바일 0.95. 실서비스 반영 뒤 각 5회 재측정 결과 "
     "목표를 넘긴 회차 0/5. 양쪽 충족"),
    ("입력 반응성 (FID → TBT 대체)", "TBT 200ms 이하", "모바일 372ms / 데스크톱 6ms",
     "각 5회 중앙값. FID 는 폐지된 지표로 총 차단 시간(TBT)으로 대체 측정. 데스크톱 충족, "
     "모바일은 CPU 4배 감속 가정에서 미달. LCP 와 원인을 공유한다. 상시 관찰 대상"),
    ("TTFB (서버 응답 시간)", "800ms 이하", "모바일 609ms / 데스크톱 172ms", "양쪽 충족"),
    # 2026-08-21 정정: 앞서 적었던 「중앙값 5.7초 미달」은 잰 대상이 틀렸다.
    # 그 값은 회귀 시험기(scripts/run-regression-tests.ts)의 것인데, 이 시험기는 주석대로
    # 「generateChatReply 로직을 간소화해서 재현」한 것이라 ①검색 단계가 없고 ②스트리밍을
    # 쓰지 않아 «완료 시각»만 잰다. NFR-02 의 기준은 «첫 토큰»이므로 서로 다른 자였다.
    ("AI 챗 첫 토큰 응답", "5초 이하 (NFR-02)",
     "첫 토큰 중앙값 2.30초 · 상위 5% 3.84초 (완료 기준 3.03초 / 4.00초)",
     "실서비스 스트리밍 응답 10건 실측(2026. 8. 21., ko·en·ru·kz). 검색·프롬프트 구성까지 "
     "포함한 실제 경로다. 충족"),
]


CWV_NOTE = ("Lighthouse 12.8.2 실측(2026. 8. 21., https://healwith.co.kr/ko). 모바일·데스크톱 "
            "각 5회 측정의 중앙값이며, 모바일은 저속 4G·CPU 4배 감속 조건이다. "
            "첫 화면 전송량은 모바일 841KB · 데스크톱 956KB(자원 70건)이고, 구성은 "
            "스크립트 302KB · 이미지 179KB · 글꼴 173KB · 문서 129KB · 스타일 43KB 순이다. "
...
def main():
    ...
    if os.path.exists(path):
        ...
        hit = set_cells(doc, "성능 측정 (Lighthouse)",
                        {2: "5항목 전부 측정",
                         3: "2026-08-21 실측(모바일·데스크톱 각 5회). CLS·TTFB·AI 첫 토큰 응답 충족, "
                            "LCP·TBT 는 데스크톱만 충족 — 모바일 2건 개선 과제"})
