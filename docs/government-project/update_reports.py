...
CWV_ROWS = [
    ("LCP (최대 콘텐츠 페인트)", "2.5초 이하", "모바일 6.96초 / 데스크톱 1.47초",
     "각 5회 반복 측정의 중앙값. 데스크톱 충족, 모바일 미달. 저속 4G·CPU 4배 감속이라는 "
     "가혹한 조건에서 잰 값이며, 첫 화면 실제 전송량은 322KB 다(문서 127KB + 스크립트·스타일). "
     "개선 과제"),
    ("CLS (레이아웃 안정성)", "0.1 이하", "데스크톱 0.008 / 모바일 0.045",
     "수정 전 데스크톱 0.51(5회 중 4회 발생)·모바일 0.95. 실서비스 반영 뒤 각 5회 재측정 결과 "
     "목표를 넘긴 회차 0/5. 양쪽 충족"),
    ("입력 반응성 (FID → TBT 대체)", "TBT 200ms 이하", "모바일 183ms / 데스크톱 0ms",
     "각 5회 중앙값. FID 는 폐지된 지표로 총 차단 시간(TBT)으로 대체 측정. 양쪽 충족"),
    ("TTFB (서버 응답 시간)", "800ms 이하", "29~43ms", "충족"),
    # 2026-08-20: 「미측정」이 아니었다. AI 회귀 시험 기록에 응답시간이 2,212건 쌓여 있다.
    # 다만 그 값은 «응답을 다 만든 시각» 기준이라 「첫 토큰까지」와는 다른 자다.
    # 목표는 01 요구사항정의서 NFR-02(5초)에 맞춘다 — 3초는 이 문서에만 있던 값이었다.
...
CWV_NOTE = ("Lighthouse 12.8.2 실측(2026. 8. 20., https://healwith.co.kr/ko). "
            "모바일은 저속 4G·CPU 4배 감속 조건이며 5회 반복 측정의 중앙값이다. "
            "측정 환경 주의: 측정에 쓴 PC 에 광고차단 프로그램이 동작 중이었고, 이 프로그램은 "
            "응답을 가로채 압축을 푼 뒤 브라우저에 넘긴다. 그 결과 브라우저가 재는 전송량이 "
            "압축 전 크기로 잡힌다(2026. 8. 21. 확인 — 첫 화면 자원 50건 전부에서 압축 후 "
            "크기와 압축 전 크기가 같게 보고되었다). 서버가 실제로 보내는 첫 화면 전송량은 "
            "브로틀리 압축 기준 문서 127KB · 전체 322KB 이며, 전송량 판단에는 이 실측값을 쓴다. "
            "로딩 시간 지표도 같은 이유로 실제보다 불리하게 잡힌 값이다. "
...
def main():
    ...
    if os.path.exists(path):
        ...
        hit = set_cells(doc, "성능 측정 (Lighthouse)",
                        {2: "5항목 전부 측정",
                         3: "2026-08-20 실측(모바일·데스크톱 각 5회). CLS·TBT·TTFB·AI 첫 토큰 응답 충족, "
                            "LCP 는 데스크톱만 충족 — 모바일 LCP 1건 개선 과제"})
