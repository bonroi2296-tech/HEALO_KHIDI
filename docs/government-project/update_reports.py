...
CWV_ROWS = [
    ("LCP (최대 콘텐츠 페인트)", "2.5초 이하", "모바일 6.4초(5회 중앙값) / 데스크톱 1.5초",
     "데스크톱 충족. 모바일 미달(첫 화면 전송량 831KB 가 저속 회선에서 병목). 개선 과제"),
    ("CLS (레이아웃 안정성)", "0.1 이하", "평상시 0.04 / 발생 시 0.95",
     "5회 반복 측정 중 1회 발생. 화면 골격이 본문보다 먼저 그려진 회차에서만 "
     "꼬리말이 밀린다. 원인 규명 완료, 수정 반영 예정"),
...
def main():
    ...
    if os.path.exists(path):
        ...
        hit = set_cells(doc, "성능 측정 (Lighthouse)",
                        {2: "5항목 중 4항목 측정",
                         3: "2026-08-20 실측(5회 반복). LCP 는 데스크톱만 충족, CLS 는 간헐 발생(원인 규명 완료)"})
