...
CWV_ROWS = [
    ("LCP (최대 콘텐츠 페인트)", "2.5초 이하", "모바일 5.31초 / 데스크톱 1.12초",
     "각 5회 반복 측정의 중앙값. 데스크톱 충족, 모바일 미달. 모바일은 저속 4G·CPU 4배 감속 "
     "조건이다. 원인을 나눠 보면 첫 화면 그리기의 주 병목은 전송량이 아니라 브라우저 메인 "
     "작업 2.4초이며, 그중 스타일·레이아웃 계산이 1.05초로 가장 크다. 개선 과제"),
    ...
    ("입력 반응성 (FID → TBT 대체)", "TBT 200ms 이하", "모바일 372ms / 데스크톱 6ms",
     ...
     "모바일 미달. LCP 와 원인이 같다(메인 작업 과다). 개선 과제"),
