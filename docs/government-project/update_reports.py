...
CWV_ROWS = [
    ...
    ("AI 응답 완료 시간", "5초 이하 (NFR-02)", "중앙값 5.7초 · 상위 5% 7.8초 — 목표 미달",
     "AI 회귀 시험 기록 2,212건 기준(ai_regression_runs.latency_ms, 2026-08-17까지 누적). "
     "「첫 토큰까지」는 이 값에 안 담긴다. 별도 계측이 필요하다. 개선 과제"),
]
...
def main():
    ...
    if os.path.exists(path):
        ...
        hit = set_cells(doc, "성능 측정 (Lighthouse)",
                        {2: "5항목 전부 측정",
                         3: "2026-08-20 실측(모바일·데스크톱 각 5회). CLS·TBT·TTFB 충족, LCP 는 데스크톱만 충족, "
                            "AI 응답 완료 시간 5.7초로 목표(5초) 미달 — 2건 개선 과제"})
