...
CWV_ROWS = [
    ("LCP (최대 콘텐츠 페인트)", "2.5초 이하", "모바일 6.4초 / 데스크톱 1.5초",
     "데스크톱 충족. 모바일 미달(첫 화면 전송량 831KB 가 저속 회선에서 병목). 개선 과제"),
    ("CLS (레이아웃 안정성)", "0.1 이하", "모바일 0.95 / 데스크톱 0.51",
     "양쪽 미달. 첫 화면 본문이 뒤늦게 끼워지며 꼬리말이 밀린다. 원인 규명 완료, 수정 반영 예정"),
    ("입력 반응성 (FID → TBT 대체)", "TBT 200ms 이하", "모바일 388ms / 데스크톱 28ms",
...
CWV_NOTE = ("Lighthouse 12.8.2 실측(2026. 8. 20., https://healwith.co.kr/ko). "
            "모바일은 저속 4G·CPU 4배 감속 조건이다. 측정 PC 의 광고차단 프로그램이 값을 "
...
def main():
    ...
    if os.path.exists(path):
        ...
        hit = set_cells(doc, "성능 측정 (Lighthouse)",
                        {2: "5항목 중 4항목 측정",
                         3: "2026-08-20 실측. LCP 는 데스크톱만 충족, CLS 는 양쪽 미달(원인 규명 완료)"})
