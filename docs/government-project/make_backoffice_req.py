...
for k, v in [
    ...
    ('반영 결과', '충족 19건 · 부분충족 2건 · 대안반영 1건 (반영률 100%, 완전충족률 86.4%)'),
    ('상위 산출물', '01_요구사항정의서.docx (FR-01~28 / NFR-01~23)'),
    ('수행기관', '(주)본로이 (Bonroi) — 플랫폼 HEALO'),
    ('작성일', '2026-07-25'),
...
REQS = [
    ...
    ('BO-NFR-01', '화면 재사용 표준화',
     '계층 간 동일 기능은 화면을 복제하지 않고 재사용(re-export)하여 화면 불일치(drift) 발생을 구조적으로 차단해야 한다.',
     'H', 'S2', 'NFR-17'),
...
for r in [
    ('BO-FR-06', '부분충족',
     '데이터 0행이나 존폐 영향도가 확정되지 않은 2개 화면(치료·암종 관리 / 의료진 관리)이 메뉴에 남아 있음. '
     '의료진 관리는 화상상담 담당의사 선택 목록이 해당 테이블을 참조하여 임의 비활성 시 상담 생성에 영향 가능.',
     '영향도 확인 후 발주자 판단으로 비활성 여부 결정', '후속-ⓒ'),
    ('BO-SR-02', '부분충족',
     '콘텐츠 쓰기 경로의 표준 가드·요청량 제한은 적용 완료. 다만 공통 인증 헬퍼의 인증 실패 이벤트가 '
     '감사로그에 적재되지 않는 공백이 남아 있음(콘텐츠 API 한정이 아닌 공통 사항).',
     '공통 인증 헬퍼 차원의 실패 로깅 보강 검토', '후속-ⓓ'),
    ('BO-SR-03', '대안반영',
...
for r in [
    ...
    for i in range(1, 7):
        row.cells[i].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
row = add_data_row(t, ('합계', '22', '19', '2', '1', '0', '86.4%'), size=9)
...
set_font(p.add_run('작성: (주)본로이 | 2026-07-25 | 근거: 소스코드 실측 + 운영DB 정확 COUNT'),
