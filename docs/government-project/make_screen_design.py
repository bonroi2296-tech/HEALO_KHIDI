...
SCREENS = [
    ...
    dict(
        ...
        data=["inquiries — 목록에 실제로 뜬 건수 8건 = 실환자 8건(시험 문의는 자동으로 빠진다)",
              "notifications 1,027건(새 문의 즉시 알림)"],
        nexts="→ P-07 케이스 상세",
        notes=["로그인해서 실제로 확인(2026-08-19). 목록 열은 문의번호·담당자가 아니라 위 7가지다.",
               "시험 문의는 목록·알림·성과지표에서 모두 빠진다 — 실적 정직성 장치."],
    ),
    dict(
        ...
        data=["cotreatment_referrals 4건 · case_shared_documents 8건 · case_opinions 6건(전달 5건)",
              "attachment_translations 12건 — 외국 검사지를 표의 행·칸까지 구조화(숫자·단위·정상범위는 원문 그대로)"],
        nexts="→ P-05 소견 확인(환자) · P-04 원격협진",
        notes=["로그인해서 실제로 확인(2026-08-19). 「접수 후 추가 정보」는 저장할 때 암호화된다.",
    ...
    dict(
        ...
        data=["2026-08-19 기준 — 문의 8 · 사전상담 완료 1(12.5%) · 유치 0 · 사후관리 0",
              ...
              "kpi_snapshots 93건 · 목표값 단일 기준 파일 하나(화면과 문서가 어긋나지 않게)"],
        nexts="— (보고서 산출)",
        notes=["로그인해서 실제로 확인(2026-08-19). 화면 이름은 「유치 전환 상세」다.",
               "깔때기의 사전상담(1)은 «문의에 연결된» 건, 기관별 표의 사전상담(3)은 전체 세션 — 세는 기준이 다르다.",
               "시험 데이터는 「실적만」 스위치로 갈라 보며, 평가에 내는 숫자는 실적만이다."],
    ),
    dict(
