...
# ── 화면 정의 (근거는 전부 코드·운영DB 실측 — 2026-08-19 초판, 2026-09-06 재실측·P-05 정정·P-11 신설) ──
SCREENS = [
    dict(
        ...
        parts=[
            ...
            "우하단: 화면에 떠 있는 대화 단추(「도움말」 말풍선). 누르면 문의 갈림길(P-02)로 가고, 거기서 ①AI 상담원을 고르면 AI 사전상담(P-03)이 열린다",
            "맨 아래: 유치업 등록번호 · 보증보험 · 법정 고지 · 개인정보 처리방침",
            "첫 방문 시 화면 아래 쿠키 설정 띠",
        ],
        data=[
            f"다국어 사전(i18n): 화면이 쓰는 문구 {F.ROUTE_STATS['i18n_keys']:,}개를 6개 언어에 모두 채웠다. 러시아어·카자흐어는 자동 검사로 매 변경마다 100%를 확인한다({F.AS_OF} 실측). 콘텐츠 칸(치료법·암종 상세·병원 소개) 249개는 2026-09-06 부터 코디가 콘텐츠 편집기에서 6개 언어로 직접 고친다",
            "병원(hospitals): 발행 8곳(협진 대학병원 4 + 면력한방병원 4지점). 나머지 1곳은 시험용 등록이라 공개하지 않는다",
            "화면 설정(site_settings): 노출 문구·배너를 관리자 화면에서 고친다",
        ],
        nexts="→ P-02 문의 갈림길(우하단 대화 단추도 여기로) → P-03 AI 사전상담",
    ...
    dict(
        ...
        parts=[
            ...
            "동의 필수 4종 + 선택 1종: 개인정보 수집 · 민감정보(건강) · 제3자 제공(병원) · 국외이전 + 마케팅(선택). 「모두 동의」 단추와 동의서 판번호(2.0.0)·동의 시각 기록",
    ...
    dict(
        ...
        ict="① · ②", path="홈 위젯(/inquiry ①) · 메신저(텔레그램·왓츠앱 봇)", who="누구나(비로그인)",
        ...
        data=[
            "AI 상담 대화(chat_threads) 396건, 주고받은 말(chat_messages) 1,077건 (시험 문의 제외. 채널: 웹 394 · 텔레그램 4 · 에이전시 1)",
            "근거 문서(rag_documents) 63건 · 조각 105개. 병원·진료 정보를 담으며, AI 는 답할 때 이 안에서 먼저 근거를 찾고, 없으면 HIRA 공공 API·네이버 지역검색을 보조로 부른다(구글 검색 그라운딩은 미작동 — 알려진 결함)",
            "품질 자동 시험 기록(ai_response_evaluations, ai_regression_runs)",
        ],
        nexts="→ 3턴을 넘기면 실제 문의로 자동 승격 → P-06 코디네이터 인박스",
        notes=["동작: 질문 → 의미 변환 → 자체 DB 검색 → 근거 읽기 → 출처와 함께 답변(첫 글자 중앙값 2~4초).",
               "품질은 주 2회 자가시험(실서비스와 같은 경로 50문항)과 매일 판사(Judge)가 본다. 의료 레드라인(완치 단정·지어낸 병원·연락처 없는 「접수됨」) 위반을 검출해 경보한다.",
               "3턴을 넘기면 실제 문의로 승격되고, 연락 수단이 없는 대화는 승격하지 않는다(2026-09-03)."],
    ),
    dict(
        ...
        parts=[
            ...
            "자막 켜기·끄기: 누를 때만 켜진다(자동 켜짐 없음). 잡음 제거는 단추 없이 자동으로 걸린다. 화자 이름·시각·원어를 함께 표시한다(2026-09-01)",
            "도구 막대: 마이크 · 카메라 · 화면 공유 · 채팅 · 언어 · 자막 크기 · 음성 출력 · [종료]",
        ],
        data=[
            "상담 세션(consultation_sessions) 159건, 입퇴장 기록 364건 (실환자 문의에 연결된 세션 1)",
            "통역 자막(consultation_translations) 3,735건(시험 방 제외 3,396). 확정 자막과 하단 중간 자막(is_partial)을 갈라 저장한다",
    ...
    dict(
        no="P-05", file="P-05_opinion.png", name="전문의 소견 작성(의료진용)",
        ict="②", path="/opinion/<토큰>", who="협진 전문의(계정 없이 코디가 보낸 링크로)",
        purpose="한국 전문의가 계정 없이 환자 자료를 보고 소견을 남기게 한다. 받는 사람은 환자가 아니라 «의료진»이다.",
        parts=[
            "머리말: 전문의 소견 요청 · 환자/임상 정보 · 요청한 코디네이터",
            "환자/임상 정보: 국적·암종·병기·환자가 원하는 것·환자 메시지 원문, 첨부 검사지 열람, CT 초견(AI 초안 — 판독 아님)",
            "소견 작성: 소견 주시는 분·소견 내용·언어. 정상범위·단위는 원문 그대로",
            "서류 같이 내기(선택): 견적서 등 PDF·이미지·워드·엑셀, 각 50MB·최대 5개",
            "[소견 제출] → 「감사합니다. 담당 코디네이터가 확인합니다」",
        ],
        data=[
            "소견 요청(opinion_requests) 3건 · 제2의료소견(case_opinions) 6건. 전건 환자에게 전달 완료",
            "서류 번역(attachment_translations) 23건. 외국 검사지와 진료기록을 AI 가 읽어 표의 행과 칸까지 구조를 살려 옮긴다",
        ],
        nexts="→ P-07 케이스 상세(소견 도착·번역 초안·코디 교정) → P-11 환자 진행상황(공개 후)",
        notes=["첫 판(2026-08-19)은 이 화면을 «환자용 소견 확인»으로 잘못 적었다. 코드 주석이 «받는 사람은 환자가 아니라 한국 전문의»라고 박아 두고 있다. 환자가 보는 화면은 P-11 이다.",
               "환자에게 전달한 시점(released_at)이 성과지표 판정 기준이다. 작성만 하면 세지 않는다."],
    ),
    dict(
        ...
        parts=["왼쪽 메뉴 16개: 대시보드 · 음성 정리(2026-09-04 신설) · 문의함 · AI 상담 리드 · 의뢰·케이스/병원배정 · 상담 일정 · "
               "유치 전환 · 사후관리·만족도 · 파트너 발굴 · 메시지 · 비자 트래킹 · 견적 · 증상 알림 · "
               "콘텐츠 편집 · 개선 요청함 · 설정",
               "받은함 탭: 전체 / 추가 정보 필요 / 매칭 준비 완료 / 종료(환자가 안 온다고 한 건, 2026-09-06)",
               "「환자 새 글 · N일째 안 읽음」 배지·필터: 환자가 진행상황 화면에서 남긴 글을 코디가 놓치지 않게(2026-09-05)",
               "거르기 세 갈래: 전체 / 추가 정보 필요 / 매칭 준비 완료 (각 건수 표시)",
               "목록 표: 이름 | 국적 | 암종 | 연락방법 | Step 완료(1만 / 1+2) | 접수일 | 상태",
               "상태 칸에 빨간 배지 「N일째 정체」: 단계 기준일을 넘긴 케이스를 표시"],
        data=["문의(inquiries): 목록에 뜨는 건수 8건. 실환자 수와 같다(시험 문의는 자동으로 빠진다 — 단 시험으로 «잘못» 찍힌 문의는 표시해 주고 사람이 표식을 뗄 수 있다, 2026-09-04)",
              "알림(notifications) 1,175건(최근 30일 300). 새 문의·환자 새 글·식은 문의(7일 무동작)가 종 알림으로 온다"],
        nexts="→ P-07 케이스 상세",
    ...
    dict(
        ...
        parts=["머리말: 환자 이름 · 회원/비회원 · 담당자 · 문의 번호 · 접수 시각 · Step 완료 · 상태",
               ...
               "「진단이 아니며 검수 필요」 고지",
               "연락 정보 / 의료·여정 정보 / 문의 메시지 원문 / 추가 정보(2단계 접수분)",
        ...
        data=["협진 의뢰(cotreatment_referrals) 4건, 공유 서류(case_shared_documents) 9건, 제2의료소견(case_opinions) 6건 전부 전달 완료",
              "서류 번역(attachment_translations) 23건. 표의 행과 칸까지 구조를 살려 옮기되 숫자·단위·정상범위는 원문 그대로 둔다",
              "병원 의뢰서: 협진 대학병원이 준 워드 양식 «원본 그대로»를 채워 화면·인쇄·파일이 한 원본에서 나온다(2026-09-04). 음성·텍스트 메모는 AI 가 읽어 코디가 듣지 않고 확인한다"],
    ...
    dict(
        ...
        parts=[
            ...
            "화면 뒤에서 자동으로 도는 것: 증상 기록의 위험도를 규칙(응급 키워드 + 점수식)으로 매기고 담당자에게 알린다. 3일 이상 입력이 없으면 경보",
            "단계별 교육 자동 발송: 사후관리 단계(D+7·14·30·90·180)가 오면 환자 언어로 교육 콘텐츠를 보낸다(2026-08-25 연결, 발송 이력으로 중복 방지)",
        ],
        data=[
            "교육 콘텐츠(education_contents) 18건. 위·유방·간·폐·갑상선 5종이며 러시아어 본문까지 전건 채워 발행했다",
            "증상 기록(symptom_reports) 22건(시연·점검 포함) · 사후관리 일정(followup_schedules) 2건. 기능은 완성했으나 유치 확정 환자가 아직 없어 실환자 기록이 없다",
            "알림 기기 등록(device_tokens) 23건 — 스토어 앱(iOS 심사 대기 · Android 검토 대기) 푸시용",
        ],
    ...
    dict(
        ...
        parts=["기간 고르기: 최근 30일 / 90일 / 1년 + 「실적만」 스위치(켜면 시험 데이터 제외)",
               ...
               "유치 확정 대기: [유치 확정] [이탈] [테스트로 표시] 로 결과를 코디가 입력. 확정된 것은 [유치 취소]로 되돌린다. 코디 화면에는 [종료(환자가 안 온다고 함)] 단추가 따로 있다(2026-09-06)"],
        data=[f"{F.AS_OF} 기준: 문의 8건, 사전상담 완료 1건(12.5%) + 글로 전달한 소견 6건, 유치 0건, 사후관리 0건",
              "국가별 KZ 5 · ET 1 · KG 1 · RU 1 / 채널별 웹 문의폼 6 · 소급등록 2",
              "성과 기록(kpi_snapshots) 111일치. 목표값은 파일 한 곳(src/lib/khidi/targets.ts)에만 두어 화면과 문서가 어긋나지 않게 한다"],
        nexts="— (보고서 산출)",
    ...
    dict(
        ...
        parts=["환자 동선: 홈 → 문의 갈림길 → AI 상담 / 문의 접수 → (코디) → 원격협진 · 진행상황·소견 확인(P-11) → 사후관리",
               "운영 동선: 인박스 → 케이스 상세 → 병원 의뢰(원본 양식) → 전문의 소견 작성(P-05) → 번역·교정 → 환자 전달",
               "권한 5종: 환자·코디네이터·에이전시·해외 의료기관·병원"],
        data=["모든 화면의 실적 이벤트가 성과 대시보드(P-09)로 자동 모인다"],
        nexts="—",
        notes=["환자는 로그인 없이도 링크로 상담방(P-04)·진행상황(P-11)에 들어올 수 있다. 전문의도 계정 없이 소견 작성(P-05)에 들어온다."],
    ),
    dict(
        no="P-11", file="P-11_claim.png", name="진행상황·소견 확인(환자용)",
        ict="② · ③", path="/claim/<토큰>", who="접수한 사람 누구나(환자·가족·에이전시, 가입·로그인 없이)",
        purpose="왓츠앱·메일·에이전시 경유처럼 계정 없이 들어온 문의가 표준 동선이므로, 「보려면 먼저 가입」을 입구에 두지 않고 진행상황과 우리가 준 서류를 링크 하나로 보여준다.",
        parts=[
            "6단계 막대(누를 수 있는 탭): 아직 안 온 단계는 흐리게 잠그고, 지나온·지금 단계만 눌러 그때 받은 것을 본다",
            "이 단계에 있었던 일 + 「다음은요」: 단계 시작 날짜 · 코디 소식(새것부터) · 앞으로 어떻게 되나(지금 단계일 때만)",
            "우리가 준 것: 소견서·사전상담 정리본 — 코디가 「환자에게 보이기」를 켠 공식 문서만",
            "환자가 준 것: 추가 자료 올리기(200MB)·메시지. 환자 글은 코디 목록에 「환자 새 글」 배지로 뜬다",
            "아래 띠: 계정 연결·가입 «권유». 어떤 상태가 되어도 위 진행상황은 그대로 남는다",
        ],
        data=[
            "케이스 단계 이력(case_status_history) 31건 — 단계마다 «언제 시작됐나»를 기록하고 소견·서류는 날짜로 그 단계에 속한다(새 칸 없이)",
            "공유 서류(case_shared_documents) 9건 · 제2의료소견(case_opinions) 6건",
        ],
        nexts="→ P-04 원격협진(초대 링크) · P-08 사후관리(가입 뒤)",
        notes=["표시 언어는 사람이 고른 언어 → 문의서에 적힌 환자 언어 → 브라우저 언어 → 영어 순. 국적으로 추측하지 않는다.",
               "소견은 «화면에 다시 그린 요약»이 아니라 코디가 올린 공식 문서로만 나간다(2026-08-18 PO 결정).",
               "이 화면은 2026-08-04 에 생겼고 8/19 첫 설계서에는 빠져 있었다(P-05 와 뒤섞여 있었음)."],
    ),
]
...
def main():
    ...
    para(doc, "「완료」 뒤에 남은 것을 그대로 적는다. 근거 위치는 코드 파일이다.", size=9)
    table(doc, ["항목", "화면", "상태", "계획·근거"], [
        ("병원 가용 일정 입력 화면", "P-07 · P-09", "없음(리마인더·구글 캘린더 등록은 가동)", "실환자 유치 확정 시점에 맞춰. src/lib/calendar/googleCalendar.ts"),
        ("AI 상담 3계층의 구글 검색 그라운딩", "P-03", "미작동(SDK 에 없는 옵션이라 조용히 무시)", "정식 googleSearch 도구로 교체하거나 «3계층» 표현을 내림. src/lib/chat/generateReply.ts"),
        ("AI 응답 첫 글자 5초 목표", "P-03", "실서비스 경로 자가시험 P95 5.33초(8/21)", "9/07 실행부터 재계측. src/lib/chat/regressionRunner.ts"),
        ("인앱 알림 실시간화", "P-06 · P-08", "30초 폴링", "필요 시 상담방처럼 채널 구독으로. src/components/NotificationBell.jsx"),
        ("환자 진행상황 화면 다국어 검수", "P-11", "기계 검사 통과 · 원어민 검수 대기", "docs/rules/I18N_QUALITY.md §3"),
        ("만족도 응답 확보", "P-08", "실환자 응답 0건", "코디 [상담 완료] 처리가 발송 조건"),
    ], widths=[4.6, 2.6, 4.4, 4.4])
