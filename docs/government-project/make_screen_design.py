...
SCREENS = [
    dict(
        ...
        parts=[
            ...
            "우하단: 화면에 떠 있는 대화 단추(「도움말」 말풍선)로, AI 상담으로 들어가는 입구다",
            "맨아래: 유치업 등록번호 · 보증보험 · 법정 고지 · 개인정보 처리방침",
            "첫 방문 시 화면 아래 쿠키 설정 띠",
        ],
        data=[
            "다국어 사전(i18n): 언어당 1,753키, 러시아어·카자흐어 각 100% (자동 검사 통과)",
            "병원(hospitals): 발행 8곳(협진 대학병원 4 + 면력한방병원 4지점). 나머지 1행은 시험용 등록이라 미발행 상태다",
    ...
    dict(
        ...
        data=[
            "문의(inquiries): 전체 145건 중 실환자 8건이다. 시험 문의는 표식으로 갈라 둔다",
            "환자 상세(cancer_patient_intakes): 64건으로 암종·병기·치료 이력을 담는다",
    ...
    dict(
        ...
        data=[
            ...
            "근거 문서(rag_documents) 21건으로 병원·진료 항목을 담는다. AI 가 답할 때 찾아 읽는 원장이다",
    ...
    dict(
        ...
        data=[
            ...
            "통역 자막(consultation_translations) 3,277건이다(시험 방 제외). 러→한 2,487, 한→러 393, 카자흐→한 117 등",
    ...
    dict(
        ...
        data=[
            "제2의료소견(case_opinions) 6건을 확보해 6건 모두 환자에게 전달했다. 소견 요청(opinion_requests)은 2건이다",
            "서류 번역(attachment_translations) 12건이다. 외국 검사지와 진료기록을 AI 가 읽어 표의 행과 칸까지 구조를 살려 옮긴다",
        ],
    ...
    dict(
        ...
        parts=["왼쪽 메뉴 16개: 대시보드 · 문의함 · AI 상담 리드 · 의뢰·케이스/병원배정 · 상담 일정 · "
        ...
        data=["문의(inquiries): 목록에 실제로 뜬 건수가 8건으로 실환자 수와 같다. 시험 문의는 자동으로 빠진다",
              "알림(notifications) 1,043건으로, 새 문의가 들어오면 바로 보낸다"],
        nexts="→ P-07 케이스 상세",
    ...
    dict(
        ...
        data=["협진 의뢰(cotreatment_referrals) 4건, 공유 서류(case_shared_documents) 8건, 제2의료소견(case_opinions) 6건 전부 전달 완료",
              "서류 번역(attachment_translations) 12건이다. 표의 행과 칸까지 구조를 살려 옮기되 숫자·단위·정상범위는 원문 그대로 둔다"],
    ...
    dict(
        ...
        parts=[
            ...
            "[남은 것] 단계별 교육 자동 발송: 귀국 후 시기에 맞춰 보내는 부분을 아직 만들지 않았다",
        ],
        data=[
            "교육 콘텐츠(education_contents) 18건이다. 위·유방·간·폐·갑상선 5종이며 러시아어 본문을 전건 채워 발행했다",
            "증상 기록(symptom_reports) 1건, 사후관리 일정(followup_schedules) 0건이다. 기능은 만들어져 있으나 유치 확정 환자가 없어 실사용이 없다",
            "알림 기기 등록(device_tokens) 11건",
        ],
        nexts="→ P-04 원격 경과상담 · P-09 성과 대시보드",
        notes=["사후관리 카드는 6종이다.",
               "교육 콘텐츠는 환자 전용 화면이 아니라 공개 화면 /education 을 그대로 쓴다. 6개 언어로 실제 서비스하고 있다.",
    ...
    dict(
        ...
        data=[f"{F.AS_OF} 기준: 문의 8건, 사전상담 완료 1건(12.5%), 유치 0건, 사후관리 0건",
              ...
              "성과 기록(kpi_snapshots) 94건이다. 목표값은 파일 한 곳에만 두어 화면과 문서가 어긋나지 않게 한다"],
        nexts="— (보고서 산출)",
        notes=["깔때기의 사전상담(1)은 문의에 연결된 건, 기관별 표의 사전상담(3)은 전체 세션: 세는 기준이 다르다.",
...
def main():
    ...
    table(doc, ["표기", "뜻"], [
        ...
        ("점선 상자", "설계는 확정했으나 아직 만들지 않은 요소다. 숨기지 않고 그대로 적는다"),
    ...
    doc.add_paragraph()
    heading(doc, "5. 남은 설계 항목", size=14)
    table(doc, ["항목", "화면", "상태", "계획"], [
        ("단계별 교육 콘텐츠 자동 발송", "P-08", "판정 부분은 만들었고 화면 연결이 남았다",
         "2026-08-27 이전 완료 목표"),
