...
## 🔵 2026-07-07 gen-UI(챗봇 컴포넌트 렌더) 손질 검토 — PO 고민 중, 적용 여부 미결 (지금 코드 안 건드림)

> **맥락(디자인시스템 탐색 결론)**: Astryx·Google Stitch·제너레이티브 UI 3개를 공정하게 테스트(실제 홈 A/B 포함)한 결과 — **프런트 톤은 현행 유지가 정답**(Astryx는 프런트 2회 다 별로, Gemini 생성은 우리 DESIGN.md로 수렴해 "크게 안 다름"). 셋 중 **실전 가치가 남은 건 gen-UI(챗봇이 특정 순간 검증 컴포넌트 렌더)뿐**. PO: "gen-UI는 손봐야 할 것 같은데, 적용할지는 고민 후 결정 — 일단 메모만."

- **현재 상태**: 개념·실동작 검증 완료. 파일럿 살아있음 — `app/astryx-pilot/genui/page.jsx`(우리 Tailwind 챗 UI) + `app/api/astryx-pilot/genui/route.ts`(실제 Gemini 툴콜, 화이트리스트 4개 중 선택). 실서비스 미연결(격리·noindex).
- **손볼 후보(PO가 적용 결정 시)**: ①화이트리스트 컴포넌트 확대(현재 병원비교·예약슬롯·비용·채널 4개) ②컴포넌트 6언어 i18n화(현재 한국어 예시) ③실데이터 연결(현재 예시 데이터) ④스트리밍 응답 ⑤**실제 공개 챗(`/api/public/chat/*`)에 tool 패턴 통합** ⑥"언제 컴포넌트를 띄울지" 트리거 정교화(과다 노출 방지).
- **결정 대기**: PO가 gen-UI를 실전에 넣을지 vs 파일럿으로 보관할지 아직 미정. **PO의 명시적 "적용해/진행" 전까지 실전 연결 착수 금지.**
- **함께 딸려온 자산(gen-UI와 별개, 유지)**: DESIGN.md 토큰 허브·의료광고 가드 강화(#66)·디자인타임 draft 생성기(`/api/astryx-pilot/draft`, 새 화면 초안 속도 도구). Astryx 라이브러리 자체는 프런트 탈락 확정(백오피스 외 쓸모 없음) — 정리 여부도 PO 결정 대기(이전 무단삭제는 되돌려놓음).

## 🟡 2026-07-06 면력 3개 지점 DB 설명이 피벗 전 콘텐츠로 실서비스 노출 중 — PO 결정 대기

> 병원 상세는 **DB 우선·정적 fallback**. 면력 강서·신촌·광명이 DB(`hospitals`)에 있어서, 정성 들여 만든 정적 파일(`src/lib/data/partnerHospitals.js`, 암환자 중심·6개 언어)이 **가려지고** DB의 옛 설명("여성건강·난임·산후조리·소아과" 등 피벗 전 일반 한의원 톤)이 그대로 노출 중 (성동만 DB에 없어 정적 콘텐츠로 나옴). 반성문 #66 후속 발견.

- **선택지**: (A) DB 3곳 설명·전문분야를 정적 파일의 암환자 중심 콘텐츠로 교체(카피 확정 필요 — DB라 프리뷰 분리 불가, 반영 즉시 실서비스) (B) DB 3곳 `is_published=false`로 내려 정적 fallback으로 통일(단, /hospitals 목록에서 빠지는지 확인 필요) (C) 현상 유지.
- 유치기관 등록 표기는 이 이슈와 별개로 `certifications` 칩으로 해결(2026-07-06, PR #657) — DB 3곳은 코드 배포 후 DB에 문구 추가 예정.
- ⚠️ 관련: 비자 초청장 발급 주체(`src/lib/visa/inviterHospitals.ts`)에 성동·광명 추가하려면 **등록증 원본 정보(등록번호·대표자·유효기간) 필요** — PO 제공 대기.
