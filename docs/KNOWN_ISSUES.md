...
- **고친 방법**: `x-locale` 이 **없을 때만**(= 포털·noindex 경로) `healo_lang` 쿠키를 서버 렌더 언어로 쓴다. 공개 경로는 `x-locale` 이 항상 있어 URL 로케일 우선이 그대로 유지되고, 검색봇은 쿠키가 없어 `en` 이라 SEO 도 불변.
- **실측**: 포털 경로에서 6개 언어 전부 확인 — ko→`lang="ko"` · en→`en` · ru→`ru` · kz→`kk` · zh→`zh` · ja→`ja`, 잘못된 쿠키값·쿠키 없음 → `en` 폴백. 공개 경로(`/ru`·`/kz`·`/en`)는 쿠키와 무관하게 URL 언어 유지 확인.
- 캐시 영향 없음: 루트 레이아웃은 이 변경 **이전에도** `cookies()` 를 부르고 있어 이미 전 라우트가 동적 렌더였다.
