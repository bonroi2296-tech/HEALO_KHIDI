...
- ✅ **해소(2026-07-28, PO 확인)**: LiveKit Ship 영수증 결제수단 **Mastercard ••••0953 = 사업비 전용카드(IBK 기업은행)**. 즉 **해외 정기(구독)결제가 이제 통과한다** — 7/24 의 「전용카드가 LiveKit/Stripe 를 거절」 증상은 해소됐다.
  - 따라서 11번 줄의 *"구글 $25 는 전용카드가 아니었다"* 도 **오판이었다** — 같은 0953 이므로 **전용카드로 결제된 것**이다(「강주영」 은 구글 결제 *프로필 명의* 표시일 뿐, 카드 자체가 개인 것이라는 뜻이 아니었다). 정산 증빙 위험 없음.
