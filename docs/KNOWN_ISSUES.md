# healwith KHIDI — 알려진 이슈 / 전수 QA 발견사항
...
## 🔸 화상상담 자막 — 2026-08-06 실검증에서 새로 나온 2건

1. 🛑 **통역봇 원문 자막을 상담 기록에 저장하면 «두 번 적힌다» — 그래서 아직 안 붙였다.**
   - 오늘 통역봇이 **원문 자막**을 내보내게 만들었고(실방에서 나오는 것 확인), 그걸 기록에
     저장하려다 멈췄다. 지금 구조에서 붙이면 **같은 발화가 기록에 두 번 이상 들어간다**:
     ①말한 사람 본인의 클라이언트가 자기 받아쓰기를 이미 저장한다(`voiceOn` 과 무관하게 동작)
     ②봇 자막은 방 전체에 뿌려지므로 **듣는 사람 수만큼** 저장 시도가 생긴다.
   - 코드 주석엔 이미 *"통역(음성) 사용 중엔 봇 자막이 표시·기록을 담당"* 이라 적혀 있는데
     **「기록」 쪽은 구현된 적이 없다** — 표시만 이중 억제돼 있다(`page.jsx` 의 `voiceOn && agentPresent` 게이트).
   - 그래서 필요한 건 저장 코드가 아니라 **「누가 기록의 주인인가」 규칙**이다.
     후보: 봇이 나를 전사 중이면 내 클라이언트 받아쓰기를 멈추고 봇 자막을 기록의 정본으로 삼기.
     (봇이 «나를» 전사 중인지는 내 identity 로 온 원문 자막이 실제로 도착하는지로 알 수 있다.)
   - **지금 위험 0** — 통역봇은 기본 꺼짐이고 PO 는 통역을 안 쓴다. 켜기 전에 반드시 정할 것.
   - **재검토: 2026-09-05** · 통역 기능을 실제로 켜기로 할 때 선행 조건.

2. 🔴 **통역봇: 방에 들어오자마자 곧바로 말하면 앞 몇 초가 통째로 유실된다.**
   - 실측(2026-08-06): 입장~통역쌍 생성 **약 2초**, 첫 자막까지 **약 5초**.
     정답 대본 `안녕하세요. 위암 수술을 받은 지 두 달 되었습니다…` 를 입장 0.15초 만에 흘렸더니
     자막이 **`든지 두 달 되었습니다…`** 로 시작했다(첫인사+병명이 통째로 증발).
   - 원인 지목 전 반증 검사 통과: *준비 지연이 원인이면 앞에 무음을 두면 살아나야 한다*
     → 앞에 무음 5초를 붙이니 **문장이 전부 나왔다.**
   - ⚠️ **「실사용에서 반드시 유실된다」고 쓰지 말 것** — 사람은 입장 직후 0.15초에 말하지 않는다.
     정확한 서술은 **「입장 직후 곧바로 말하면」**. 실제 상담 첫인사가 걸릴 수 있어 남긴다.
   - **재검토: 2026-09-05** · 위 1번과 같은 시점(통역 켜기 결정).
...
⚠️ **정정(2026-07-20) — 이미 유료다. 위 서술은 낡았고 위험했다.** PO가 AI Studio 화면을 확인한 결과 `healwith`(gen-lang-client-0011286315) 프로젝트의 Gemini 키가 **`Tier 1 · 후불`(결제 계정 연결됨)** 상태다. 즉 **한도 소진이 아니라 실제 청구가 리스크**다. 언제 전환됐는지는 그 화면으로 알 수 없다(표시된 `2026-02-23`은 *키 생성일*) — 필요하면 [Cloud Console 결제 거래내역](https://console.cloud.google.com/billing)에서 첫 청구일로 확인.
