...
> | 만족도 응답 | 1건 | **1건** | — |
>
> 🔴 **아래 「어시가 할 수 있는 것 3개」 중 ①②는 코드에 여전히 없다**(2026-08-03 전수 검색: 「실적에 안 잡힙니다」류 경고 0건 · 상담 종료 시 자동 완료 처리 0건). **2026-07-24 에 「고칠 방향」으로 적힌 뒤 열흘째 미착수다.**
> ③(소급 반영)은 `/admin/khidi/conversion` 에 **유치확정 버튼은 있다** — 다만 **「상담↔문의 연결」 소급 수단은 아직 없다**.
> → **①②는 PO 결정을 안 기다려도 되는 코드 작업이다.** 남은 24일 동안 실적이 생겨도 지금 구조로는 **또 안 잡힌다** — 유입을 늘리는 것보다 이게 먼저다.
> ⚠️ 단 **①②를 넣는다고 지금 0 이 채워지지는 않는다.** 이미 쌓인 79개는 파트너 미팅·시험분이라 소급해도 실적이 아니다(아래 원문 그대로). **앞으로 놓칠 것을 막는 장치**다 — 이걸 「지표 회복 수단」으로 보고하지 마라.
...
> 🔴 **2026-08-03 건강검진 — 위 기한(2026-07-31)이 지났고 결과 기록이 없었다.** 실DB 재조회: `surveys` **1건**·사후관리(`post_care`) 세션 **0건**. **넘어간 케이스는 없다.**
