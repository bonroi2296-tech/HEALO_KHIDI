...
- 코드는 완료 — `app/api/cron/daily-deploy/route.ts` + `vercel.json` 예약 등록.
- ⏳ **PO 손 필요(마지막 한 칸)**: 깃허브 세분화 토큰(이 저장소 `contents: write` 하나만) 발급 → Vercel 환경변수 `GITHUB_DEPLOY_TOKEN` 에 등록. **토큰 값은 어시가 대신 넣을 수 없다**(비밀번호·열쇠 입력 금지 규칙).
- 토큰이 없는 동안은 이 경로가 **503 을 내고 아무것도 안 한다**(성공한 척 안 함) → 그동안은 깃허브 예비 창구가 늦게라도 연다.
