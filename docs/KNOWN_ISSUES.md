...
## 🔴 AI 자가시험이 «실서비스 경로»를 안 타고 있다 — 고침은 2026-08-21 에 만들어졌는데 본판에 안 들어갔다 (2026-09-05 발견)

**증상**: 자가시험이 `generateText` 를 직접 불러서 **3-Tier RAG 를 통째로 건너뛴다.** 응답시간도 「완료 시각」만 재는데, 우리 기준(NFR-02)은 **첫 토큰 5초**라 다른 자다.

**실측 (2026-09-05)**

| 무엇 | 상태 |
|---|---|
| 본판 `src/lib/chat/regressionRunner.ts` | `generateText` 직접 호출 (`streamChatReply` 0줄) |
| 관리자 화면 `app/admin/khidi/ai-regression` | 「첫 글자까지」 0줄 |
| DB `ai_regression_runs` | 전체 **2,512건** 중 `first_token_ms` 가 있는 건 **50건** |
| 그 50건 | 2026-08-21 에 아래 작업본이 돌린 것. **그 뒤로 안 채워진다** |
| 최근 실행 | 2026-09-03 — 시험은 계속 도는데 새 지표는 안 쌓인다 |

**🛑 DB 는 이미 바뀌어 있다.** `first_token_ms`·`rag_chunk_count` 컬럼이 있는데 **채우는 코드가 본판에 없다.** 마이그레이션만 적용되고 코드는 안 들어간 형태다.

**고침은 이미 있다**: 작업본 `claude/wonderful-shirley-b85229`(2026-08-21). 원격에 올라와 있지 않았고 신청서도 없어서 **증발 직전이었다** — 2026-09-05 에 건져 신청서 [#1617](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/1617) 로 올렸다.

**왜 중요한가 — 환자가 받는 답 얘기다.** 자가시험은 「환자에게 나가는 답이 괜찮은가」를 대신 봐 주는 장치다. 그런데 그 시험이 실서비스와 «다른 길»로 답을 만들어 채점하고 있으면, **실서비스 답이 나빠져도 시험은 계속 「통과」라고 한다.** 지금 쌓이는 점수는 실서비스가 실제로 내는 답이 아니라 다른 것을 잰 값이다. 응답시간도 마찬가지다 — 환자가 체감하는 건 「첫 글자가 언제 뜨나」인데 우리는 「완료 시각」을 재고 있다.

**⚠️ 합치기 전에 확인할 것** (2026-09-05 실측: `git merge-tree` 로 본 **충돌 덩어리 0개** — 코드가 부딪히지는 않는다):
- 2026-08-24 에 품질 점수 관련 다른 조치가 있었다(그 이전 값은 오염으로 기록돼 있다). **두 조치가 같은 자리를 다르게 고치지 않았는지** 본다.
- 2주 넘은 작업본이라 `generateReply.ts` 가 그 사이 바뀌었을 수 있다. **검사를 돌려 보고** 판단한다.
