...
## 🔴 KHIDI 사전상담 실적이 **구조적으로 0** — 실제 화상회의를 해도 자동집계에 한 건도 안 잡힘 (2026-07-24 실측, 8/27 중간평가 직결)
...
## 🔴 자동검사(E2E)가 프로덕션 DB를 그대로 물고 **작업본 22갈래가 동시 진입** — 전역 상한 없음, 미조치 (2026-07-24 실측, 2026-07-27 주 최우선)
...
## 🔴 프로덕션 장애를 제때 알 방법이 없음 — **PO 결정 필요(외부 업타임 모니터, 무료)** (2026-07-24, POSTMORTEMS #117)
...
**⚠️ 그런데 믿으면 안 되는 이유(실측)**: 이 저장소의 GitHub 예약 실행은 **선언 시각보다 64~98분 늦게** 뜬다(chat-smoke `30 18`→19:53~20:08 · e2e `0 19`→20:04~20:16 · audit `0 16`→17:12, 10회 전부). 신설한 `*/10`은 **2시간 20분 동안 0회 발화**(배선 정상 확인: `state: active`·main 반영·문법·수동 실행 성공). 즉 **즉시 경보가 아니라 "언젠가 남는 기록"** 수준.
...
## 🟠 의존성 취약점 공고 1건이 「고칠 상위 버전 없음」 상태로 한시 예외 중 — **만료 2026-08-25** (2026-07-25)
...
**만료일에 할 일**: `npm audit` 재확인 → 상위 패치가 나왔으면 올리고 ALLOWLIST 에서 제거, 아직도 없으면 근거를 다시 적고 연장.
