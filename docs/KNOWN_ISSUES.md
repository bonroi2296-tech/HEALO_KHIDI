...
- **④ 수신 자막 실기기 미검증**: 원격 트랙 MediaRecorder 녹음이 iOS Safari에서 동작하는지 미확인(마이크 캡처와 달리 getUserMedia 불필요라 이론상 안전). 2인 실통화 검증 체크리스트는 PR 본문. ※ 수신 자막(구 "청취 모드")은 별도 토글이 아니라 **통역 스위치에 통합**됨(2026-07-11 PO "하나로 통일" — 마이크 끄면 듣기만).
- **⑤ 수신 자막 이중 자막 경계 케이스**: 화자가 직접 통역을 켠 경우 DataChannel 자막 우선(60초 억제창)이지만, 그 화자의 **첫 발화**와 60초+ 침묵 직후 발화는 수신 STT와 한 번씩 겹칠 수 있음(비용 2배 + 같은 문장 두 번 표시). 또 Gemini Live Translate(경로 C, 현재 꺼짐)를 켜면 그 자막은 dcActivityRef에 안 잡혀 수신 자막과 상시 중복 — **Live Translate GA 재검토 시 함께 처리**.
- **⑥ 수신 자막 N명 중복 비용**: 통역을 켠 참가자마다 상대 발화를 각자 STT할 수 있음(화자가 직접 자막을 보내면 억제되므로 보통은 1회). 통역 통합(2026-07-11)으로 노출이 커졌으니 사용량을 어드민 외부서비스 사용량 화면에서 관찰 — 늘면 전사 결과를 DataChannel로 공유해 1회 전사로 통일.
