...
- ✅ **영상 (2026-07-29 — PO: *"영상도 잇자나"*)**
  - **병원 유튜브 `@면력한방병원`(채널 `UCskgk33_l9lrmRrFlOif9ig`)에 영상이 47개** 있다. 판엔 6개만 실려 있었고
    **전부 요리·공예였다 — 의료 정보 영상 11개가 하나도 안 실려 있었다.** 해외 암환자용 화면인데
    「수술 후 팔이 붓는 이유(림프부종)」를 병원이 이미 갖고 있으면서 안 보여주고 있었다. → 5개 추가(총 15개).
  - **목록 뽑는 법(다음 세션용)**: 채널 페이지를 `curl` 로 받으면 `"videoId":"..."` 가 전부 들어 있다.
    제목은 `https://www.youtube.com/oembed?url=…&format=json` 으로 하나씩(영상 페이지 직접 긁기는 **429로 막힌다**).
    최신 15개만 필요하면 `https://www.youtube.com/feeds/videos.xml?channel_id=…` 가 더 쉽다.
  - 🔴 **유튜브 「쇼츠」 썸네일 함정**: 쇼츠 썸네일도 1280x720 으로 오는데 **가운데 405px 만 진짜 내용**이고
    양옆 3분의 2는 «같은 그림을 흐리게 확대한 배경»이다. 16:9 상자에 그대로 넣으면 카드의 3분의 2가 얼룩이 된다.
    → 썸네일을 `405x720` 으로 미리 자르고 데이터에 `portrait: true` 를 준다. 판이 9:16 카드로 그리고,
    **세로줄·가로줄을 따로 그린다**(섞으면 카드 높이가 2배 차이 나 줄 아래가 톱니처럼 된다 — 화면으로 확인).
    쇼츠인지 판정은 «가장자리 선명도 대비 가운데 선명도» 비율로 갈린다(쇼츠 11.8~16.6배 vs 가로 1.7배 이하).
  - 🔴 **썸네일은 「글자까지 읽고」 골라야 한다**: 대상포진 영상 하나(`RTGo9vgjM3Y`)를 넣었다가 뺐다 —
    썸네일 글자가 「우리 병원은 포기할게요… 앰뷸런스 타고 가야합니다」였다. 내가 붙인 번역 제목과 그림이 안 맞고,
    한국어를 못 읽는 방문자에겐 **그림만 남는다.**
  - 🔴 **환자 치료후기 영상은 안 쓴다** — 드라이브 `02_영상/01_환자후기` 에 9건이 있지만
    **의료법 제56조가 「치료경험담 광고」를 금지**한다. 병원이 촬영해 뒀어도 병원 홈페이지에 싣는 건 다른 문제다.
    (덤으로 원본이 670~900MB라 받지도 못한다.)
  - ⚠️ **못 확인한 것**: 이 영상들에 러시아어 자동자막이 붙는지 — 유튜브가 요청을 막아(429) **검증 못 했다.**
    화면에는 「한국어 영상이며 상담 때 통역과 함께 설명한다」로만 적어 뒀다(확인 안 된 걸 단정하지 않으려고).
  - 🎬 **PO 손 필요 — 이게 제일 큰 한 방**: 드라이브 `02_영상/09_시설 스케치 영상/성동` 에
    **`260629_성동점_이뮨포레스트_30s.mp4`(34MB)** 와 **`260629_성동점시설영상_highlight_30s.mp4`(25MB)** 가 있다.
    **말이 안 나오는 30초 시설 영상 = 언어가 필요 없다 = 외국어 페이지에 제일 좋은 자산**인데
    ①25~34MB라 내가 못 받고 ②유튜브에 안 올라가 있다. **유튜브에 올려주면 그 자리에서 판 맨 위에 붙인다.**
- 🔴 **`14_푸드테라피`·`15_면역밥상` 은 사실상 못 쓴다 (2026-07-29 표본 확인)** — 파일 크기는 받을 만한데
  (350~790KB) **환자 얼굴이 그대로 나온다.** 표본 4장 중 2장이 환자복 입은 환자들의 식사 장면·수업 장면이었다.
  이 폴더들은 「환자가 참여하는 활동」 기록이라 당연한 결과다. → 얼굴 규칙에 따라 **쓰지 않는다.**
  쓸 수 있는 건 「접시만·셰프만」 나온 소수인데, 그건 이미 `chef-*`·`dining-*` 로 충분하다.
  `12_원데이클래스`(클래스 20여 개)도 같은 성격이라 같은 결론일 가능성이 높다 — 열더라도 한 장씩 눈으로 보고 골라라.
- ✅ **병원 홈페이지 전수 크롤 (2026-07-29, PO: *"병원 홈페이지부터 계속 파봐"*)** — 내부 링크를 전부 뽑아
  **26쪽을 받아 읽었다.** 앞선 세션이 「의료진·시설·비급여·서류발급」만 보고 **끝났다고 판단했는데 틀렸다.**
  - 🔴 **제일 큰 누락은 FAQ 였다.** 사이트 FAQ 가 **4쪽 페이지네이션**인데 1쪽만 읽고 끝냈었다
    (`?page=2`·`3`·`4`). 2~4쪽에 **해외 환자한테 제일 필요한 것**이 다 있었다 —
    입원 필수 지참 서류 5종 · 입원 준비물 · 보호자 상주/간이침대/식대 6,000원 · 면회 규칙 ·
    외부 음식 반입 · 외출증 · 지참약 제출 · 응급 시 대학병원 전원 · 퇴원 정산 시간.
    → 「해외 환자 안내」에 표 2개(`비행기 타기 전에 챙기실 것`·`입원해 계시는 동안`)로 옮겼다.
    **교훈: 목록형 페이지는 페이지네이션부터 확인하라.** 1쪽만 보고 「다 봤다」고 하면 4분의 1만 본 것이다.
  - ✅ 암면역센터(`/pages/cancer/home.php`)에 **프로그램 주기**가 적혀 있었다(평일 오전 산책 버스 · 주 1회
    소풍/운동치료/원데이클래스 · 2주 1회 푸드테라피). → 「진료 안내」에 `입원해 계신 한 주` 표로 옮겼다.
  - ⏸ **아직 안 옮긴 것(쓸 만한데 자리를 못 잡음)**: `/pages/rehab/postop.php` 의 **재활 3단계**
    (수술 직후 → 보호기 → 회복기)와 **장비·치료 이름**(CPM 지속수동운동 · CRYO 극저온 냉각 · 1:1 전담 운동치료).
    지금 판이 종양 중심이라 안 넣었다 — 재활을 앞세울 때 여기서 가져오면 된다.
  - 🔴 **이 페이지들엔 옮기면 안 되는 게 섞여 있다.** `/pages/rehab/postop.php` 에는 **환자 치료경험담**
    (「김길영님 67세 … 2주 만에 눈에 띄는 변화」)이, `/pages/cancer/home.php` 에는 **효과 단정 문구**
    (「항암치료율 향상」·「암세포 증식 억제」)가 그대로 있다. 둘 다 **의료법 제56조**가 막는 것이다.
    → 새로 본 표현을 `tenant.test.ts` 금지 목록에 추가하고 빨간불 확인까지 했다(극대화·항암치료율·재발 억제 등).
  - ℹ️ 한약 페이지(`/pages/herb/gongjin.php`·`uahwan.php`)는 **비대면 예진표**(약 5분짜리 문진 폼)다.
    한국어 전용이라 그대로는 못 쓰지만, **«상담 전에 문진을 받아 둔다»는 구조 자체는 판이 배울 만하다.**
  - ℹ️ 게시판 3개는 **더 나올 게 없다**: `board_no=11` 치료후기(**의료법 제56조로 못 쓴다**) ·
    `12` 면역채널(셰프특식 3건 = 유튜브 미러, 이미 실려 있음) · `13` 면역소식(월간 진료일정 공지, 내용은 이미지).
    목록은 서버 HTML 에 없고 자바스크립트가 나중에 채운다 — `curl` 로는 사이드바의 최근 3~4건만 보인다.
- 🖼 **지점 시설사진 풀 — 아직 79장 남아 있다 (2026-07-29 실측)**
  네 지점 사이트의 시설안내에 **총 98장**이 있는데 **한 장도 겹치지 않는다**
  (강서 20 · 성동 41 · 신촌 19 · 광명 18, 겹침 **0**). 지점마다 따로 촬영했다는 뜻 = 대표 사진 돌려쓰기 불가.
  - 받는 법: `https://{sc|km|sd}.immunehospital.com/pages/hospital/facility.php` 를 `curl` →
    `uploads/banners|facilities/` 경로의 `<img>` 를 뽑는다. **`alt` 에 무슨 공간인지 적혀 있어** 분류가 쉽다
    (「VIP 입원실 1」·「다인입원실 2」·「회복을 끌어 올리는 다양한 치료공간 3」·「24시간/365일 힐링…」).
  - 크기가 제각각이다: 신촌은 전부 546×546, 광명은 410×410 / 720×928 / 1101×1101 이 섞여 있다.
    사진 격자 첫 칸(2×2)에 쓰려면 **1100px 이상**이 필요해서 쓸 수 있는 게 몇 장 안 된다.
  - 🔴 **광명 2장은 환자 얼굴이 나온다**(안마의자에 앉은 두 분 · 족욕 자리의 한 분). 병원이 자기 사이트에
    올린 사진이어도 **우리가 다시 싣는 건 별개 판단**이라 뺐다. 지점 사진을 더 쓸 땐 **한 장씩 눈으로 보고** 골라라.
  - 이번에 쓴 5장: 광명 1인실·식당·커튼 친 치료 자리 / 신촌 창가 라운지·운동치료실 → 「오시는 길」 사진 격자.
  - 아직 안 쓴 것 중 눈에 띄는 것: 신촌 **고압산소챔버**(병원 로고 붙은 O2 CHAMBER)·창가 러닝머신,
    광명 **물결 나무 천장 식당**(410px라 작음)·좌훈 캐비닛·귀중품 금고 키패드.
