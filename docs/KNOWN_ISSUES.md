...
## 🟠 KHIDI 중간평가 — **평가일(2026-08-27)이 지났다. 결과가 저장소 어디에도 안 적혀 있다** (본문은 2026-08-04 에서 멈춤)

> 🛑 **PO 확인 한 줄 필요**: 8/27 평가가 ①끝났나(결과는?) ②연기됐나. 잔금 30% 가 걸린 항목이라 «모르는 채로 지우지 마라».
> · 끝났으면 → 제목을 「(종료) 2026-08-27 평가 — 결과 …」로 바꿔 기록으로 남기고 경보를 내린다.
> · 연기됐으면 → 새 날짜로 갱신한다.
> **2026-08-31 정정**: 이 제목은 나흘 전까지 「**29일 남았는데**」라고 현재형으로 말하고 있었다(2026-07-29 기준 그대로). 저장소 전체에 평가 결과·연기를 적은 줄이 0건이라, 지금 이 항목을 읽는 사람은 **이미 지난 일정을 지시받는다**. 아래 본문의 「남은 N일 / 하루 0.41건」식 계산도 전부 그 날짜 기준이다.
...
## 🟡 P1 — K-01 유치 점수판 admitted 4건 = 시드 데모데이터 (진짜 유치 0건) — **기한(8/27)이 지났고 사후 기록이 없다**

> **2026-08-31 정정**: 「8/27 전 실데이터 대체 필요」라는 기한이 지났는데 그 뒤 아무 메모가 안 붙었다. 위 KHIDI 항목과 같은 뿌리다 — PO 에게 **「8/27 평가에서 이 숫자를 어떻게 설명했나」** 한 줄을 받아 여기 적고, 그다음에 시드 4건을 계속 둘지 지울지 판정한다.
