...
## 🔸 화상상담 — 2026-07-23 PO 결정 4건이 **8일째 미착수** (2026-07-31 PO 지적으로 등재)

> **누락 경위**: 이 4건은 어시 기억파일(`consult-redesign-pending-0723`)에만 있었고 **이 문서·`PROJECT_CONTEXT` 「다음 할 일」 어디에도 안 적혔다.** 기억엔 「다른 화상상담 세션이 끝나면 착수」라는 조건이 붙어 있었는데, 그 세션들은 7/24~7/25 에 끝났고 **조건이 풀렸는지 아무도 다시 안 봤다.** 조건부 대기 항목을 추적 문서에 안 올리면 이렇게 증발한다.

1. ~~**초대 링크가 너무 길다**~~ ✅ **완료 — 139자 → 57자 ([#1212](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/1212) 머지 2026-07-31 09:38, 실서비스 반영은 같은 날 15:00 창구).** `healwith.co.kr/c/<32자>`. 넘김주소 표는 **안 만들었다** — 코드 자체가 입장권이라 해시로 상담을 찾으면 돼서, DB엔 여전히 해시만 남는다. 예전 링크·긴 형식도 계속 유효.
2. **역할(환자·의사·통역사·코디·참관·게스트) 제거** — 6/21 이후 권한이 전부 같아 죽은 이름표. PO: 역할만 없애고 호스트/강퇴는 보류.
3. **「✅ 영상 연결 준비 완료」 문구 정직화** — 실제로는 카메라를 안 보고 인터넷망만 잰다. 6개 언어 문구 수정(`_roomCopy.js`).
4. **카메라 없을 때 표시** — 미리보기 검은 화면에 "카메라 못 찾음 / 소리만 입장" 안내.
5. **(2026-07-31 추가) 입장 화면 문구가 실제와 다르다** — 방 입장 폼 아래에 *"이 링크는 본인 상담 1회에만 유효합니다"* 라고 적혀 있는데, 실제 발급 기본값은 **만료 전까지 무제한**(`max_uses: 0`, PO 2026-07-15 결정)이다. 3번 문구 정직화와 같은 자리에서 같이 고친다.

**언제 되나** — 2~5번은 **상담방 화면을 건드리므로 실회의가 없는 시간에.** 어시 손으로 됨(PO 손·남의 회신 불필요).
...
- 🆕 **2026-07-30 — 첫 실측 시도 결과: 「안 되고 있었다」.** PO 가 `?ga_debug=1` 로 열어 보낸 콘솔에
  ...
  - **아직 «검증됨» 아님.** 남은 것: **차단 없는 기기·회선에서** 자가진단 전 줄 ✅ + GA4 DebugView 에 이벤트 도착 확인.
    PO PC 에서 하려면 AdGuard 예외(또는 잠시 끄기)가 선행조건이다.
