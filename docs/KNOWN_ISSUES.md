...
## 🟠 외부 칼럼 3편 대조에서 나온 보완점 3건 — **분석만 완료, 수정 미착수 (PO 판단: 오늘은 분석만)** (2026-07-27)
...
- **애초 가설은 빗나갔음(기록용)**: `text-gray-400` 617곳 때문에 위반이 쏟아질 거라 봤으나, 실제로는 대부분 아이콘·큰 글씨라 위반 아님. 남은 `text-gray-500` 약 50곳도 흰 배경 위 4.83:1로 통과. **토큰 미달 ≠ 위반 건수** — 다음에 같은 추론을 반복하지 말 것.
- **남은 것(백로그)**: ①**측정 범위가 공개 페이지뿐** — 백오피스·환자포털은 로그인 뒤라 미측정(gray-400 다수가 거기 있음). 인증 세션을 스캐너에 물리는 건 별도 작업. ②대비 미달 토큰의 `check:content` 정적 차단은 미도입(정적 grep으로는 배경색을 알 수 없어 오탐이 많음 — axe 실측이 더 정확하다는 판단).
