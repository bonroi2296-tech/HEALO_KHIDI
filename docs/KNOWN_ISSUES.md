...
## 🔸 [B2B 판] 면력 의료진 27명 중 **2명은 사진 없이** 싣는다 — 병원 사이트에도 사진이 없다 (2026-07-29, 해결됨)

- **해결**: 의료진 **27명 전원**(강서 6·광명 7·신촌 6·성동 8)을 「의료진」 탭에 실었다. 이름·직함·진료과 6개 언어.
- **사진 확인 방법(이게 핵심)**: 파일명(`gangeo-dr-lee-useok.jpg`)을 근거로 삼지 않고, 병원 사이트에서 **27명 사진을 하나씩 받아 우리 파일과 나란히 놓고 눈으로 대조**했다. 결과 **25명 일치 확인**.
  - ⚠️ 사진 파일은 지점 사이트가 아니라 **본원 서버**(`immunehospital.com/uploads/doctors/`)에 있다. 지점 주소로 받으면 404 — 여기서 한 번 헛돌았다.
  - ⚠️ 연속 요청을 막는 서버라 **요청 간격(1.8초)**을 둬야 전부 받아진다.
- **남은 2명 — 배상근(강서)·조현실(신촌)**: 병원 사이트가 이 둘만 **로고 자리표시**를 띄운다(사진이 없다). 우리 저장소에 `gangeo-dr-bae-sanggeun.jpg` 같은 파일이 있지만 **지금 사이트로는 그 사람이 맞는지 확인할 방법이 없다.** → **사진 없이 싣는다.** 이름은 맞는데 얼굴이 다르면 주소 오류보다 큰 사고라, 확인 안 된 얼굴은 안 붙인다.
  - **다음에 할 것**: 병원에 이 둘의 사진을 요청하거나, 재직 여부를 확인한다(사이트에서 사진을 내린 게 퇴사 신호일 수도 있다).
- **곁가지로 고친 것**: 기존 의료진 4명의 **중국어 이름이 한자**(黄利俊·柳亨镇·裴吉俊·姜周安)로 적혀 있었는데 **병원 사이트 어디에도 한자 이름이 없다** = 지어낸 것. 한글 이름에서 한자는 유추할 수 없다(황이준 = 黃利俊 일 수도 黃二準 일 수도 있다). **실존 인물의 이름을 틀리게 쓰는 것**이라 지점 주소를 지어낸 것과 같은 부류(반성문 #159) → 한자가 확인되기 전까지 한글 그대로 둔다.
