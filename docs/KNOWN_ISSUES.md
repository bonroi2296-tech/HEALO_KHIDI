...
- **공개 AI챗(`/inquiry`)을 환자챗(`/patient/chat`)처럼 멀티 스레드로 통일** (🟢 큰 플로, PO 결정 2026-06-29): 현재 공개챗은 쿠키 토큰 1개로 **한 대화방을 30일 영구 재개** → "대화가 끝없이 연장"(PO 지적). 끝내기·새 주제 시작 버튼이 코디 resolve 외엔 없음. 모델 입력은 최근 12개로 제한돼 비용/품질은 안전하나, 긴 스레드가 빈응답·옛 암종 over-anchoring·반복루프 버그를 유발해 **땜질 가드가 누적**(generateReply.ts repetition/empty-reply 가드) = 근본은 "세션 경계 부재". 방향: Claude/타 AI처럼 **세션(대화방)별 분리 + "새 상담" 버튼 + 오래 쉰 대화 자동 새 세션**, 로그인 사용자는 **계정에 대화 이력 동기화(기기 간 공유)**. 환자챗에 이미 있는 멀티스레드 UI(`PatientChatClient.jsx` newChat/history)를 공개 퍼널로 통합하는 게 최단. ※ 비로그인 게스트는 쿠키 기반 유지.
