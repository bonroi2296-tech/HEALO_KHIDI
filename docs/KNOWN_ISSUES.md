...
- **어시가 한 것 (2026-08-19)**: ①동작 보강 — 첫 화면에서 바로 끄지 않고 **「한 번 더 누르면 종료」**(2초, 6개 언어 안내) ②그 동작을 잠그는 시험 5건 ③**`npm run sweep` 에 「폰 앱에 안 들어간 고침」 칸 신설** — 같은 함정이 다시 조용히 지나가지 않게. 경위·근본원인 = 반성문 `#171`.
- 🛑 **남은 것 = PO 손**: **앱 파일(AAB)을 새로 구워 Play 에 올려야 폰에 반영된다**(Codemagic 실행 + 콘솔 업로드는 어시가 원천적으로 못 한다). 그 판에는 뒤로가기 말고도 **아이디·비밀번호 칸이 서로 바뀌어 입력되던 것**(8/14, 입력 가로채기 설정 제거)이 같이 들어간다 — 지금 폰에서는 그것도 안 고쳐진 상태다.
