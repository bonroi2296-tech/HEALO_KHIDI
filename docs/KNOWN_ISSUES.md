...
## 🔸 공개 사이트 언어 저장 `setLangCookie()` 도 **백오피스와 같은 모양** — 재보지 않았다 (2026-07-29, 반성문 #155 곁가지)

- **무엇**: `src/lib/i18n/index.js` 의 공개용 `setLangCookie()` 가 아직 `if (!DICTIONARY[code]) return;` 로 코드를 검사한다. 백오피스 쪽 같은 코드는 **브라우저 번들에 사전이 없어 항상 실패**했고(=언어 전환 사망) 오늘 `isKnownLangCode` 로 고쳤다.
- **왜 안 고쳤나**: 공개 화면은 **실제로 6개 언어가 잘 돈다**(러시아어·카자흐어 화면이 그려지고 검색엔진에도 색인된다) → 공개 라우트 번들에는 사전이 실려 있는 것으로 **보인다**. 근거 없이 같이 고치면 오히려 동작을 바꾼다.
- **⚠️ 그러나 「보인다」이지 잰 게 아니다.** 확인법: 공개 화면에서 언어를 고른 뒤 `document.cookie` 에 `healo_lang` 이 실제로 바뀌는지 브라우저로 1회 측정. 안 바뀌면 같은 병이고, 그때는 `isKnownLangCode` 로 교체.
- **영향 범위**: 만약 같은 병이면 **환자가 고른 언어가 저장되지 않는다**(새로고침하면 원래대로) — 광고로 들어온 러시아어 사용자에게 직접 영향.
