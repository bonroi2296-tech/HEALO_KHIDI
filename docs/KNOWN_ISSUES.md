...
**한 줄**: 뒤로가기를 받는 건 **네이티브 부품(`@capacitor/app`)** 인데, 스토어에 나가 있는 판(빌드 **7 / 1.0.6**, 8/13 게시)은 **8/4 소스**라 그 부품이 없다. 고침은 **8/5**(`bd3f2061`). 하루 차이로 안 실렸고, 그 뒤 앱 파일을 다시 안 구웠다. **웹 배포로는 영원히 안 간다.**

| | 스토어 판(폰에 깔린 것) | 저장소(지금) |
|---|---|---|
| 버전 | 7 / 1.0.6 (8/13 게시, 소스는 8/4) | 9 / 1.0.8 (아직 안 구움) |
| `capacitor-app`(뒤로가기 부품) | **없음** | 있음 |
