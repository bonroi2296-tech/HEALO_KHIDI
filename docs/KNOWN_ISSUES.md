...
## 🔸 쿠키 동의 배너(`z-[9999] fixed bottom-0`)가 **화면 하단 고정 버튼을 덮는다** — 배너를 닫기 전엔 클릭 불가 (2026-07-28 실측)
...
- **고친다면**: 배너가 떠 있는 동안 body 에 하단 패딩을 주거나(배너 컴포넌트가 자기 높이를 CSS 변수로 노출), 하단 고정바들을 배너 위 레이어로 통일. 배너 자체를 가리는 방식은 동의 UI라 피할 것.
