...
**같은 밤 덤 — 치료 카드 설명도 비어 있었다**: 암종 상세 페이지의 치료법 카드(name·description·evidence)는 5축 검사 밖이라 description 이 **ru 10개·kz/zh/ja 19개**, evidence 3개 치료법의 ru·kz·zh·ja 가 비어 영어로 조용히 폴백되고 있었다 → 79문장 채움(AI 번역, 검수 전 「제안」; 완치 검사기 0건 — 카자흐어 「8 МГц」 가 mg 용량 규칙에 걸려 주파수 숫자를 뺐다). `check:cancer-i18n` 이 이제 이 세 칸도 본다(천장 0). 로컬 dev 실측: /kz/treatments/thyroid 카자흐어 저요오드 설명·/ja/treatments/digest 일본어 위절제 식이 설명 표시, 영어 폴백 0.
