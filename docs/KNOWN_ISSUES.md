...
**A. 🔴 접근성 가드에 구멍 — "위반 0건"을 신뢰할 수 없음**
- `scripts/audit/a11y-scan.mjs`에 **내용 검증이 없다**(HTTP 상태·요소 존재 확인 X). 화면이 하얗게 렌더되면 axe가 위반 0을 반환하고 그게 "통과"로 기록됨 — SEO 진단 때 겪은 「자동화 브라우저가 빈 화면을 렌더」와 같은 부류.
- 커밋된 `docs/audit/a11y-report.json`은 7개 페이지 전부 `ruleViolations: 0, nodes: 0`인데 `text-gray-400`이 코드에 617곳 — **이 0은 검증된 0이 아니다.**
- `.github/workflows/audit-live.yml`은 `npm run audit:a11y || true` = **절대 안 막음.** 리포트도 아티팩트로만 올라가 커밋본은 2026-06-20 프리뷰 URL 기준(5주 낡음). 워크플로 자체는 7/20까지 정상 실행 중.
- **수리안**: ①스캔에 "본문 노드 N개 이상" 단언 추가(빈 화면=실패) ②대비 미달 토큰을 `check:content` 룰로 차단 ③리포트를 커밋(증거 보존).
