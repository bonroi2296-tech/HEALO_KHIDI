...
## 🔸 스토어 앱(안드로이드)에서 화면이 **상태표시줄·하단 버튼줄과 겹친다** — 「안전영역」 여백이 헤더/푸터에 안 붙어 있음 (2026-07-28, PO 가 실기기 스크린샷으로 최초 발견)

- **증상**: 갤럭시 S25 Ultra 실기기에서 ①상단 — 시계·배터리 뜨는 줄(상태표시줄) 밑에 웹 화면이 깔려 겹침 ②하단 — 안드로이드 3버튼 줄 밑까지 내용이 내려감.
- **원인**: 앱이 웹뷰를 화면 끝에서 끝까지 그린다. 그걸 피하라고 브라우저가 주는 값(**안전영역 · safe area**)은 이미 들어오고 있고(`app/layout.jsx:187` 의 `viewport-fit=cover`), 그걸 쓰는 CSS 도구도 만들어져 있다(`app/styles/healo-tokens.css:266` 의 `.healo-safe-top/bottom/left/right`). **그런데 실제로 쓰는 곳이 `app/patient/messages/MessagesClient.jsx:268` 딱 한 군데뿐이다.** 헤더에도, 다른 하단 고정 요소에도 안 붙어 있다.
- **왜 지금껏 안 드러났나 (근본원인)**: 이 결함은 **브라우저에선 원리상 안 보인다** — 상태표시줄도 안드로이드 버튼줄도 없으니 겹칠 대상이 없다. #240(2026-06-22)은 「스토어에 제출 가능한 껍데기」까지가 범위였고(아이콘·스플래시·푸시 등록·라이브 로드 설정), 그 뒤 어느 세션도 **실기기·에뮬레이터로 앱을 열어본 적이 없다.** 검증이 전부 웹 프리뷰였다 = 「빌드 통과 ≠ 동작」 부류. 오늘 PO 스크린샷이 **첫 실기기 확인**이다.
- **고치는 법 (3개, 미착수)**:
  1. 헤더에 `healo-safe-top` — 상태표시줄 높이만큼 자동으로 내려감.
  2. 하단 고정 요소(푸터·모바일 하단내비·CTA 바)에 `healo-safe-bottom`. ⚠️ 여기는 쿠키 배너 변수(`--cookie-banner-h`)와 **둘 다 걸리는 자리**라 `calc()` 로 합산해야 한다(위 쿠키 배너 항목 참조 — 그냥 덮어쓰면 한쪽이 날아간다).
  3. (대안) `capacitor.config.ts:38` StatusBar 에 `overlaysWebView: false` — 웹뷰가 상태표시줄 **밑에서부터** 시작. **1번과 둘 중 하나만** 해야 한다(둘 다 하면 여백을 두 번 먹어 뻥 뜬다).
- 💡 **1·2번은 앱 재제출이 필요 없다** — 이 앱은 `server.url` 로 실서비스 사이트를 그대로 불러오는 구조라 웹만 고치면 앱에 바로 반영된다. 3번만 네이티브라 앱을 새로 빌드해야 함.
- ⚠️ **검증 수단이 없는 게 진짜 문제다.** 고쳐도 어시는 실기기로 확인할 방법이 없다 → 수정 후 **PO 폰 스크린샷**으로 확인받아야 한다. 스토어 등록 전에 이 확인 절차를 한 번은 넣을 것.
...
- ⚠️ **이건 네이티브라 앱을 새로 빌드해 스토어에 재제출해야 한다** (위 안전영역 항목의 1·2번은 웹만 고치면 되는 것과 다름). **심사 통과 전이면 위 3번(`overlaysWebView`)과 묶어 한 번에 올리는 게 이득.**
