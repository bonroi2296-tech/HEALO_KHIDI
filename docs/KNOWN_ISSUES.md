...
> **2026-09-05**: `ITCRN_FRAMEWORK` 의 잎 38개(제목·설명 빈 칸 38 + 한국어뿐이던 근거·태그·항암지원 24)를 전부 `{ko,en,ru,kz,zh,ja}` 로 채웠다. 화면(`CancerDetailClient`)의 `lang === "ko"` 조건 제거 → 러·카·중·일·영 페이지에도 근거 문장과 요법 태그가 뜬다. RAG 는 예전처럼 한국어 문서만 만든다(객체 잎 대응). `check:cancer-i18n` 은 이제 **잎 전부**를 보고 천장 0(늘면 차단). AI 안전 검사기(cure/prognosis)에 새 문자열 228개를 돌려 걸린 것 0.
