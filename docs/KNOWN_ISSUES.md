...
## 🟡 상담방 「통역」 버튼이 **지킬 수 없는 약속**을 한다 — 봇은 꺼져 있는데 «들어오면 들려요» (2026-07-28 실측, PO 결정 대기)

- **상태 실측**: 프로덕션 `LIVE_TRANSLATE_ENABLED=false`(2026-07-23 16:51 UTC, PO 판단 — 봇 타일이 1:1 레이아웃을 깨고 삼중자막 유발) / `NEXT_PUBLIC_LIVE_TRANSLATE_ENABLED=true`(그대로). **두 스위치가 어긋나 있다.**
- **증상**: 게스트·의료진 화면에 통역 토글이 그대로 보이고, 누르면 «통역봇이 방에 들어오면 들려요» 토스트가 뜬다. 서버가 봇을 디스패치하지 않으므로 **영원히 안 들어온다.** 야간 로봇 통화가 이 토스트를 그대로 물어와 실패했다(반성문 #138).
- **왜 지금 이 상태인가**: 클라 플래그를 `true` 로 남긴 건 의도 — `PartnerLangBridge` 의 `lang` 속성 발행(자막 경로)이 이 플래그를 탄다. 봇만 끄고 자막은 살리려던 것.
- **선택지(PO 결정)**: ①버튼 숨김(봇 없을 땐 아예 안 보이게) ②문구를 «준비 중»으로 ③공존설계 후 봇 재가동(`LIVE_TRANSLATE_ENABLED=true` + 재배포 + `E2E_INTERPRETER=1` 동시에). ③은 봇을 참가자 타일·클라 STT 경로에서 배제하는 작업이 선행조건.
