...
> ✅ **①번 조치 완료 (2026-07-27)** — `.github/workflows/e2e.yml` 의 **job 레벨**에 `concurrency: {group: e2e-prod-db, cancel-in-progress: false}` 를 세 잡(smoke·full·production-nightly) 전부에 걸어 **저장소 전역 직렬화**(동시 1개, 나머지는 대기)했다.
> - **왜 workflow 레벨이 아니라 job 레벨인가**: GitHub 의 concurrency 는 «동시 N개»가 아니라 «그룹당 1개»다. workflow 레벨 그룹을 전역으로 바꾸면 `cancel-in-progress: true` 면 **남의 PR 실행을 내 PR 이 죽이고**, `false` 면 **브랜치 내 중복 제거를 잃는다**. job 레벨은 workflow 레벨과 독립된 자물쇠라 둘 다 가진다(workflow=브랜치 내 중복 제거, job=전역 직렬화).
> - **상한 1이 타당한 근거(산수)**: 위 실측 «14시간 89회» × 1회 4~7분 ≈ 점유 445분 / 창 840분 = **약 53% 가동률** → 직렬화해도 흘러간다. 대기 중인 잡은 러너를 점유하지 않는다.
> - ⚠️ **아직 «응급 처치»다.** 근본은 여전히 ②번(연습용 DB 분리). 그리고 **직렬화 효과는 실측 안 했다** — 다음 세션이 며칠치 실행 이력으로 ⓐPR 초록까지 걸리는 시간이 얼마나 늘었나 ⓑ동시 진입이 실제로 1로 떨어졌나를 확인할 것.
> - 📝 같은 날 `#1004` 에서 workflow 레벨 그룹에 `github.event_name` 을 추가했는데(수동 점검이 남의 머지에 죽는 문제), 그건 **동시성을 더 잘게 쪼개는** 방향이라 이 항목과 반대였다. job 레벨 전역 자물쇠가 그 부작용까지 덮는다.
