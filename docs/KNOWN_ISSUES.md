...
| 3 | 전역 「움직임 줄이기」 블록이 `animation-delay`·`transition-delay` 는 안 죽이고, 3초 자동 넘김 갤러리가 «미끄러짐» 대신 «점프컷» 이 된다 | 맞다. 다만 점프컷은 **어지럼 쪽에는 더 안전한 방향**이고, 자동으로 넘어간다는 사실 자체가 남는 게 더 큰 문제다 | 자동 넘김을 줄이기 사용자에겐 **아예 멈추는 것**이 정답. 갤러리 두 곳 손봐야 함. **재검토: 2026-08-20** · 지금 가능 · 어시 몫 |
...
1. **에이전시 포털 — 병원 응답 경로의 타임라인 중복이 미해결** (기억파일 `agency-portal-improvements-0707`, 2026-07-07 PR #706 때 남긴 것). 케이스 상태 이력 중복은 잡았는데 `hospital_leads → case_status_history` 경로는 별도 소스라 그대로다. **재검토: 2026-08-07** · 지금 가능 · 어시 몫.
