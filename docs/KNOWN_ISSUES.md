...
## 🔸 AI 케이스 브리프가 **한국어로만** 나온다 — 러시아어 코디는 못 읽는다 (2026-07-29 실측)

- 문의함 상세 맨 위의 「Бриф по случаю(케이스 브리프)」는 **틀·라벨은 러시아어인데 본문은 한국어**다(실측 스크린샷: 「인적사항 및 임상 상황에 대한 정보가 전혀 제공되지 않은 상태임.」).
- 화면 글자를 다 번역해도 **정작 읽어야 할 알맹이**가 한국어라 코디에게 반쪽이다.
- 손이 큰 이유: 브리프는 AI가 만들어 **암호화해 저장**한다. 언어를 바꾸려면 ①만들 때 코디 언어로 만들거나 ②언어별로 따로 저장해야 한다(한국 스태프는 한국어로 봐야 하므로 「그냥 러시아어로 바꾸기」는 답이 아니다).
- 다음에 이 화면 손볼 때 같이 — 지금은 화면 글자만 정리했다.
