...
- **무엇이었나**: 편집기가 「N곳 변경됨」 개수만 보여주고 저장해서, **무슨 키를 무엇에서 무엇으로 바꾸는지**를 저장 직전에 확인할 수 없었다. 값이 한 칸씩 밀려 들어가는 사고(#136 코디 붙여넣기, #139 내 자동화)는 **값들이 서로 겹치지 않아** 「문구 중복」 배지가 원리상 못 잡고, 두 사고 다 저장 후 DB를 봐야 발견됐다.
