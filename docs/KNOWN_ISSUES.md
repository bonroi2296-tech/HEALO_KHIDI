...
## ✅ [닫힘] 병원 지도 — 「열쇠가 없다」가 아니라 코드였다 (2026-08-14 발견 → 같은 날 닫힘)

> ⚠️ **이 항목의 1차 진단(「실서비스에 열쇠가 안 들어가 있다 / 회색 상자는 의도된 대체 화면」)은 틀렸다.**
> 같은 날 PO PC 세션이 실서비스 화면에서 다시 재서 뒤집었다. 원문은 지우고 정정본만 남긴다.

**진짜로 무엇이 고장나 있었나**: 병원 상세·시술 상세의 「위치」 칸이 **아무것도 없는 빈 상자**였다.
회색 대체 화면(핀 아이콘 + 병원 이름)이 아니라 **완전한 공백**이었고, 이 차이가 오진의 갈림길이었다.

**실측 (2026-08-14, PO PC 크롬 + 실서비스 화면)**
- 열쇠는 **2026-03-11 부터 버셀에 이미 있었다** — `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`, production·preview·development 전부.
- 실서비스 화면이 `maps.googleapis.com` 을 **정상 호출**하고 있었고 `google.maps.Map` 까지 살아 있었다.
- 같은 화면에서 지도를 네 조합으로 만들어 비교(3회 재현, 콘솔 오류 0건):

  | 만든 옵션 | 결과 |
  |---|---|
  | `{UI만}` ← 고치기 전 코드 | 빈 상자 (내용 83자) |
  | `{UI + styles}` | 빈 상자 (83자) |
  | **`{center + zoom + UI}`** | **정상 (457자)** |
  | `{center + zoom + UI + styles}` | 빈 상자 (83자) |

**원인 2개** (둘 다 신청서 [#1372](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/1372) 에서 고침)
1. `center`·`zoom` 이 `options` **밖**에 있었다. `@react-google-maps/api` 는 지도를 만들 때 `options` 만 넘긴다(`dist/esm.js` 의 `getInstance`). 중심 없이 태어난 지도는 나중에 좌표를 넣어도 안 살아난다.
2. `styles`(옛날식 지도 색칠)를 넣고 있었다. 구글이 접는 중이라 넣으면 지도가 통째로 죽는다.
