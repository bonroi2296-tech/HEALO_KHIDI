...
## 🔴 정부지원금 전용카드가 해외결제·정기결제를 거절 — KHIDI 정산 증빙 위험 (2026-07-24 실측, 2026-07-28 회수)

> 🛟 **회수 경위**: 이 기록은 `docs/handoff-db-outage-diagnosis` 브랜치의 중간저장 블록에만 있었고 **PR 이 없어 본판에 한 번도 안 들어왔다.** 낡은 브랜치 7개를 지우기 전 전수 대조하다 발견해 옮긴다(어제 [#1058](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/1058)·[#1059](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/1059) 와 같은 부류, 3번째).

- **증상(2026-07-24 실측)**: 사업비 **전용카드 IBK기업은행 BC corporate(gosims)** 로 **Apple**(「인증이 실패했습니다」)·**LiveKit/Stripe**(「해당 신용카드는 이러한 유형의 구매를 지원하지 않습니다」) **둘 다 거절**. **발급사 거절이라 가맹점 쪽에선 못 고친다.**
- **대응 경로(미착수)**: 카드사에 **①방금 거절 사유 조회 ②해외결제 개방 ③정기(구독)결제 개방** 요청. 안 풀리면 (a)**자기비용 선집행 후 대체**(관리지침 33p) 또는 (b)진흥원 문의 — 공문 문구가 필요하면 어시가 작성한다.
- 🔴 **정산 증빙 위험이 실제로 발생했을 수 있다 (두 기록을 이어보면 드러남 — 지금까지 서로 다른 곳에 흩어져 있었다)**:
  - 2026-07-24 기록: *"구글 $25가 어느 카드로 결제됐는지 확인 필요 — **전용카드가 아니면 정산 증빙에서 문제됨**"*
  - 2026-07-27 실측([보관 핸드오프](archive/PROJECT_CONTEXT_handoffs.md)): 구글 $25 = **비씨 ••••0953, 「강주영」 개인 결제 프로필**(거래ID `PDS.5744-4231-0107-77303`)
  - → **전용카드가 아니었다.** 애플 $99 도 KG이니시스 영수증 보유이나 결제수단이 전용카드인지는 미확인. **PO 확인 필요.**
- **결제 현황 정정(2026-07-28 실측 — 영수증 대조)**: **Supabase = Pro · Vercel = Pro · Apple $99 결제완료 · LiveKit = Ship 구독 완료**(영수증 `2042-2471`, 7/28~8/1 일할 US$6.36, 8/1부터 $50/월). → 「LiveKit 유료 전환」 숙제는 **종결**. 남은 건 Gemini spend cap.
- ⚠️ **카드가 죽은 게 아니라 「해외 정기결제」만 막힌 패턴이다** — 같은 0953 카드로 구글 **일회성** 결제는 성공했다. 카드사에 요청할 때 이 구분을 말할 것.
- 🔴 **새로 드러난 것(2026-07-28)**: LiveKit Ship 영수증의 결제수단이 **Mastercard ••••0953** — 구글 $25 를 결제했던(=「강주영」 개인 결제 프로필로 확인된) **그 카드와 끝 4자리가 같다.** 두 갈래 중 하나이고, 어느 쪽이냐로 대응이 갈린다:
  - (A) 0953 = **개인카드** → 매달 $50 이 개인카드로 빠진다. 전용카드 정산 증빙 오염이 **매월 누적**되는 구조 → 결제수단 교체 or 자기비용 선집행 처리(관리지침 33p)를 지금 결정해야 한다.
  - (B) 0953 = **전용카드**(IBK BC corporate)인데 정기결제가 이번엔 통과 → 7/24 의 「거절」 이슈가 해소된 것이므로 이 항목 자체를 닫으면 된다.
  - → **PO 만 아는 정보(카드 실물). 확인 전까지 어느 쪽으로도 단정하지 말 것.**
