...
- 🆕 **2026-07-30 — 첫 실측 시도 결과: 「안 되고 있었다」.** PO 가 `?ga_debug=1` 로 열어 보낸 콘솔에
  ...
  - **500 의 원인은 우리 코드가 아니다**(실측: gtag.js 10/10 200 · CSP 정상 · `/ko?ga_debug=1` 리다이렉트 없음).
    남은 후보는 **구글 일시 오류** 또는 **PO 브라우저 확장**(콘솔에 우리 것 아닌 `dev-tools-inject.js`(`rhwp`) 있었음)
    — ⚠️ 재현 못 했으므로 **둘 다 가설**이다.
  - **그래도 우리 몫 2개를 고쳤다**: ①로드 실패 시 **2.5초 뒤 1회 재시도**(쌓인 이벤트까지 복구)
    ②`?ga_debug=1` 에 **화면 자가진단 상자** → DevTools 없이 판정. 상세 = 반성문 #161, `GA4_SETUP.md` §2.
  - **아직 «검증됨» 아님.** 남은 것: 배포 후 **확장 없는 시크릿창**에서 자가진단 6줄 ✅ + DebugView 에 이벤트 확인.
