...
**⚠️ 그대로 합치지 마라.** 2주 넘은 작업본이고, AI 품질 지표는 KHIDI 평가와 닿아 있다. 2026-08-24 에 품질 점수 관련 다른 조치가 있었으므로(그 이전 값은 오염으로 기록돼 있다) **어긋나지 않는지 확인한 뒤에** 합쳐야 한다. 검사를 돌려 보고 판단하라.

**왜 중요한가**: 지금 쌓이는 품질 점수는 **RAG 를 안 탄 값**이다. 즉 실서비스가 실제로 내는 답과 다른 것을 재고 있다.
