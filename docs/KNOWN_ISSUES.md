...
## 🔴 원격 작업본 51개의 상태를 아무도 모른다 — 「12개 봤다」가 전부가 아니었다 (2026-09-05 발견)

**무엇**: `main`·`production` 을 뺀 원격 작업본이 **51개**다. 2026-09-05 세션이 그중 12개를 훑고 「11개는 본판에 반영됨, 1개만 진짜 방치」로 판정했는데, **전수를 뽑아 보니 51개였다.** 나머지 39개는 아무도 안 봤다.

**🛑 「앞선 커밋 수」로 판정하지 마라.** 이 저장소는 squash 로 합치므로 본판에 이미 들어간 작업본도 `git rev-list --count origin/main..<작업본>` 이 크게 나온다. 마찬가지로 **「본판과 다른 파일 수」도 신호가 아니다** — 본판이 그 뒤로 그 파일을 고쳤을 때도 다르게 나오므로 오래된 작업본은 무조건 걸린다.

**되는 판정법**: 그 작업본이 «추가한 줄»이 본판에 실제로 있는지 센다.

```bash
base=$(git merge-base origin/main origin/<작업본>)
git diff $base origin/<작업본> -- '*.ts' '*.js' '*.md' | grep '^+' | grep -v '^+++' |
  while IFS= read -r l; do git grep -qF -- "${l#+}" origin/main || echo "미반영: $l"; done
```

**실측 (2026-09-05, 9월에 손댄 것 7개 중 6개 — 나머지는 시간이 모자라 못 쟀다)**

| 작업본 | 추가한 줄 | 본판에 없는 줄 |
|---|---|---|
| `claude/healwith-geo-aio-seo-audit-rrbqj9` | 124 | **103 (83%)** |
| `docs/expense-receipts-index-0904` | 258 | 79 (31%) |
| `docs/handoff-0901` | 154 | 30 |
| `feat/upload-audio-text` | 251 | 30 |
| `docs/inquiry-alert-outage-0904` | 80 | 10 |
| `fix/chat-promote-needs-contact` | 88 | **0 — 전부 반영됨** |

⚠️ **「본판에 없는 줄」이 곧 「살려야 할 것」은 아니다.** 중간 초안·버려진 시도일 수 있다. 다만 **아무도 안 봤다는 것**이 문제다.

**왜 중요한가**: 작업본이 사라지면 코드는 자동 저장으로 남아도 **「왜 그렇게 했는지」는 통째로 증발한다.** SEO 감사 작업본은 추가한 줄의 83%가 본판에 없다 — 그 조사를 다음 사람이 처음부터 다시 하게 된다.

**다음에 할 일**: 나머지 45개를 위 방법으로 재고, ①살릴 것 ②버릴 것 ③이미 반영된 것으로 가른다. 지우는 것은 그 판정 뒤에.
