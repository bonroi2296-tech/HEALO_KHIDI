...
**같은 밤 세 번째 — 대표 제휴 병원 페이지가 러시아어로 3/4 영어였다**: `/hospitals/immune`(사이트맵 우선순위 0.85, 러시아어 랜딩이 «Клиника Immune» 으로 가리키는 바로 그 페이지)의 본문 데이터(`immuneHospitalInfo.js`) 183칸 중 **ru 170·kz/zh/ja 177칸이 비어** 있었다 — 로컬 실측 본문 키릴 비율 **23%**. 645문장을 채워 **77%**(남은 라틴 339자는 브랜드·Wi-Fi 같은 고정 표기). 치료법 이름 15개는 `immuneTherapies.js` 정본을 참조하게 바꿔 두 파일 표기가 갈리지 않게 했다. `check:cancer-i18n` 이 이 파일 잎 전부를(천장 0), `check:i18n-quality` 가 같은 그물로 본다(용어집 위반 4건은 그 자리에서 고침). ⚠️ 의료진 28명 이름 음차(러·카)·로마자(중)·가타카나(일)는 특히 원어민 검수 대상.
