...
## 🟡 병원 지도가 실서비스에서 «조용히» 꺼져 있다 (2026-08-14 발견)

**무엇**: 병원 상세 화면에 구글 지도가 붙어 있는데(`app/hospitals/[slug]/HospitalDetailLegacyClient.jsx`
→ `src/components/GoogleMap.jsx`), **실서비스에 열쇠가 안 들어가 있어** 지도 대신
「회색 위치 상자」(위치 아이콘 + 병원 이름 + 주소 «글자»)만 나온다.
환자는 병원이 서울 어디쯤인지 **지도로 못 본다**.

**실측(2026-08-14)**
- 실서비스 화면 4개(첫 화면·병원 목록·병원 상세·치료 목록) + 병원 상세가 부르는
  자바스크립트 조각 **9개 전수** 훑기 → 열쇠 문자열 **0건**
- `maps.googleapis.com` 호출 **0건**
- 코드상 열쇠가 없으면 `LocationFallback`(회색 상자)로 대체 — **고장이 아니라 의도된 대체 화면**

**왜 아무도 몰랐나 (진짜 원인)**
`scripts/check-env.js` 가 **엉뚱한 이름**을 보고 있었다 — 코드는 `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`
를 읽는데 검사기는 앞머리 없는 `GOOGLE_MAPS_API_KEY` 를 「선택」 항목으로 갖고 있었다.
→ 진짜 열쇠가 비어 있어도 검사가 **아무 말을 안 했다**. ✅ **이건 2026-08-14 고쳤다**(이름 교정 + 설명).

**남은 것 — 어시 권한 밖**
| 무엇 | 왜 어시가 못 하나 |
|---|---|
| ① 구글 클라우드 콘솔에서 **사용처 제한** 걸기 (https://console.cloud.google.com/apis/credentials) | 구글 계정 로그인 화면. 클라우드 세션엔 PO 계정이 없다 |
| ② 버셀에 `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` 넣기 | 버셀 계정 설정 — 어시 도구에 환경변수 «쓰기»가 없다 |

⚠️ **순서를 지킬 것: ①제한 먼저 → ②열쇠 넣기.** 이 값은 브라우저로 그대로 나가는 공개 값이라,
제한 없이 켜면 **남이 우리 열쇠로 지도를 쓰고 우리에게 청구**된다.

**안 켜도 되나**: 된다. 주소 글자는 나오고 서비스는 정상이다. 다만 「띄우려고 만든 기능이 꺼져 있는」 상태다.
**재검토: 2026-09-15** · PO 손 필요(로컬 세션이면 그 자리에서 가능)
