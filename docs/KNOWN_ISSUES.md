...
## 🔸 에이전시·의료기관 포털의 언어 선택은 **아직 안 재봤다** (2026-07-29, 반성문 #156 곁가지)

- **확인된 것(실측)**: 공개 사이트는 **정상**이다 — 러시아어를 고르면 주소가 `/en` → `/ru` 로 옮겨가고 쿠키 `healo_lang=ru` 가 저장된다. 단 그건 **서버가 심어 준 것**이지 `setLangCookie()` 덕이 아니다(그 함수는 브라우저에서 항상 조기 return 이었다 — 사전이 빈 껍데기라서).
- **못 잰 것**: 에이전시(`/partner`)·의료기관 포털은 **주소 이동이 없어** `setLangCookie()` 가 유일한 저장 경로다 → 그쪽 언어 버튼은 안 먹었을 가능성이 높다. 이 환경에 에이전시 테스트 계정 자격(`E2E_AGENCY_EMAIL` 등)이 없어 **로그인 자체를 못 해 확인하지 못했다.**
- **오늘 한 조치**: `setLangCookie()` 도 `isKnownLangCode` 로 맞췄다(백오피스 두 함수와 같은 수리). 공개 동작은 그대로고, 포털 쪽이 병이었다면 같이 낫는다.
- **다음에 할 일**: 에이전시 계정으로 `/partner` 를 열어 언어 버튼 → **화면 글자가 실제로 바뀌는지** 1회 측정. 안 바뀌면 그 포털이 쓰는 사전(로컬 TR) 쪽을 따로 봐야 한다.

## 🔸 변경 이력의 「어디인가」 세부 위치(예: 「통계 / 항목1 · 문구」)는 아직 한국어 (2026-07-29)

- 화면 이름(「홈 화면」·「문의폼」)과 비고는 6개 언어로 번역했지만, 그 옆의 **세부 자리 이름**은 `HOME_CONTENT_REGISTRY` 의 한국어 라벨을 그대로 쓴다(홈 문구 38개).
- 지금은 화면 이름·「화면 열기」 링크만으로도 찾아갈 수 있어 급하지 않다. 번역하려면 레지스트리 라벨을 6개 언어로 바꿔야 한다(별도 작업).
