...
## ✅ ~~사전상담 실적 43건 — PO 가 직접 정리하기로~~ → **정리할 것이 없다 (2026-08-18 재측정으로 종결)**

> **2026-08-18 정정.** 8/15 실측이 `is_test` 를 안 걸렀다. 다시 재보니 사전상담 방 **105개 중 시험 방이 104개**이고,
> 실제 상담은 **1건**(문의 #60, 08-03 18:18, 통화 21시간대 방)뿐이다. 「통화 흔적 43건」의 정체는
> **매일 04~05시(KST) 도는 야간 자동 검사 방 + 7/28 시험 일괄** 이며 전부 이미 `is_test=true` 로 분류돼 있다.
> 즉 **PO 가 「어느 방이 진짜 환자인가」를 표시해도 늘어날 실적이 없다** — 이 작업은 하지 마라.
...
> (`09. 중간평가\사전상담_실적명세_초안.md` 참조).

<details><summary>아래는 2026-08-15 당시 기록 (보존)</summary>
