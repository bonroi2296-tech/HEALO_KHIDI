...
## 🔸 콘텐츠 편집기에 **「저장 전 미리보기」가 없다** — 한 줄 밀린 수정을 아무도 못 잡는다 (2026-07-28, 반성문 #139에서 도출)

- **무엇**: 편집기는 「N곳 변경됨」 개수만 보여주고 저장한다. **무슨 키를 무엇에서 무엇으로 바꾸는지**를 저장 직전에 확인시키지 않는다.
- **왜 문제**: 값이 한 칸씩 밀려 들어가는 사고(#136 코디 붙여넣기, #139 내 자동화)는 **값들이 서로 겹치지 않아** 이번에 넣은 「문구 중복」 배지가 **원리상 못 잡는다**. 두 사고 다 저장 후 DB를 봐야 발견됐다.
- **고친다면**: 저장 버튼을 누르면 «키 · 이전값 → 새값» 목록을 띄우고 확인받기(변경 이력과 같은 색 표기 재사용). 백로그 — 편집 빈도가 늘면 우선순위 올릴 것.
