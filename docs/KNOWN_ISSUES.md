...
- **적용된 바 5곳**: 공개 모바일 하단내비(`src/components.jsx`) · 병원 상세 모바일 CTA · 치료 상세 모바일 CTA · 환자 포털 하단내비/서랍(`app/patient/layout.jsx`) · 코디 콘텐츠 편집기 저장바.
- ✅ **새로 하단 고정 UI를 만들 땐 `bottom-0` 대신 이 변수를 써라.** (동의 UI를 가리는 방식은 피했다 — 배너는 그대로 보이고 다른 바가 비킨다.)
- **실측**: 375px 치료 상세 = 배너 199px, 하단바 bottom 501/700 · 1000px 코디 편집기 = 배너 130px, 저장바 bottom 590/720 — 두 경우 다 `가려짐 false`(`elementFromPoint` 가 바 자신을 반환).
