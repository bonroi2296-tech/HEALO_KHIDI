...
1. **초대 링크가 너무 길다** — 지금 `healwith.co.kr/consultation/<36자>?invite=<64자>` = 약 139자. → 짧은 넘김주소 `healwith.co.kr/c/xxxxxx` 하나 추가(상담방 화면은 안 건드림). ⚠️ 열쇠(토큰)는 해시로만 저장돼 원본이 없으니 넘김주소 표를 따로 둬야 한다. 만드는 자리는 `src/lib/auth/guestToken.ts:94` 한 곳.
...
**지금 가능** — 4건 다 어시 손으로 오늘 됨(PO 손·남의 회신 불필요).
