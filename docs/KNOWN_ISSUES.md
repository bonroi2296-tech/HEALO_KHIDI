...
### 아직 못 짚은 한 단계
새 `referral/route.ts` **236행에 `after(() => sendAdminNotification(...))` 호출이 «있다».**
있는데 안 돈다. 실제로 요청을 넣어봐야 그 안쪽이 보이는데, **시험 문의가 실적 숫자에 섞이는 위험**이 있어
이 세션에서는 하지 않았다.

⚠️ **왜 16일간 아무도 몰랐나 = 실패가 어디에도 안 남는 구조다.**
`sendAdminNotification(...).catch(() => {})` (호출부) + 함수 내부 fail-safe `try/catch` **이중**이라
터져도 로그·알림·화면 어디에도 흔적이 없다. **고칠 때 이 구조부터 손대야 같은 일이 재발하지 않는다.**
