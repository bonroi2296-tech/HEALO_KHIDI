...
**같은 낮 네 번째(2026-09-06) — 제휴 병원 8곳 주소가 러·카·중·일에서 영어였고, 카자흐어 메일은 «kz/kk» 표기 불일치로 엉뚱한 언어로 나갈 구조였다**: `partnerHospitals.js` 의 `address` 8칸이 ko·en 뿐이라 병원 상세의 주소 줄이 4개 언어에서 영어로 폴백 → ru·kz·zh·ja 32문장 채움(성동구 1곳은 원문이 「구」까지라 그대로 «구»까지만 — 없는 주소를 지어내지 않음). 같은 파일의 카자흐어 본문에서 용어집이 금하는 «пациент*»(러시아어 차용) 7곳을 «науқас» 굴절형으로 교정. **메일(덫 제거, 실제 누수는 아님)**: 교육 메일(`educationContent.ts`)·상담 리마인더(`consultationReminder.ts`) 템플릿 키가 BCP47 `kk` 뿐이라 부르는 쪽이 내부 코드 `kz` 를 `kk` 로 바꿔 넘겨야만 카자흐어였다 — 기존 호출부 2곳(설문 크론의 `normalizeSurveyLang`·리마인더 크론)은 매핑이 있어 **지금 새지는 않았고**, 매핑을 빠뜨리는 새 호출부가 생기면 교육 메일은 러시아어·리마인더는 한국어로 조용히 떨어지는 구조였다(POSTMORTEMS #23 이 그 사고). 두 템플릿이 **kz·kk 둘 다** 받고 모르는 값은 ko 로 내리게 하고 시험(`kazakhAlias.test.ts`)으로 고정. 🔑 **파생 파손을 내가 만들 뻔했다**: 리마인더 크론의 kz→kk 매핑을 빼면서 그 아래 화이트리스트(`kk` 만 있고 `kz` 없음)를 안 봐 kz 가 ko 로 떨어지게 됐다 — 내 diff 를 다시 읽다 잡아 화이트리스트를 지우고(정규화는 템플릿 몫) 시험을 보탰다. 실DB 카자흐어 환자 0명. **검사기**: `check:cancer-i18n`(천장 0)·`check:i18n-quality` 그물에 제휴 병원 8곳(name·type·description·address·specialties·highlights)을 추가 — 앞으로 주소·강점 칸이 비면 CI 가 막는다. ⚠️ 주소 음차(러·카)·한자 표기(중)·가타카나(일)는 원어민 검수 대상.
