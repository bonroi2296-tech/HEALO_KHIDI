...
## [TASK-20260421-01] i18n ru / kz 번역 채우기
...
## [TASK-20260421-02] /telemedicine 페이지 다국어화
...
## [TASK-20260421-04] 환자 대시보드 /patient 페이지 카피 러시아어 번역

### 배경
환자 포털 UI 가 현재 한국어 / 영어만. 실제 사용자 대부분 러시아어 사용자.

### 담당 영역
- 수정 가능: `app/patient/**/*.jsx` 의 텍스트 부분
- 건드리면 안 됨: 관리자 페이지, 상담방, API

### 상세 단계
1. `app/patient/` 하위 파일 모두 읽고 하드코딩된 한국어/영어 문자열 찾기
2. useLang() 훅 이미 있는지 확인
3. 없으면 COPY 객체 추가 + `useLang()` 로 분기
4. ru 번역 추가
5. 기존 en / ko 는 유지

### 검증
- [ ] 각 /patient 페이지에서 `?lang=ru` 로 전환 시 번역 출력
- [ ] `npx next build --webpack` 성공
