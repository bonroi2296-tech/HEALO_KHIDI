...
- **감시 공백 실측**: ①`/api/health`(DB까지 실측하는 정직한 헬스)가 있는데 **아무도 폴링 안 함** — 12시간 로그에 호출 2건(전부 이 조사 중 수동) = 업타임 모니터 미연결. ②Vercel `runtime_errors` 도 0 — 503은 "처리된 응답"이라 에러로 안 잡힘. 장애가 나도 사람이 우연히 발견하기 전까지 아무 경보가 없는 구조 → **후속: /api/health 외부 업타임 모니터 연결(무료: UptimeRobot/BetterStack 류) = PO 결정 1건**.
