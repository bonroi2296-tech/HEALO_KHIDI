...
7. **`head`로 자른 검색 결과를 "없음"으로 읽지 않는다.** 이 수리 중에 `source_id`를 "되읽는 곳이 전혀 없다"고 판단해 코드에서 지웠는데, **`| head`에 잘려 안 보였을 뿐** 실제로는 `job-review.ts`가 읽어 `hospitals.data_source`를 채우고 있었다. 지웠다면 다음 크롤에서 같은 병원이 중복 생성되고 폐업 감지에서 빠졌을 것이다(독립 리뷰가 잡음). 존재 여부를 결론내는 검색은 **개수를 세거나 자르지 말고 전부 봐라.**
