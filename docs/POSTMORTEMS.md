...
- **재발 방지**:
  ...
  - ✅ **그래서 「저장 전 미리보기」를 같은 날 만들었다**(PO 지시): 저장 버튼 → «키 · 언어 · 이전값 → 새값» 목록 확인 후 「이대로 저장」. #136·#139 두 사고 모두 **저장 직전에 사람 눈에 걸렸을** 형태다(«intakeLabels.cancer.stomach: Асқазан → Liver cancer» 가 한 줄로 보인다). 색 표기는 변경 이력과 통일.
