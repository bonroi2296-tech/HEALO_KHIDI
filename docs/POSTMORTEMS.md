...
- **재발 방지**:
  ...
  - **남은 리스크(정직)**: 이번에 만든 「문구 중복」 배지는 이 사고를 **못 잡는다** — 값이 한 칸씩 밀리면 서로 겹치지 않아 전부 «중복 아님»이다. 사람이 붙여넣다 한 줄 밀리는 경우도 마찬가지. 진짜 방어는 «저장 전 무엇을 무엇으로 바꾸는지 확인시키기»(저장 미리보기)인데 아직 없다 — `docs/KNOWN_ISSUES.md` 백로그.
