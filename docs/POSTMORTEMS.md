...
## #116 — PR 스모크 E2E 간헐 실패: 테스트마다 UI 로그인 → PR 폭주 시간대에 **공유 Supabase(프로덕션 겸용) 포화** — 매번 다른 테스트가 떨어지는 복권 (2026-07-24, 06~07시 UTC 실패 3건 실측 규명)
...
> - **교훈(부류)**: 상관(동시 실행 수)과 기전(trace 지연)을 둘 다 잡았어도 **"그 부하가 없을 때도 같은 일이 나는가"** 를 안 물으면 오진이다. #116 은 이 반증 질문을 *수리를 다 하고 PR 을 올린 뒤에야* 던졌다.
