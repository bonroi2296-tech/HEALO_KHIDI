...
**재발 방지 (제안·PO 결정 대기)**
- **구조화 + 프레시니스 CI 가드**: 국가/정책별로 `{ lastVerified, expiresOn?, sourceUrl }` 구조 필드를 두고, `check-visa-freshness.mjs`를 추가해 **오늘 > expiresOn 이거나 lastVerified가 ~180일 초과면 CI 실패**. 그러면 "6월 지나면 CI가 대신 빨간불" → PO가 UI에서 찾을 일이 사라짐(코드베이스의 "기계가 오류 잡는다" 원칙과 정합).
- **커버리지 갭**: CIS 타겟인데 우즈베키스탄·키르기스스탄·타지키스탄·아제르바이잔이 데이터에 없어 일반 영어 폴백으로 빠짐(전부 비자필요·K-ETA 비대상). 최소 우즈벡 추가 권장.
