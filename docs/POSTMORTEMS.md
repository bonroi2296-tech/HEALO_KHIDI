...
**재발 방지**
- **신규 가드(실물 검증)**: `e2e/sitemap-health.spec.ts` — 사이트맵의 모든 `<loc>`을 대상 서버에 실제 요청해 400+ 이면 실패. Production Nightly E2E(cron)에 자동 편입 = **매일 밤 프로덕션 실물로 "사이트맵에 있다=열린다" 불변식 확인**. 가드 자가검증: 수리 전 프로덕션 상대로 돌려 유령 6건 정확히 적발 확인.
