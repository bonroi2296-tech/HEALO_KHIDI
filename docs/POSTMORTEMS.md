...
## #150 — 말투 규칙을 **매 턴 다시 띄우면서도 어겼다** — 알려주기만 하고 «어겼는지»는 아무도 안 재고 있었다 (2026-07-28, PO 가 같은 세션에서 세 번 지적 → **"규칙 그렇게 해놔도 오늘 쉽게 설명하라고 한 것도 안 지키잖아"**)
...
- **행동 규칙을 새로 만들 때 「이걸 어떻게 잴 건가」를 같이 정하라.** 못 재는 규칙은 PO 의 눈을 검사기로 쓰겠다는 뜻이고, 그건 PO 에게 일을 떠넘기는 것이다(#149 와 같은 부류 — **기계가 할 수 있는 걸 사람 몫으로 두지 마라**).
- **PO 가 "진짜야?" 라고 물으면 약속하지 말고 그 자리에서 만들어 실증하라.**
- 부수 수리: 검사기를 다른 파일에서 가져다 쓰면 표준입력을 기다리며 멈추던 것(2분 정지) — 「직접 실행할 때만 본체를 돈다」로 수정.

## #149 — 병렬 세션 사고를 막으라고 만든 경보 2개가 **정작 병렬 세션을 제일 많이 돌리는 환경에서 한 번도 안 떴다** — 그래서 「배포 창구」 하나를 브랜치 6개가 각자 고쳤다 (2026-07-28, PO 가 **"내가 빠가사리라서 그렇게 지시를 못 한다는 거잖아"** 라고 해서 착수)
