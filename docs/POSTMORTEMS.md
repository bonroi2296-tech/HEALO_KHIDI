...
## #29 — 계층 백오피스 점검: 안쪽 포털(환자·코디)에 기능 구멍 3개 (2026-06-23)

**무슨 일**
계층별 백오피스 전수 점검에서 바깥(병원·해외파트너)은 건강한데 안쪽에 3개 결함:
1. **환자 증상기록이 본인과 연결 안 됨** — `symptom_reports`에 작성자(환자) 컬럼 자체가 없어, 포털 환자가 입력해도 고아 레코드로 저장 → 본인 대시보드/차트에 영영 안 뜸. premium 화면은 RLS(service_role 전용)로 직접쿼리 0행 → 항상 빈 화면.
2. **환자 재진예약 확정/무시 버튼 무동작** — `/api/khidi/consultation/[id]` PATCH가 doctor/coordinator/admin 전용이라 환자는 403. 에러는 catch로 삼켜지고 UI는 낙관적으로 행을 지워 "되는 것처럼" 보이나 DB는 그대로.
3. **코디 메뉴가 실제 화면과 어긋남** — NAV가 없는 라우트(`/coordinator/patients`·`/kpi`)를 가리켜 404, 정작 핵심화면(cases·inbox·messages·visa·cost-estimates)은 메뉴에 없음.

**왜 못 잡았나 (근본원인)**
- 빌드·타입검사는 통과하는 "런타임/데이터 흐름" 결함 — 버튼이 그려지고 fetch도 나가지만 권한/스키마에서 막힘. 정적 검사로 안 잡힘.
- 두 모델(피벗 전 consultation_sessions / 피벗 후 cases 허브)이 한 포털에 섞이며 NAV가 옛 모델에 멈춤.
- 낙관적 UI 갱신이 실패(403)를 가려서 "동작하는 것처럼" 보임 → PO도 모름.

**어떻게 고쳤나**
- ①`symptom_reports.patient_user_id` 컬럼 추가(가역 마이그레이션) + POST 저장 시 기록 + GET `?mine=true`(본인만) + 레거시 증상화면이 본인 기록을 불러와 표시(죽은 "이전 보고" 토글도 동시 복구).
- ②환자 전용 `/api/khidi/rebooking/[id]` 신설(requireConsultationAccess=본인 소유 통과, 재진예약 행만). 클라는 성공 시에만 UI 갱신(실패를 안 삼킴).
- ③코디 NAV를 실제 라우트로 교체, 죽은 링크 제거.

**재발 방지 (시스템 적용)**
- 교훈: **포털 PATCH 버튼은 "환자 롤이 그 엔드포인트를 통과하는가"를 반드시 확인**한다. 낙관적 UI 갱신은 `res.ok && data.ok`일 때만(실패를 삼키지 말 것).
- 교훈: **사용자별 데이터 테이블은 작성자 식별 컬럼(`*_user_id`)을 처음부터** — 없으면 본인 조회·RLS가 불가능.
- 후속(미적용): premium 증상화면들의 브라우저 직접쿼리도 `?mine=true` API로 교체 필요(현재 비기본 모드). 코디 NAV↔라우트 정합은 자동 검사로 묶을 여지.
