...
- **가드 3중 보강**:
  1. `[인라인사전]` 정규식을 `const (L|COPY|T|DICT|TEXTS?|LABELS) = {` 로 확장 — 실제 우회에 쓰인 이름을 전부 포함.
  2. `INLINE_L_ALLOW` **면제 목록을 비웠다**(전면 차단). 존량 0이 됐으므로 예외가 필요 없다.
  3. **`[유령키]` 가드 신설**: 공개 파일이 부르는 정적 `t("키")` 가 사전에 없으면 CI 실패. `t()` 는 없는 키를 조용히 그대로 렌더해 **빌드·타입·테스트 어디도 안 잡는다**(그 화면을 연 사람만 안다). 심자마자 작업 중이던 파일의 미삽입 키 10건을 실제로 잡았다.
...
- **문구를 옮기는 작업은 "옮겼다"가 아니라 "원본과 같다"를 증명해야 한다.** 대량 이관에서 사람·에이전트가 값을 미묘하게 다듬는 일은 반드시 생긴다(이번 12건). 옮긴 뒤 **원본 AST 값과 전수 대조**하고, 불일치는 하나하나 "왜 다른가"를 답할 것 — 개수가 아니라 목록으로 확인한다. 이스케이프 표기(`\'`·`\"`)나 템플릿 리터럴 때문에 단순 문자열 검색은 오탐이 나므로 **AST 파싱이 기준**이다.
