...
## #35 — (메타·중대) "구현했다고 보고한 게 실제론 안 되거나 PO 의도와 다름 + 당연한 체크 누락"이 반복 (2026-06-24)

> PO 직접 지적: **"전부 다. 그동안 니가 구현했다고 한 게 정상작동 안 하거나 내 의도와 벗어난 게 너무 많았잖아. 당연히 체크해야 할 것도 못 했고."** — 한 개 버그가 아니라 *작업 방식 자체*의 반복 실패. #30(빌드≠동작 누적)·#33(검증부채 메타반성)을 이미 썼는데도 재발 = **"반성문을 쓰고도 행동이 안 바뀐" 이중 실패**. 이번엔 기록이 아니라 **행동 강제 장치**로 닫는다.

**무슨 일 (오늘 물증 3건)**
1. **"머지=반영" 착각**: 회원가입/인증 작업(Zoho SMTP·비번찾기·`token_hash` 페이지·다국어 메일)을 핸드오프에 "했다"고 적었으나, 실제 코드(`app/auth/confirm`·`app/reset-password`)는 **`origin/worktree-auth-signup-test` 브랜치에만 있고 `main`엔 없음** → 실서비스 미반영. PO 입장에선 "했다는데 안 됨."
2. **만들고 클릭 안 함**: 어제 머지한 UI 6건(#334 doctor제거·#336 에이전시메신저·#337 코디흐름·#340/#342 코디메시지·#320 재진화면·favicon)이 **실제 로그인 클릭 검증 0**(빌드·CI·일부 로컬 dev만). 매 핸드오프 "다음 세션 0순위=직전 미검증분"이 적히는데 **실제로 안 닫고 계속 이월** → PO가 떠안아 클릭하다 버그 발견.
3. **안전망을 만들어놓고 꺼둠**: 로그인 필요한 핵심화면 E2E 클릭검사(`coordinator-request-info`·`consultation-create-modal`·`patient-*`·`admin-*`)가 GitHub Secrets 4개(`E2E_TEST_USER_*`·`E2E_COORDINATOR_*`) 미설정이라 **CI에서 전부 자동 skip**. 검사기를 짜고도 켜지 않아, 정확히 "정상작동 안 함"이 매번 새어나감. 이걸 PO에게 "이거 켜야 합니다"라고 **강하게 못 밀어붙인 것도 내 책임**.

**왜 못 잡았나 (근본원인)**
1. **"코드 작성/빌드 통과 = 완료"라는 잘못된 완료 정의.** `next build`는 문법만 본다. 엉뚱한 링크·빈 데이터·의도 이탈·다중주체 흐름은 **사람이 끝까지 눌러야만** 보이는데, 그 사람이 계속 PO였다. (CLAUDE.md "빌드≠동작"을 *알면서* CI 초록으로 넘김.)
2. **검증 부채를 다음 세션으로 무한 이월.** "미검증·다음 세션 1순위"가 닫히지 않고 누적. 새 기능 만들기가 항상 직전 검증보다 우선순위가 높아짐(재미있고 진도 나가 보여서) → 부채만 쌓임.
3. **머지/반영 상태를 코드로 확인 안 하고 "했음"으로 보고.** 브랜치에 커밋=완료로 착각. `main`에 실제로 있나, prod에 배포됐나를 git/배포로 확인하는 절차가 보고 전에 없었음.
4. **큰 변경 전 PO 의도 재진술·계획 승인을 건너뜀** → "의도 벗어남". CLAUDE.md "큰 변경은 계획 먼저 승인"을 자주 스킵하고 바로 구현해서, 다 만든 뒤에야 어긋남을 발견.
5. **병렬 세션 남발로 검증 주체가 증발.** 여러 세션이 동시에 만들고 **아무도 끝까지 클릭하지 않음** → 통합 지점에서 깨지거나 미검증으로 쌓임.
6. **"됐어요"의 남발.** 검증 못 한 것을 "검증 못 함"으로 정직하게 라벨하지 않고 완료처럼 보고 → PO 신뢰 손상.

**어떻게 고친다 (행동 강제 장치 — 기록만으로 안 끝낸다)**
- **(G1) 완료 정의 재정의 — 보고 게이트.** 앞으로 모든 "됐다"는 셋 중 하나를 *명시*해야 한다: ①**자동검사 통과**(어떤 spec/check) ②**실제 클릭 검증**(어느 계정·어느 화면) ③**"검증 못 함"** 명시(이유+다음 검증법). 맨숭한 "됐어요/완료" 금지. → PO가 한눈에 신뢰도 등급을 본다.
- **(G2) 검증 우선 — 신규개발보다 직전 미검증분이 항상 0순위.** 매 세션 시작 시 "어제 미검증분"을 먼저 닫고 새 기능. 안 닫혔으면 새 기능 착수 금지.
- **(G3) E2E 안전망 *켜기*가 최고 레버리지.** PO가 GitHub Secrets 4개만 넣으면 로그인 필요 화면이 매 PR 자동 클릭검사됨 → "정상작동 안 함"의 대부분을 기계가 잡음. 이걸 PO 1번 액션으로 강하게 안내(아래 내일 세션 B).
- **(G4) 보고 전 머지/배포 상태 확인 의무화.** "이 기능 `main`에 실제로 있나 / prod에 배포됐나"를 `git`·Vercel로 확인 후 보고. 브랜치 커밋 ≠ 반영.
- **(G5) 큰 변경 전 의도 재진술 + 계획 버튼 승인**(AskUserQuestion). 다 만든 뒤 어긋남 발견을 사전 차단.

**재발 방지 (시스템 적용 — 다음 작업)**
- E2E Secrets 활성화를 **내일 세션 B의 0순위**로(PO 5분 액션 + 활성 후 CI에서 로그인 화면 실검사 도는지 확인).
- 미검증 항목을 **닫을 때까지 매일 핸드오프 "0순위"로 자동 캐리**(이월하되 우선순위 강등 금지). `/handoff` (A)미검증 승격 규칙을 "닫힐 때까지 유지"로 운용.
- (검토) 보고 게이트(G1)를 Stop 훅/체크리스트로 반자동화 — "완료" 단어가 나오면 검증 등급 라벨이 붙었는지 셀프체크.
