...
1. **뚫린 가드 보강 — `check-schema-refs.mjs`에 축 D(쓰기 경로) 추가.** 축 C가 `select` 문자열만 보던 것을 `.insert()/.update()/.upsert()`의 **객체 리터럴 키**까지 생성타입과 대조하도록 확장했고, 축 C와 달리 **처음부터 차단(exit 1)**으로 걸었다. 근거: select 오류는 화면에 0/[]로 뜨기라도 하지만 쓰기 오류는 "저장 눌렀는데 아무 일도 없음"으로 끝나 아무도 신고하지 않는다. 오탐 0을 위해 **직접 객체 리터럴만** 보고 스프레드(`...`)·계산된 키(`[x]:`)가 있으면 건너뛴다. → 켜자마자 위 2차 발견 6건을 잡았다.
