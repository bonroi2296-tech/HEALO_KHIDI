...
## #16 — 크롬 카자흐어 화자가 깨진 러시아어 STT 폴백에 갇힘 (1순위 시장 통역 정확도) (2026-06-21)

**무슨 일**
- 주간 트렌드 스캔(Gemini 3.5 Live Translate 조사) 중 우리 통역 파이프라인을 추적하다 발견: 크롬을 쓰는 **카자흐어 환자**의 화상방 발화가, 카자흐어를 직접 지원하는 **서버 STT(Gemini 멀티모달)** 로 안 넘어가고 브라우저 STT 의 **러시아어 폴백(ru-RU)** 에 머물러 러시아어로 오인식됨. 카자흐스탄은 우리 **1순위 타깃 시장**이라 통역 정확도·KHIDI 평가에 직결.

**왜 그랬나 (근본원인)**
1. `useSpeechRecognition.js` 의 `LANG_MAP.kz = "ru-RU"` — 크롬에 kk-KZ 인식기가 없어 의도적으로 러시아어로 폴백(자막이 아예 안 뜨는 것보단 낫다는 옛 판단).
2. 그러나 `page.jsx` 의 `useServerStt` 분기는 "브라우저 STT 가 **실패/미지원**일 때만" 서버 STT 로 전환. 크롬은 kz 를 ru-RU 로 폴백해 `isSupported=true`·에러 없음 → **"지원"으로 판정**되어 서버 STT 전환 조건에 영원히 안 걸림.
3. 즉 "폴백이 성공적으로 동작하는 것"과 "그 폴백이 정확한 것"을 구분하는 신호가 없었음 — 조용히 틀린 언어로 인식.
4. 빌드·기존 테스트로 안 걸림: 순수 분기 로직이지만 "kz=폴백"이라는 사실이 코드 어디에도 1급 데이터로 없었고(주석뿐), 라우팅 불변식을 검사하는 테스트가 없었음.

**어떻게 고쳤나**
- `useSpeechRecognition.js`: 순수 헬퍼 `isBrowserSttNative(lang)` + 폴백 집합 `STT_FALLBACK_LANGS={kz}` 추가. `LANG_MAP` 도 export(가드 테스트가 읽도록).
- `app/consultation/[id]/page.jsx`: 통역 켤 때 화자 언어가 폴백 전용(kz)이면 `forceServerStt` ON + 브라우저 STT 시작 안 함 → 서버 STT(kz 직접 지원)로 라우팅.
- 가드 테스트 `sttRouting.test.ts`(8개): kz=비네이티브 고정 + **"LANG_MAP 의 모든 교차언어 폴백은 STT_FALLBACK_LANGS 에 등록돼야 한다"** 불변식(미래에 다른 CIS 언어를 ru 로 폴백 추가 시 등록 깜빡하면 CI 실패).

**유사 이슈 전수 스캔 결과**
- STT: 6개 언어 중 크롬 네이티브 미지원은 **kz 하나뿐**(ko·ru·en·zh·ja 는 네이티브) → `STT_FALLBACK_LANGS` 완전.
- 서버 STT(`/[id]/stt`, Gemini): 6개 언어 모두 지원(kz 포함) → 라우팅만 고치면 정확.
- **TTS(`useTTS.js`) — 같은 부류, 낮은 심각도**: `kz="kk-KZ"` 인데 브라우저에 kk 음성이 거의 없어 매칭 실패 시 기본 음성으로 엉뚱하게 읽거나 무음. 단 TTS 는 **선택 기능(자막 음성낭독, 옵션)** 이고 클라이언트만으론 깔끔한 해결책 없음(서버 TTS 필요) → 이번엔 KNOWN_ISSUES 에 한계로 기록만, 미수정.

**재발 방지 (시스템 적용)**
- "폴백이 동작한다 ≠ 폴백이 정확하다." 언어/포맷 폴백을 넣을 때는 **그 폴백이 부정확하다는 사실을 1급 데이터(집합/플래그)로 남기고**, 더 정확한 경로(여기선 서버 STT)가 있으면 그쪽으로 라우팅하는 분기 + **불변식 테스트**로 잠근다.
- 이 부류 가드는 `sttRouting.test.ts` 의 "교차언어 폴백 등록" 불변식이 CI 매 PR 에서 차단.
