...
**어떻게 고쳤나**
- 공개·폐기 라우트 5개를 `redirect()` → `permanentRedirect()`(308)로 전환. 프로덕션 실측으로 307→308 확인.
- **의도적으로 안 바꾼 것(판단 근거 기록)**: `proxy.ts`의 `/partner`→`/hospital`, `/doctor`→`/`는 그대로 307 유지 — 로그인 벽 안 + robots.txt Disallow 대상이라 SEO 무관이고, 코드 주석이 "되돌리기 쉽게 임시"를 명시한 의도적 선택. `app/coordinator/intakes`·`app/patient/education`도 로그인 벽 안이라 제외.

**재발 방지**
- **신규 가드 `check:content` §23**: 공개(비로그인) 영역에서 **본문이 리다이렉트뿐인 page**(= 라우트 폐기용 껍데기)가 `redirect()`를 쓰면 CI 실패. 오탐 방지 3중(JSX 있으면 제외 = 조건부 리다이렉트는 임시가 맞음 / 로그인 뒤편 제외 / proxy.ts 분기는 검사 밖). **가드 유효성 검증: 수정을 일부러 되돌려 실제로 적발됨을 확인.**
