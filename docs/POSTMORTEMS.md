...
1. **가드 추가**: `check-content-consistency.mjs` **§24** — 구조화 `EMERGENCY_NUMBERS` ↔ 각 언어 고지 산문을 **국가별로** 대조(그 국가 표기가 있는 줄에 그 국가 번호가 다 있는지). 응급번호 드리프트는 결과가 "잘못된 번호로 전화"라 사람 검토에 안 맡긴다.
   - ⚠️ **이 가드의 초안이 실측에서 깨졌다(기록으로 남긴다)**: 처음엔 "그 번호가 그 언어 블록에 **존재하나**"로 짰다. 한국 119 → 118 로 바꿔놓고 돌렸더니 **통과**했다 — 같은 블록의 **일본이 119** 라 존재검사가 만족돼서. 한국·일본(119), 카자흐·러시아(103·112)가 번호를 공유하니 **5개국 중 4개가 이 구멍**이었고, 하필 가장 흔한 드리프트(한 국가 번호만 수정)를 정확히 못 잡는 검사였다. → 국가별 줄 단위 대조로 교체하고 **깨뜨리는 테스트 2종(KR 119→118, KZ 112 삭제)이 실제로 실패하는 것**까지 확인하고 채택. **교훈: 통과하는 가드는 증거가 아니다. 일부러 깨뜨려서 빨간불이 켜지는 걸 본 뒤에 채택할 것.**
