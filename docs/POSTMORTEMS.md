...
**어떻게 고쳤나**
- `src/lib/i18n/config.js` 에 `LEGACY_LANDINGS` + `isLegacyLanding()` 신설(**단일 SoR**). `proxy.ts` 의 `LEGACY_SKIP` 은 이제 이 상수를 import — 한쪽만 고쳐지는 구조를 없앰.
...
**재발 방지 (뚫린 가드 보강)**
- `check-content-consistency.mjs` **§25 신설**: `app/` 최상위에 **언어 코드 모양의 실제 폴더**(`en·ko·ru·kz·zh·ja` + ISO `kk`)가 있는데 `LEGACY_LANDINGS` 에 등재 안 됐으면 CI 실패. 공개 경로는 프록시 rewrite 로 폴더 없이도 6개 언어가 다 존재하므로, **실제 폴더 = 그 언어에만 있는 번역 없는 페이지 = 이 404 함정**이라는 판정이 성립한다. 돌연변이 시험으로 실증(`app/zh/some-landing` 주입 → 검출 → 원복 → 통과).
- `src/lib/i18n/config.test.ts` 신설: `localeSwitchTarget` 이 두 랜딩 × 6개 언어에서 **절대 없는 주소를 반환하지 않음**을 단정 + 일반 경로·내부 경로 회귀 4건.
- **한계**: §25 는 `app/` 최상위 1단계만 본다. 그리고 "언어 스위처가 만든 주소가 실존하나"의 일반 검사는 아직 없다 — 지금은 *언어 폴더가 곧 유일한 발생원*이라는 사실에 기대고 있다. 프록시의 `PUBLIC_PREFIXES` 밖에 새 공개 페이지를 만들 때는 언어 전환 동선을 직접 확인할 것.
