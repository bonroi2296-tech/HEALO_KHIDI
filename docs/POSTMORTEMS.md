...
- **재발 방지**: **습관을 기계로 승격** — `check-content-consistency.mjs §33`에 사전 파일의 **중복 키 검사**를 추가했다. 이제 `npm run lint`를 잊어도 내가 항상 돌리는 `check:content`가 먼저 잡는다. (#61의 "lint 를 습관화" 는 5년치 교훈으로 남기되, 실제 차단은 기계가 한다.)
