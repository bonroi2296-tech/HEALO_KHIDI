...
## #149 — 시작화면이 영영 안 걷혀 앱이 «못 쓰는 상태»가 될 뻔함 — 내가 넣은 `showSpinner: true` (2026-07-28, 에뮬레이터가 잡음)
...
## #148 — 🔁 #32 부류 재발 — 모바일에서만 나는 결함을 «폰으로 안 봐서» 못 잡음: 44px 터치타겟 규칙이 아이콘 버튼을 밀어냄 + 투명한데 눌리는 삭제 버튼 (2026-07-28)
