...
- `check-content-consistency.mjs` **§27 신설**: `app/ClientShell.jsx`(모든 공개 화면이 쓰는 **전역 껍데기**)의 JSX 텍스트 노드에 `영문 라벨:` 꼴 리터럴이 있으면 CI 실패. 한글누출 가드의 **반대 방향**을 처음으로 메운다. 양방향 돌연변이 시험 실증 — 옛 코드(`<div>Service Name: …`) 재주입 → 검출, 원복 → 0건.
