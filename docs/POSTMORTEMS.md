...
- **`portal/emergency` 「SOS 누를 때마다 응급 스레드 중복 생성」은 발생 불가능한 증상이었다.** 전수 검색 결과 그 라우트를 **호출하는 코드가 0건**(`EMERGENCY_SUBJECT` 를 읽는 곳도 0건). 증상을 확인하지 않고 "부류가 같다"는 이유로 사고 시나리오를 지어냈다. 게다가 넣은 실패-닫힘 500 은 응급 경로에선 **거꾸로 된 트레이드오프**다(중복 스레드=미관 / SOS 차단=실제 피해). → fail-open + 로그로 정정.
