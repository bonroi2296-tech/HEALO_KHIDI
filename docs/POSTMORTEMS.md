...
## #83 — 화상상담 통역 실질 정상률 46%: 문장 절단·언어 echo·무음 환각이 로그 전수조사에서야 드러남 (2026-07-11, PO 실사용 제보 계기)

**무슨 일** — PO가 7/10 카자흐 에이전시 실미팅(23분)에서 "통역 품질이 안 좋다"고 제보. 번역 로그 194건 전수조사 결과: ①VAD 무음 컷 0.7초가 쉼표 호흡에도 끊겨 문장 중간 절단 조각이 문제의 ~52% ②한국어 발화가 ru→ko 설정으로 들어가 번역 없이 원문 echo + `source_lang=ru`로 로그 오염 10건 ③무음 구간에 STT가 "Здравствуйте"를 20회+ 창작(환각) ④조각 단위 무맥락 번역이 대명사·돈 방향을 뒤집음("수수료를 받게 됩니다"→"부과될 것입니다"). 맞장구(39%)를 뺀 실질 발화 정상률 ~46%.

**왜 못 잡았나 (근본원인)** — 자막 파이프라인의 품질을 **측정하는 장치가 전혀 없었음**: `consultation_translations.confidence`는 전량 null, 환각·절단·echo 를 세는 지표도 대시보드도 없어 "빌드 통과 = 동작"으로 방치됨. AI 챗 품질은 매일 회귀 채점(`/admin/khidi/ai-regression`)이 도는데 **통역 파이프라인만 채점 루프 밖**이었다. 개별 원인들도 설계 때 예견 가능했던 것(0.7초는 사람 호흡보다 짧음, 언어 고정 라우팅은 다국어 현장과 불일치)이지만 실통화 로그를 아무도 안 읽어 눈에 안 보였음.

**어떻게 고쳤나** — PR #731: VAD 1.2초·10초 상한 / 서버 STT 언어 자동 감지(`l` 필드) + `translate-realtime` 결정론 echo 가드(`detectLanguage()`) + source_lang 감지값 기록 / 침묵 환각 반복 필터(클라, 억제 시 타임스탬프 미갱신 — 슬라이딩 창이면 정당한 반복 발화가 영구 억제됨) / 프롬프트에 조각 임의완성 금지·무음 창작 금지 / 직전 대화 6개 문맥 전달 / 맞장구 사전 처리(단, 물음표 있으면 사전 금지 — "네?"가 "네"로 둔갑하는 의료 사고 방지 + 큐 순서 보존 + DB 로그 유지).

**재발 방지** — ①실패 사례 10건을 `scripts/eval-translation-cases.mjs`로 고정(프롬프트 회귀 시 재현 평가 — API 키 있는 환경에서 1커맨드) ②`docs/KNOWN_ISSUES.md`에 confidence/품질 지표 수집을 후속 과제로 등록 — **통역도 AI 챗처럼 "분포로 측정"하는 루프에 넣기 전까지는 미완**(CLAUDE.md "AI 품질은 분포로" 원칙의 사각지대였음) ③7/10 이전 로그의 `source_lang`은 오염 데이터로 문서에 명시(이후 분석이 다시 속지 않게) ④교훈: **실사용 로그 전수조사가 코드 리딩보다 진단이 빨랐다** — 다음 품질 제보도 로그부터 읽는다.
