...
## #66 — /hospitals 의료진 카드 오른쪽이 통째로 잘림 ("Full Profile"→"Fu…") — flex min-w-0 누락 (2026-07-14)

- **무슨 일**: PO가 /en/hospitals 화면에서 직접 발견. DoctorCard 정보영역(`flex-1`)에 `min-w-0`가 없어, `truncate`(줄바꿈 금지) 경력 미리보기 줄의 최소폭이 카드 폭을 밀어냄 → 카드의 `overflow-hidden`이 오른쪽을 통째로 절단(태그·"Full Profile" 링크 잘림). 영어 경력 텍스트가 제일 길어 /en에서 가장 심했음.
- **왜 못 잡았나(근본원인)**: ①같은 패턴을 쓰는 다른 공개 화면 5곳(병원상세·치료상세·원격협진·한방·헤더)은 전부 `min-w-0`/`max-w`를 달았는데 이 카드만 누락 — 시각 레이아웃 회귀엔 기계 검사가 하나도 없어 사람(PO) 눈에만 의존 ②E2E 스모크(hospitals-list)가 "카드가 있다"만 보고 "안 넘친다"는 안 봄 ③이 페이지는 PO 제보 수정만 4번째(#565 밀림·#660 지점카드·#663 사진폴백·이번) — 공통 원인은 시각 회귀 무검사.
- **어떻게 고침**: DoctorCard `flex-1`에 `min-w-0` 추가(+현장 주석). 데스크톱 1500px·모바일 375px에서 의료진 카드 28개 전부 넘침 0px·링크 완전표시를 DOM 실측으로 확인.
- **재발 방지**: `e2e/hospitals-list.spec.ts`에 "의료진 카드 가로 넘침 0px" 회귀 가드 추가 — 수정을 일부러 빼고 돌려 **실제로 실패함을 확인**(가드 유효성 검증). 교훈: **flex 아이템 안에 truncate/nowrap 자식을 두면 그 flex 아이템에 min-w-0 필수** (grep 키워드: min-w-0, truncate, 잘림, overflow).
