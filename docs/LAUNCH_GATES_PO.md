...
## 🎯 지금 남은 관문 (2026-07-02 실측 — 우선순위순)

| 순위 | 관문 | 누가 | 비고 |
|---|------|------|------|
| 1 | 🔴 **관문3 — 구글 OAuth 게시** (콘솔 클릭 1회, 검수 불요) — 지금은 실환자 구글가입이 막혀 있음 | PO 콘솔 | ↓ 아래 가이드 |
| 2 | 🔴 **관문7 — Gemini 유료 결제 확인** (의료 PII 학습 방지) | PO 콘솔 | ↓ 아래 가이드 |
| 3 | 🟡 **관문9 — 테스트 문의 태깅** — #26~31 등 미태깅 더미를 대시보드 「테스트로 표시」 버튼으로(삭제 아님·가역·1분) | PO(또는 승인 후 나) | 실적 집계 오염 방지 |
| 4 | 🟢 **관문8 — 텔레그램 알림 env 2개** | PO 콘솔 | ↓ 아래 가이드 |
| 5 | 🟡 **관문10 — DPA 서명** (무료) | PO | `docs/audit/DPA_SIGNING_GUIDE.md` |
| 6 | 🔴 **관문13~17 — 앱스토어**(결제·Firebase·에셋·빌드) | PO | 아래 C섹션 |
...
| 3 | **구글 OAuth 게시(테스트→프로덕션)** | PO 콘솔 | 🔴 **열림** — ↓ 아래 가이드 |
...
| 6 | 약한비번 테스트계정 | PO | ✅ **실질 닫힘** — `test1234` 로그인 실측 400(사망), 전 계정 강비번+GitHub Secret 보관. `admin@test.com` 계정 자체는 활성 유지 중 — "강비번 유지"로 종결할지 "비활성"까지 갈지 PO 결정 1건 남음 |
...
| 9 | 🟡 **테스트 문의 정리** — #26~31(+국적 혼재 13·14·17·20) 미태깅 확인(실DB) | PO/나 | 열림 — 삭제 대신 「테스트 도장」 태깅 권장(가역) |
| 10 | 🟡 **DPA 서명** | PO | 열림(코드로 검증 불가) |
| 11 | 법무문서 PR #422·#424 | PO | ✅ **닫힘** — 2026-06-30 둘 다 머지(gh 실측) |

> 🆕 **관문 외 신규(2026-07-02 감사)**: ①Vercel prod env `TEST_OFFICE_IPS` 미설정 — 사무실 IP 자동 테스트태깅이 무장해제 상태(PO만 값을 앎, 콤마구분 공인IP) ②발급 PDF(견적서 등) 한글·키릴 깨짐 — 폰트 등록 별도 수정 트랙 진행.
