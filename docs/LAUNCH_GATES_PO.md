...
| 6 | ✅ **약한비번 테스트계정** — 2026-06-29 전수조사에서 7개 `@test.com` 비번을 `test1234`→**`Healwith2026!`(강한 비번)**으로 교체 완료(`admin@test.com` 포함). **남은 PO 액션**: E2E 자동검사용 GitHub Secret 비번값도 `Healwith2026!`로 갱신해야 로그인 자동검사가 계속 돈다. | 나(완료)/PO(시크릿 갱신) | 거의 닫힘 |
