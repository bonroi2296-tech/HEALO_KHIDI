...
> **📌 중간 저장 (2026-07-24 저녁 — 실회의 2건 «2026-07-27 주에 이어서» 메모, PO 요청)** — PO: *"오늘 회의 2건 했어. 이거 관련해서도 얘기할 거 있는데 일단 정리해봐, 다음 주에 다시 하게 메모 남겨줘."* → **회의 내용(안건·상대·결정)은 아직 못 들었다. 다음 주 세션이 PO에게 먼저 물어 이 칸을 채워라 — 추측 금지.** 지금까지 확인된 **사실만**: 회의① 방 `23275b3c` 17:53~18:19(26분, 게스트 `healwith_moon`·`Assel`·`Жанат`) + 낮 14:10~14:44 별도 세션 / 회의② 방 `61d68d3a` `Assel` 18:38~18:54·`Radmila` 19:01~19:07 / 자막 실동작(① ru→ko 181줄, ② 양방향) / 회의① 실구간 **ko→ru 0줄**(원인 미확정) / 🔴 **두 방 다 문의 미연결·`scheduled`라 KHIDI 실적 미집계**(= 아래 KNOWN_ISSUES 최상단 «사전상담 집계 0»).

## 🔖 세션 핸드오프 (2026-07-24 — **화상상담 라이브 3종 수리(하울링 2단·자막 스트리밍·통역 부활) + 실회의 2건 데이터 실측 → 🔴 KHIDI 사전상담 집계 0 발견** 세션 종료)

> 병렬 세션(같은 날 백오피스·E2E·딥테크 세션과 다름). 만진 영역 = `app/consultation/[id]`·`src/lib/consultation`·`app/api/khidi/consultation/translate-realtime`.
> 📝 **정정(중요)**: 이 세션은 작업본 `worktree-consult-live-fixes` → [#956](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/956)으로 올렸으나, **어시 머지 권한 차단 + main 선행으로 방치되던 중 다른 세션이 [#977](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/977)에 흡수해 머지**했다(#956은 CLOSED). **코드는 전부 main에 있음**(`isTonalFrame`·`maybeTranslatePartial`·`voiceOn` 존재 실측). 이 블록은 그 작업의 «왜»와 미검증 항목을 남기려고 사후에 문서만 따로 올린 것.

**1. 이번 세션 한 일**

- **회의 전 사전점검(PO 요청)** — 예약 상담 2건 확인 + **LiveKit 키 생존을 실호출로 실측**(로그인 → preflight 토큰 → LiveKit `/rtc/validate` **200**). 2026-07-02 «키 폐기» 장애 부류 아님을 확인.
- **PO 실테스트 피드백 3건 수리**(→ #977로 머지·배포)
  - ①**하울링 2단 감지(AGC 대응)** — 기존 문턱(HOWL_RMS 0.45=마이크 포화 전제)을 브라우저 자동게인이 눌러 못 넘던 경로. 중간 음량(0.22)+양쪽 «단일음»(하울링 스펙트럼 지문 `isTonalFrame`) 600ms 지속 시 자동 음소거. 단위테스트 12종 추가.
  - ②**자막 스트리밍** — 중간결과(interim)를 1.2초 간격으로 번역해 먼저 전송(기울임+…), 확정 시 최종 번역이 교체. 부분 번역은 **표시 전용**(기록·DB·TTS·문맥 제외, 서버 `partial` 플래그로 저장 스킵).
  - ③**통역(음성) 토글 부활 + 봇 공존설계** — 봇(`agent-*`)을 타일·인원수·«상대 기다림»에서 제외 / 봇 통역음성(`tx:*`) 청취모드 재전사 차단(삼중자막 원인) / 통역 ON=내 언어 통역트랙 구독+원음 음소거, OFF=전부 해제(**참가자별 독립 선택**).
- **독립 리뷰 + 완성도 감사** — 자막 순서 역전 필터(발화 세대 `utter` 동봉), 봇 부재 시 통역 자동꺼짐 10초 유예, 사용설명서 2토글 갱신.
- 🔴 **실회의 2건 데이터 실측 → KHIDI 사전상담 자동집계가 구조적 0임을 발견**(상세 = `docs/KNOWN_ISSUES.md` 최상단, 아래 3·5번).

**2. 왜 그렇게 했는지**

- **하울링을 «스펙트럼 모양»으로 잡는 이유**: AGC는 소리 **크기**만 누르고 **모양**은 못 바꾼다. 하울링=한 주파수에 에너지 몰린 단일음, 말소리=포먼트로 분산 → 음량 문턱이 뚫린 상황의 보조 지문. [#922](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/922) 독립리뷰가 경고했던 경로가 실전에서 그대로 났다.
- **부분 자막을 «표시 전용»으로 못 박은 이유**: 같은 발화가 기록·회의록·번역 문맥에 여러 번 쌓이면 AI 요약이 오염된다(7/20 «ru→ru 53건» 부류). 화면만 빠르게, 기록은 확정본 하나.
- **통역을 «참가자별 독립»으로 한 이유**: 7/23엔 봇을 켜자 방 전체가 깨져(빈 타일·삼중자막) 통째로 껐다. 공존설계에선 통역을 안 켠 사람은 기존과 100% 동일 → 한 사람이 켜도 남이 안 깨진다.
- **PO 요구 «반반 섞어»의 해석**: 2026-07-23 버전=빠르지만 거친 자막, 2026-07-24 버전=좋지만 느린 자막 → 택일이 아니라 «빨리 띄우고 뒤에서 다듬는» 스트리밍이 정답.

**3. 안 끝났거나 보류**

- 🔴 **KHIDI 사전상담 집계 0 — 미수정**(발견만). 회의를 해도 실적에 안 잡힘. 고칠 방향 4가지는 KNOWN_ISSUES에 기록, **PO와 2026-07-27 주 논의 예정**.
- **통역 봇 실가동** — 서버 스위치 `LIVE_TRANSLATE_ENABLED` 여전히 **false**(7/23 PO 결정 유지). 켜려면 ①스위치 true+재배포 ②**유료 Gemini 키**(무료키는 구글 학습 리스크로 실환자 금지 — `docs/LIVE_TRANSLATE_SETUP.md`). = PO 돈 결정 대기.
- **7/24 회의 2건 내용(안건·상대·결정) 미확보** — PO가 «2026-07-27 주에 얘기하자»며 보류. 아래 5번 참조.
- **7/23 대기 목록 그대로** — 감지 기반 통역(발화 언어 감지)·초대 URL 단축·역할(role) 제거·preflight 문구 정직화+«카메라 없음» 표시.
- **청취 모드(수신측 전사)는 여전히 발화 종료 후 자막** — 이번 스트리밍은 «자막 켠 화자»의 송신측 경로만.

**4. 주의·함정** ⚠️

- **머지 권한 차단** — 이 세션에서 `gh pr merge`·main 직접 푸시가 분류기에 막혔다. **우회하지 말고 PO 클릭 요청**, 감시는 Monitor 도구로(쉘 while 루프도 차단됨). 막힌 사이 main이 3번 앞서가 리베이스 2회 → 결국 다른 세션이 흡수해 머지(#977).
- **하울링 2단 임계값은 실측 없는 휴리스틱** — `TONAL_RATIO 5`·`HOWL_RMS_AGC 0.22`·`TONAL_SUSTAIN_MS 600`(`src/lib/consultation/useSameRoomDetect.js`). **안 잡히면 RATIO 4로 낮추고, 정상 통화에서 오작동하면 6으로 올려라.** 실패 방향은 «안 꺼짐»(안전).
- **병원 전자음 오탐 가능성**(독립리뷰 PLAUSIBLE) — 장비 비프·팬처럼 «양쪽에 동시에 잡히는 지속 단일음»이면 2단이 걸릴 수 있음. 되돌리기 막대는 항상 남음.
- **통역 버튼 회색 = 정상**(봇 꺼져 있을 때). 눌러도 «아직 안 켜져 있어요» 안내만.

**5. 다음 세션이 먼저 할 일**

1. ⚠️ **직전 미검증분 먼저 확인**: #977 배포분으로 **실측** — ①2대 실오디오 하울링 자동차단(안 되면 `TONAL_RATIO` 4로) ②말하는 중 자막이 상대 화면에 자라나는지 ③기록 패널에 부분 자막이 안 새는지. **코드·CI·빌드는 통과했으나 실동작은 하나도 검증 못 했다.**
2. 🔴 **KHIDI 사전상담 집계 0 — PO와 방향 결정 후 수리**(8/27 중간평가 K-02 점수 직결). 소급 연결 수단까지 포함.
3. **2026-07-24 회의 2건 debrief를 PO에게 받기**(안건·상대·결정·후속). 로그 사실만 아래 6번에 있고 내용은 PO만 안다 — **추측으로 채우지 말 것**.
4. **회의①의 «한국어→러시아어 자막 0건» 원인 확인**(6번) — 러시아 참가자가 자막을 못 본 것이면 게스트 자막 온보딩 문제.
5. (후순위) 통역 봇 실가동 결정 / 7/23 대기 목록.

**6. 검증 상태**

- ✅ **자동 검사 통과**: `vitest src/lib/consultation` **41/41**, `npm run check:content`, `npx next build --webpack` 전부 통과. #956 CI(`ci`·`Smoke Tests`) 초록이었고, 그 코드가 #977로 머지됨.
- ✅ **독립 리뷰** = CONFIRMED 0, PLAUSIBLE 3 중 2건 즉시 수리. **완성도 감사** = 드리프트 2건 수리, i18n 6개어 패리티 18/18.
- ✅ **LiveKit 프로덕션 키 생존 = 실측**(`/rtc/validate` 200). 상담방 페이지 프로덕션 200.
- ✅ **실회의 2건 진행 확인(DB 실측)** — ①방 `23275b3c`: 17:53~18:19(26분) 게스트 3명(`healwith_moon`·`Assel`·`Жанат`) + 낮 14:10~14:44(34분) moon+Assel. ②방 `61d68d3a`: `Assel` 18:38~18:54, `Radmila` 19:01~19:07. **자막 파이프라인 실동작**: ① ru→ko **181줄**, ② ru→ko 18·ko→ru 4(양방향).
- ⚠️ **관찰(원인 미확정)**: 회의① 실구간(17:53~18:19)에 **ko→ru 기록 0건**(ko→ru 6줄은 전부 14:12 예행). «그 회의에서 한국어 발화가 러시아어로 번역된 적 없음»은 데이터상 사실이나, 원인은 ⓐ한국어측 마이크 끔(참관) ⓑ양측 해당 방향 자막 미사용 ⓒ한국어 발화 자체가 적었음 중 **로그만으론 확정 불가** → 5번 4항.
- ✅ **KHIDI 집계 0 = 실측**: `consultation_sessions` 42건 중 `inquiry_id` 있는 것 **0건**, `status='completed'` **1건** → 집계 조건(`completed` AND `inquiry_id NOT NULL`)과 교집합 0. **배선 버그 아님을 반증으로 확인**(API가 `selected_inquiry_id` 수용, picker 17건 정상 반환).
- ❌ **미검증(정직)**: 하울링 2단의 **실오디오 동작**, 자막 스트리밍의 **실화면 동작**, 통역 토글 **실동작**(봇 꺼져 있어 애초 불가). 상담방은 로그인·실마이크 필요로 자동 검증 불가(`verify_authgated_portal`). #977 배포 상태도 이 문서 작성 시점엔 `pending`이라 **배포 완료 미확인**.

**7. 다음 세션 첫 프롬프트**

> 먼저 `docs/PROJECT_CONTEXT.md` 최상단 핸드오프를 읽어라. 2026-07-24 화상상담 수리(#977)가 배포됐는지 확인하고, 다음 회의 전에 하울링 자동차단·자막 스트리밍 실동작을 실측해. 그리고 KHIDI 사전상담 집계가 0인 문제(KNOWN_ISSUES 최상단)를 PO와 상의해서 고치고, 2026-07-24 회의 2건 내용은 PO한테 직접 받아서 기록해.
