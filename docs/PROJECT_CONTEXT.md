...
## 🔖 중간 저장 (2026-07-24 — **유료 전환 결제 진행 중**)

> 결제·외부 계정 세션(코드 변경 없음). 세션이 죽어도 "어디까지 결제됐고 뭐가 막혔는지"가 남게 중간 저장.

**🛑 월요일(2026-07-27) 최우선 — Google Play 전화번호 인증**

- ✅ **Google Play 개발자 계정 생성 + $25 결제 완료** — **개인** 계정, 이름 `healwith`, 계정 ID `5457445830881937645`.
- 🛑 **남은 것 = 연락처 전화번호 인증.** 등록된 번호가 **대표 명의(LGU+)**라 대표 본인이 인증 코드를 받아야 하는데 **2026-07-24 연락 불가 → 월요일 처리.** 이거 끝나야 계정 설정 완료(그전엔 앱 게시 불가).
- 남은 인증 2개 더: **본인 확인(공문서)** · **Android 휴대기기 액세스**(Play Console 모바일 앱 로그인).

**🔴 정부지원금 전용카드 해외결제 막힘 (모든 결제의 병목)**

- IBK기업은행 BC corporate(gosims) 카드로 **Apple**("인증이 실패했습니다")·**LiveKit/Stripe**("해당 신용카드는 이러한 유형의 구매를 지원하지 않습니다") **둘 다 거절**. 발급사 거절이라 가맹점 쪽에선 못 고침.
- → 카드사에 **①방금 거절 사유 조회 ②해외결제 ③정기(구독)결제** 개방 요청. 안 풀리면 (a)자기비용 선집행 후 대체(관리지침 33p) 또는 (b)진흥원 문의 — 그때 공문 문구는 어시가 작성.
- ⚠️ **구글 $25가 어느 카드로 결제됐는지 확인 필요** — 전용카드가 아니면 정산 증빙에서 문제됨.
- 미결제 잔여: Gemini(+spend cap 필수)·Vercel Pro·Supabase Pro·LiveKit Ship·Apple $99.

**📌 계정 유형 판단 (재검증 완료 — 다음 세션이 다시 논쟁하지 말 것)**

- 구글은 **"건강 앱(의료 앱·인간 대상 연구 앱)"에 조직 계정을 요구**하나, 공식 문서의 의료 앱 예시는 EHR·환자포털·원격모니터링·증상검사기·SaMD 등. **우리(병원 정보 제공 + 상담 연결, 진단·처방 없음)는 명시적 해당 아님** → 애플이 이미 개인이라 **개인으로 진행 확정**. 회색지대는 예시에 있는 "원격 의료(telemedicine)" 표현 하나.
- **최종 판정은 [앱 콘텐츠] → 건강 앱 선언 양식**에서 갈림. 그 양식 나오면 **실제 기능 기준(진료 주체=병원 의사, 우리는 중개)**으로 답할 것 — 등록 시 자기소개에도 "진단·처방 등 의료행위는 앱에서 제공하지 않음"이라 적어둠(진술 일관성 유지).
- 🛑 **개인 계정 함정(문서 누락분·이번에 발견)**: 프로덕션 출시 전 **테스터 12명이 14일 연속** 비공개 테스트 참여 필수(**면제 없음**, 조직만 면제). 출시 일정은 여기서 역산. 상세·출처 = [`docs/APP_STORE_LISTING.md`](APP_STORE_LISTING.md).

---

## 🔖 세션 핸드오프 (2026-07-24 저녁 — **DB 멈춤 「부하를 만든 주체」 실측: CI 자동검사(E2E)가 프로덕션 DB를 물고 22갈래 동시 실행** 세션 종료)

> PO가 "3시간째 API 호출이 계속돼 슈파베이스가 멈췄다"며 원인 파악을 시킨 **진단 전용 세션(코드·설정 변경 0, 머지 0)**. 같은 장애를 다룬 병렬 세션이 2개 더 있다(반성문 #117 = PR [#971](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/971) / E2E 안정화 = [#967](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/967) 머지됨). **이 블록은 그 둘이 못 채운 「그 부하를 누가 만들었나」 실측분.**

**1. 이번 세션 한 일**

- **DB 재시작 시각 확정** — `pg_postmaster_start_time()` = **2026-07-24 08:40:42 UTC (=17:40:42 KST)**. `pg_stat_statements`의 `stats_reset`과 초 단위까지 일치 → 통계 리셋은 재시작 지문. PO가 본 "멈춤"의 종료 시각이 이것.
- **부하 주체 실측(이 세션의 고유 성과)** — `auth.sessions`를 user+ip로 집계하니 로그인 주체가 **`admin@test.com`·`coordinator@test.com`(테스트 계정)**이고 접속 IP가 매시간 **수십 개씩 다른 Azure 대역**(20.x/40.x/52.x/57.x/135.x/145.132.x/172.x). 브라우저면 IP가 하나여야 한다 → **사람이 아니라 GitHub Actions 러너**. `gh run list`로 확증: **14시간 동안 E2E 89회 실행 / 작업본(브랜치) 22개 / 89회 중 32회 `cancelled`** (피크 06:00 UTC=15:00 KST에 1시간 19회).
- **구조적 결함 2개 특정** — ①`.github/workflows/e2e.yml`이 앱은 러너 로컬(`localhost:3000`)에 띄우면서 **DB는 프로덕션(`hvwwlkawaxabhtumjhrg`)을 그대로** 물린다(연습용 DB 없음). ②동시실행 잠금이 `group: e2e-${{ github.ref }}` = **브랜치별로만** → **전역 상한이 없어** 작업본 22개면 22개 스위트가 같은 DB에 동시 진입.
- **좀비 상담방 탭 산수 검증** — `app/consultation/[id]/page.jsx`의 폴링 3종이 `livekitToken` 존재만 보고 돌아 탭 닫기 전엔 안 멈춤. DB 실측과 정확히 일치: `consultation_messages` 1,526회·`consultation_translations` 1,526회(**동일** = 4초 폴링 × 101분), `consultation_documents` 841회(8초 × 112분) → **탭 1개가 약 2시간 동안 아무도 안 보는 채로 분당 37.5회** 호출.
- PO에게 설명 그림 2장 제공(장애 개요 / 좀비탭 원리). 파일은 세션 임시폴더라 **저장 안 됨**(필요하면 다음 세션이 재생성).

**2. 왜 그렇게 했는지**

- **IP 집계를 판정 도구로 쓴 이유** — "사람이 쓴 거냐 CI가 돌린 거냐"를 가르는 가장 싼 증거. `auth.sessions`를 user+ip로 group by 하면 끝난다(브라우저=IP 1개, CI=매번 다른 클라우드 IP). 다음에 같은 유형 의심되면 이 쿼리부터.
- **`pg_stat_statements` 절대 호출수로 결론 내지 않은 이유** — 조사 중 그 함정에 한 번 빠졌다(아래 4번 정정). 그래서 **절대 시각이 찍힌 증거**(`gh run list`·`auth.sessions.created_at`·postgres 로그)로만 결론을 다시 세웠다.
- **조치를 안 한 이유** — PO가 "다음 주에 마저 하게 메모 잘 해놓고 핸드오프해"라고 명시적으로 이월 지시. 진단 결과만 남기고 손 안 댐.

**3. 안 끝났거나 보류**

- 🔴 **E2E 전역 동시실행 상한 추가 = 미조치(다음 주 1번).** `#967`(로그인 횟수 감축)이 머지됐고 `#969`(좀비탭)가 대기 중이지만, **"22갈래가 동시에 같은 DB로 들어가는 구조" 자체는 그대로**다. 아무도 안 건드리는 유일한 구멍.
- 🟡 **E2E를 프로덕션 DB에서 떼어내기** — 가장 근본적이나 **돈이 든다**(별도 Supabase 브랜치/프로젝트). **PO 결정 필요**(비용 산출부터 해야 함).
- 🟡 **[#969](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/969) 좀비탭 수정 머지** — 다른 세션 소유 브랜치라 병렬 세션 규칙상 내가 머지 안 함. CI 초록.
- 세션 종료 시각(2026-07-24 19:40 KST 기준) **E2E는 여전히 돌고 있음** — 최근 30분 로그인 137회. 불은 껐는데 가스는 안 잠근 상태.

**4. 주의·함정** ⚠️

- 📝 **정정(내 것)**: 조사 초반에 PO에게 "초당 59회"라고 말했는데 **틀렸다**. `pg_stat_statements`의 누적 호출수를 "최근 98분"으로 환산했으나 실제 통계 창은 약 4일치였다(`admin_audit_logs`가 시간당 50건인데 통계엔 4,630건 → 역산으로 발각). **교훈: `pg_stat_statements` 숫자는 `stats_reset` 시각을 반드시 같이 읽어라.** 게다가 내 두 번의 조회 사이에 리셋(=재시작)이 끼어 카운터가 줄어든 것처럼 보였다.
- 📝 **정정(다른 세션 것 — 다음 주에 이거 믿고 일하면 안 됨)**: 위 「2026-07-24 17~19시 KST」 중간 저장이 *"E2E 로그인 폭주가 원인은 **기각** — 장애 시작 시점에 E2E는 놀고 있었음(직전 07:40 종료, 다음 07:46 시작)"*이라 적었는데, **이 사실 진술은 부정확하다.** `gh run list`로 실측하면 장애 시작(07:45 UTC) 시점에 **장시간 실행 2건이 진행 중**이었다 — `docs/handoff-i18n-961`(06:40:43→07:53:49)·`claude/pensive-knuth-8540d8`(06:58:01→**10:21:33**). 짧은 실행의 *시작 시각*만 보고 긴 실행이 걸쳐 있는 걸 놓친 것. ⚠️ 단, 아래대로 **"그래서 E2E가 진범"이라는 뜻은 아니다.**
- 🚧 **인과는 여전히 미증명(가장 중요 — 나도 여기서 과했다).** PO에게 "진범 = CI E2E"라고 단정해 말했으나 그건 **증거보다 센 표현**이었다. 확실한 것은 ①E2E가 프로덕션 DB에 크고 불필요한 부하를 실제로 만들었다 ②그 구조가 결함이다 — 여기까지. 반성문 #117이 관측한 **"Supabase 자체 시스템 점검 쿼리조차 12~16초"**는 특정 쿼리가 아니라 **인스턴스 레벨 컴퓨트 포화**를 가리키므로, "E2E만 없었으면 안 죽었다"는 증명되지 않았다. **세 세션 결론을 합치면: 불안정·소형 인스턴스 + 우리가 스스로 만든 큰 부하(E2E·좀비탭·병렬 진단쿼리)의 겹침.** 다음 주에 누가 "원인 확정됐다"고 쓰지 마라.
- ✅ **좀비탭 폴링 4개 중 2.5초짜리(대기실 승인확인)는 좀비가 아니다** — `admissionStatus !== "pending"` 가드가 있어 승인되면 멈춘다. 안 멈추는 건 3개(채팅 4초·자막 4초·자료 8초). 처음에 4개 다 안 멈춘다고 설명했다가 코드 재확인으로 바로잡음.
- **이 브랜치(`chore/handoff-cms-session`)는 origin/main으로 재정렬(rebase)했다** — 원래 27커밋 뒤처져 94개 파일이 달라서, 그대로 PR을 열었으면 **남의 작업 94개를 되돌릴 뻔했다**(#971 세션이 밟은 것과 같은 함정). 이어갈 땐 최신 상태부터 확인.

**5. 다음 세션이 먼저 할 일**

1. ⚠️ **직전 미검증분 먼저 확인**: 이 세션은 **조치를 하나도 안 했다**(진단만). 그리고 **인과관계는 미증명 상태**다 — 4번의 🚧 항목을 먼저 읽고, "원인 확정"으로 취급하지 마라.
2. 🔴 **E2E 전역 동시실행 상한 추가** — `.github/workflows/e2e.yml`의 `concurrency.group`을 브랜치별(`e2e-${{ github.ref }}`)에서 **저장소 전역 1개**로 바꾸는 게 가장 싼 한 줄. 단 PR 검사가 줄서서 느려지므로 **PO에게 트레이드오프 버튼으로 확인 후** 적용.
3. 🟡 **[#969](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/969)(좀비탭) 머지 상태 확인** — 아직 열려 있으면 PO에게 알림(다른 세션 소유라 임의 머지 금지).
4. 🟡 **E2E를 프로덕션 DB에서 분리하는 비용 산출** → PO 결정 버튼.
5. (참고) 감시 공백은 [#972](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/972)(10분 cron 생사 감시)가 메우는 중 — 중복 작업 하지 마라.

**6. 검증 상태**

- **코드 변경 0 → 빌드·CI·`check:content` 해당 없음.**
- ✅ **실측으로 확인한 것**: DB 재시작 시각(`pg_postmaster_start_time`), E2E 실행 89회·22브랜치·32회 취소(`gh run list`), 테스트 계정의 다중 Azure IP 로그인(`auth.sessions`), 폴링 주기 대 호출수 산수 일치(`pg_stat_statements`), 장애창에 E2E 장시간 실행 2건 in-flight, 현재 DB 정상(`ACTIVE_HEALTHY`·활성 2·커넥션 27).
- ❌ **검증 못 함(솔직히)**: ①**"E2E가 장애의 원인"이라는 인과는 증명 못 했다** — 상관·구조적 결함까지만. ②Supabase 대시보드 Reports의 CPU/메모리/디스크IO 그래프 **못 봄**(MCP로 접근 불가, PO 계정 몫) → 인스턴스 포화의 직접 지표 미확인. ③장애 당시 시간대별 요청량 **못 봄**(Supabase 로그 보관이 짧아 100행만 조회됨).
- **PR/CI 상태(2026-07-24 19:40 KST 실측)**: 열린 PR 8개 전부 실패 검사 0건 — #972·#971·#970·#969·#968·#965·#964·#956. #967·#966·#963은 main 머지 완료.
- **정리**: 진단용 임시 테이블(`public._diag_snap`, `_diag_snap2`) 생성 후 **삭제 완료**.

**7. 다음 세션 첫 프롬프트**

> 먼저 `docs/PROJECT_CONTEXT.md` 최상단 핸드오프 읽어라. 특히 4번 「주의·함정」의 🚧 항목 — DB 멈춤 원인은 **아직 확정 아님**(상관까지만 증명, 인과 미증명)이니 확정된 것처럼 쓰지 마라. 할 일: ①`.github/workflows/e2e.yml`의 동시실행 잠금을 저장소 전역 1개로 바꾸는 안을 트레이드오프(검사 느려짐) 정리해서 버튼으로 물어봐라 ②PR #969(좀비탭) 아직 열려 있는지 확인해서 알려줘 ③E2E를 프로덕션 DB에서 떼는 비용(별도 Supabase) 산출해서 버튼으로 물어봐라.
