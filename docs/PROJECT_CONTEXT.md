...
## 🔖 세션 핸드오프 (2026-06-25 직전 — 메타반성 #35 + 내일 4세션 작업계획·프롬프트)

> PO 지적: **"전부 다. 구현했다고 한 게 정상작동 안 하거나 의도와 벗어난 게 너무 많았고, 당연히 체크할 것도 못 했다."** → 한 개 버그가 아니라 *작업 방식* 반복 실패. 반성문 [POSTMORTEMS #35] 기록 + 행동 강제장치 G1~G5 + 내일 검증중심 4세션 계획 수립.

**1. 이번 세션 한 일**
- **메타반성 [POSTMORTEMS #35]** — "코드 써놓고=완료 보고 → 실제 미작동/의도이탈/체크누락" 반복 근본원인 6 + 행동 강제장치 G1~G5.
- **오늘 물증 3건 실제 확인**: ①회원가입/인증(Zoho SMTP·비번찾기·token_hash·다국어메일) 코드가 **`origin/worktree-auth-signup-test`에만 있고 main 미반영**(핸드오프엔 "했다"고 적힘) ②어제 머지 UI 6건(#334·#336·#337·#340/#342·#320) **실클릭 검증 0** ③로그인 필요 E2E smoke가 **GitHub Secrets 4개 미설정으로 전부 skip = 안전망 꺼짐**.

**2. 행동 강제장치 (오늘부터 — 어기면 #35 재발)**
- **G1 완료 정의**: 모든 "됐다"는 ①자동검사통과 ②실클릭(계정·화면) ③"검증 못 함"(이유) 중 하나로 등급 명시. 맨숭한 완료 금지.
- **G2 검증 우선**: 직전 미검증분이 신규개발보다 항상 0순위. 안 닫으면 새 기능 금지.
- **G3 E2E 켜기**: PO가 Secrets 4개 넣으면 로그인 화면 매 PR 자동 클릭검사(최고 레버리지).
- **G4 보고 전 머지/배포 확인**: 브랜치 커밋 ≠ 반영. git·Vercel로 확인 후 보고.
- **G5 큰 변경 전 의도 재진술+버튼 승인**.

**3. 다음 세션이 먼저 할 일 — 4세션 분할(영역 안 겹침, 동시 가능)**
> 각 세션 **첫 프롬프트 전문은 본 세션 채팅에 작성**(PO가 복붙). 요약:
1. **🔴 세션 A(검증, 최우선·신규개발 금지)**: 어제 머지분 실클릭 — 코디 메시지(#340/#342)·코디 흐름(#337)·에이전시 메신저(#336)·doctor제거(#334)·favicon200/재진화면(#320). G1 등급 명시.
2. **🟢 세션 B(인증/안전망)**: E2E Secrets 4개 활성화 안내(G3) + 회원가입/인증 브랜치(`worktree-auth-signup-test`) 실검증 후 main 머지. 영역: e2e/·app/auth·app/login·app/signup·app/reset-password·CI.
3. **🟡 세션 C(포털 빈데이터 P1)**: client의 service_role 직접조회 → `/api/portal/*` 서버 경유. 영역: app/coordinator·app/patient·components/healo·app/api/portal.
4. **🔵 세션 D(PO액션/정직성)**: 구글 OAuth 게시상태 / iOS 마이크 실기기 / K-01 시드 데모데이터 정직성. 코드 최소·결정 위주.

**4. 검증 상태**
- ✅ 반성문 #35·핸드오프 = 문서. 실DB·git으로 물증 3건 확인(인증 코드 main 부재·E2E secrets 게이트·머지 이력).
- ❌ 내일 4세션 작업 자체는 미착수(계획만).

**5. 다음 세션 첫 프롬프트**
> 위 3번 세션 A부터. (각 세션 전문 프롬프트는 직전 채팅 또는 PO가 보관한 것 사용.) 검증(A)으로 어제 부채부터 닫고 → B(안전망 ON)·C·D 병렬.
