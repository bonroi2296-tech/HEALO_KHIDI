...
## 🔖 진행 중 — PO 결정 즉시기록 (2026-07-11): 화상상담 통역의 핵심 니즈 = "제3자 간 외국어 대화 듣기"

> 세션 진행 중 중간저장(정식 핸드오프는 세션 종료 시). 브랜치 `claude/video-conference-improvements-empqb0`.

- **PO가 통역 기능의 우선순위를 명확히 함**: "너와 나의 대화 번역"보다 **외국어 간 대화를 제3자가 듣기 위한 번역**이 핵심. 실무 시나리오 = PO가 마이크·스피커 다 끄고 입장(하울링 방지, 동석한 카자흐 직원 기기가 대표 마이크) → 직원↔외국인의 ru/kz 대화를 자기 화면에서 한국어 자막으로 따라감. → **청취 모드**(상대가 통역 안 켜도 수신측에서 원격 음성을 자막화, 참가자별 트랙 = 화자 이름표) 구현으로 대응.
- 부수 결정: ①같은 마이크에 여러 언어 섞이는 케이스 존재 → 발화 언어 자동 감지 채택 ②1:1 무통역 직접 소통은 "아직 어렵다" 공감대 — Gemini Live Translate GA 시 재검토(기존 관망 항목과 일치) ③언어 설정은 "A ⇄ B 방향형" 멘탈모델로.
- **(추가 결정, 같은 날) 통역 스위치 하나로 통일** — PO "꼭 청취모드라고 따로 만들어야 해?" → "듣기 통역" 별도 토글 폐기. 새 규칙: 지구본 통역 ON + 마이크 ON = 내 말 송신+상대 말 수신 / 마이크 OFF = 수신(듣기)만. 부수 효과로 음소거 중 발화가 자막으로 방송되던 privacy 구멍도 해결(마이크 게이트).
- **7/10 통화 로그 194건 전수조사 결과**(재사용 가치): 실질 발화 정상률 ~46%. 원인 지분 — 문장 절단 52% > STT/언어 라우팅(echo 버그 10건) 28% > 번역 자체(심각 1건뿐). "번역 모델 문제가 아니라 파이프라인 앞단 문제"로 확정, 개선 우선순위 전면 재배치. 상세는 PR 본문·KNOWN_ISSUES 2026-07-11 항목.
