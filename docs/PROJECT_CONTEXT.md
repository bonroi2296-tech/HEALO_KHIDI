...
**4. 주의·함정**
- **(2026-07-14 중간저장, 화상상담 세션)** 실회의(카자흐 파트너, 한→러 통역 112건 실작동) 로그 분석으로 결함 3종 수리: 인앱 「브라우저에서 열기」 무반응 함정(#760 머지·배포), 권한차단 안내 상주 배너화 + 퇴장시각(left_at) webhook 기록(후속 PR). 로컬 `.env.local`의 SUPABASE_SERVICE_ROLE_KEY 는 **회전돼 죽어있음**(REST 401) — 로컬 DB 스크립트 짜기 전에 키부터 갱신할 것.
