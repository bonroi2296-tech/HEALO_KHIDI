...
>
> **📌 중간 저장 (2026-07-24, E2E 안정화 세션 — PR 스모크 간헐 실패 원인 확정)** — ①**06~07시 UTC 실패 = 공유 Supabase(프로덕션 겸용) 지연이 병목(⚠️아래 17시 칸에서 "우리 부하가 원인"은 반증으로 기각 — 증폭 요인으로 정정).** 실패 아티팩트 trace로 실측: REST 단순조회 10~25초·로그인 토큰 무응답, 서로 다른 러너 2개가 같은 분에 동시에 겪음 = 프로젝트 전체 지연(**그 분간 프로덕션 실사용자도 느렸음**). 부하원 = 스모크 1회당 UI 로그인 10회+(스펙 beforeEach마다) × 동시 E2E job 3~4개(PR 폭주+재실행). 컴파일·per-IP rate limit·cron 가설은 반증으로 기각. ②수리 = 로그인 역할당 1회(setup 프로젝트 세션 저장→쿠키 주입, 스펙 무수정) + webServer 240s + `[e2e-ui-login]` CI 가드. 반성문 #118. ③**구조 리스크(PO 판단 대상): CI와 프로덕션이 같은 Supabase인 한 PR 폭주→프로덕션 지연 전이 가능** → CI 전용 무료 프로젝트 분리가 근본 차단(비용 0, 시드 이관 필요). ⚠️함정: E2E 스모크는 `next build`가 아니라 **`next dev --webpack`(콜드 컴파일)** 대상 — "빌드 서버 검사"로 착각하지 말 것.
>
...
- **[PR #968](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/968) — 인증 무응답 시 화면이 영원히 로딩에 갇히던 것** (미머지, 전 검사 초록): `withAuthTimeout(promise, 20s)` 신설(`src/lib/supabase/browser.ts`) + **적용 6곳**(로그인·가입·비번재설정 `updateUser`/`verifyOtp`·이메일확인 `verifyOtp`·비번변경 후 재로그인) + 안내문구 `login.timeout` **6개 언어** + 역할판별 fetch 2건 `AbortSignal.timeout(10s)` + 테스트 3케이스 + **CI 가드 §20** + 반성문 #118.
- **[PR #969](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/969) — 방치된 상담방 탭이 영원히 DB를 두드리던 것** (미머지, 전 검사 초록): 유휴 연결종료 워치독(끊긴 뒤 60초 무재연결 → 폴링 정지) + `document.hidden`이면 폴링 tick 스킵(채팅·번역·자료·대기열 4곳) + 공유자료 8초→20초 + 반성문 #119 예정(현재 브랜치엔 #118로 적혀 있음 — 아래 4번 함정 참조).
...
- **반성문 번호가 병렬 세션과 계속 충돌했다** — 이 세션에서만 3번 밀었다(#116→#117→#118). 다른 세션이 #116·#117을 먼저 가져감. **#969 브랜치의 반성문은 아직 #118로 적혀 있어 #968과 겹친다 → 머지 전에 #119로 바꿔야 한다.**
...
2. **#969 반성문 번호 #118 → #119로 수정**(#968과 충돌). 그 뒤 두 PR 머지 여부는 PO 판단.
...
> 먼저 docs/PROJECT_CONTEXT.md 최상단 읽어. 로그인 장애 세션 후속이다. ①#969 반성문 번호 #118→#119로 고쳐라(#968과 겹침) ②화상 상담방 2인 통화 띄워서 통화 끝나고 60초 뒤에 폴링이 진짜 멈추는지 Vercel 로그로 확인해라 ③확인되면 #968·#969 머지 여부 물어봐. 장애 원인 재조사는 하지 마라 — 다른 세션들이 이미 했다.
