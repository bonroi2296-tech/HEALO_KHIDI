...
## 🔖 세션 핸드오프 (2026-07-15 — 실환자 운영 실무 SOP v0 초안 작성: 관계 질문 → 픽업·통역·수납 절차 + 면력 담당자 지정 요청서)

**1. 이번 세션 한 일**
- PO 질문("본로이·면력·대학병원·환자 관계를 정확히 아나?")에서 출발 → PO가 진짜 원한 건 추상 관계도가 아니라 **실환자가 왔을 때의 운영 실무 절차(SOP)**임이 드러남(픽업 차량은 면력 소유인데 누구에게 요청? 접수~퇴원 통역은 코디가 어떻게? 등).
- 탐색 3종(돈흐름·케어경로·역할RACI)으로 근거 수집 후 **`coo/04-operations/PATIENT_JOURNEY_SOP.md` v0 초안** 신설: 입국前~픽업~접수·진료·협진·입원·퇴원~귀국·사후관리 4단계 SOP + 결제 3시나리오(자비/에이전시/보험사 직접지불) + 코디 통역 교육(러↔한 암 용어집).
- **대표님 리뷰 미팅 문서화(PO 방침)** — 별도 요청서 대신 **SOP를 대표원장님과 함께 보며 교정+담당자 지정**하는 미팅 문서로 통합. SOP 상단에 「대표원장님과 함께 보는 법 + 정해주실 것」 프레임 + §4 담당자 8창구를 미팅 중 직접 기입 표(성함·연락처·가능시간)로. (`coo/04-operations/의료용어집_러한_v0.md` 코디 통역 시드도 신설.)
- 상호링크: `KHIDI_역할_프로세스_기획.md` §6(“프로세스 1바퀴 후 확정” 보류)의 실무층이 이 SOP임을 명시 + `coo/README.md` 갱신.

**2. 왜 그렇게 했는지**
- `KHIDI_역할_프로세스_기획.md`는 **디지털 계정·6대 ICT 권한층**이고, 픽업·통역·수납 같은 **사람·물리 실무층**은 비어 있었음 → 그 빈칸을 채운 것. 두 문서는 계층이 다름(중복 아님).
- 케어 경로는 **케바케로 유연**하게(면력 앵커 + 4갈래: 풀샌드위치/수술후만/한방단독/수술만) — PO가 “아직 환자를 안 받아봐서 순서를 못 박는다”고 명시.
- 대학병원은 **현행 유지**(계정 없음·게스트 링크·집계 축), 전용 포털은 “필요 시 나중에”(PO 결정).

**3. 안 끝났거나 보류 / PO가 채울 칸**
- 🔴 **면력 담당자 8창구 지정**(대표님 요청 발송) · 항공·숙소 예약 책임 주체 · 보험사 경유 수수료 회수 경로 · 리드타임/SLA 확정.
- 🟡 **본로이 실무 인력**(코디 채용/한국 상주 규모) — SOP의 “R=코디” 주체가 여기 걸림 · 번역 QA 주체.
- 러 암 의료용어집 v0 = ✅ 작성됨(`coo/04-operations/의료용어집_러한_v0.md`). PO 1차 교정 완료(코디 실재·항공숙소 케바케·번역QA/주력경로 미정 유지).

**4. 주의·함정**
- 문서 초안 = **PO 교정 대기(실환자 前 가설)**. 🔴=미지정/🟡=내 가정 표기 그대로 두고 PO가 채우게 설계함 — “확정”으로 오독 금지.
- `coo/`는 운영이 **`04-operations/`**(03은 privacy-data). 경로 혼동 주의.
- **한글문서(hwpx) 함정**: `kordoc`은 npm 패키지(`npx -y kordoc`, 매 세션 새 컨테이너라 미리 없음 — npx로 부르면 됨). 단 기능이 **①한글·PDF·워드→마크다운 파싱 ②`fill`(양식 hwpx 빈칸 채우기)** 뿐 — **자유 문서를 md→hwpx로 생성하는 기능 없음.** 자유 문서 hwpx화는 **.docx로 뽑아 한글에서 '다른이름저장→hwpx'** 가 정답(PO 승인). docx 생성 = `docx` npm(`--no-save`) + 스크래치패드 `md2docx.cjs`를 `NODE_PATH=<repo>/node_modules`로 실행, kordoc 되읽기로 검증.

**5. 다음 세션이 먼저 할 일**
1. PO 교정 반영(면력 담당자 실명·리드타임·인력 규모 등) → SOP v1로.
2. (승계) content-clip-sweep 야간 cron 첫 실행 결과 확인 / 앱스토어 결제 여부.

**6. 검증 상태**
- ✅ 문서 3종 신설·상호링크. `npm run check:content` 통과(커밋 전 확인).
- ⚠️ **미검증**: SOP 내용의 실무 정합(실환자 前이라 검증 불가 — 설계상 당연) / 🟡 가정들은 PO 교정 대기.
