...
3. **구글 비즈니스 프로필 동영상 인증 결과 확인** — 📌 **2026-07-20 PO 가 동영상 제출 완료**(구글 화면 실측: "인증을 처리하는 중입니다. 최대 5일"). **결과는 7/25 전후** → 「내 비즈니스」 검색해서 `공개되지 않음` 딱지가 사라졌는지 보고, 사라졌으면 **프로필 URL 을 `app/layout.jsx` sameAs 에 한 줄 추가**. 반려되면 사유를 보고 → healwith 명패 추가 또는 「방문 서비스 업체」 전환.
...
- ❌ **검증 못 함 — 구글 비즈니스 동영상 인증 통과 여부.** ~~PO 가 아직 촬영 전~~ → **2026-07-20 제출 완료, 구글 심사 중(최대 5일)**. 통과 여부는 7/25 전후에나 확인 가능.
