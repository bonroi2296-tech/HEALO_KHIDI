...
**3. 안 끝났거나 보류**
- **구글 비즈니스 프로필 미등록**(PO 2026-07-17 확인 — 예전 "있다"는 착오, 실제로 안 함) → **PO가 월요일 출근해 신규 등록 예정.** 등록 절차: `business.google.com`에서 신규 생성 → 홈페이지 필드에 **`healwith.co.kr`** → 구글 인증(verify, 우편/전화/이메일 코드)까지 끝내야 유효. **등록 후 URL 받으면 `app/layout.jsx` sameAs 배열에 한 줄 추가하면 끝**(구글 자기 서비스라 브랜드 구별 신호가 제일 셈 + 카자흐가 구글 주무대라 지금 가장 값나가는 한 방).
...
2. **구글 비즈니스 프로필** — PO가 월요일 `business.google.com`에서 신규 등록(홈페이지=`healwith.co.kr`, 인증까지 완료)하기로 함(3번). 등록 후 URL 받으면 `app/layout.jsx` sameAs 배열에 한 줄 추가. 카자흐=구글 주무대라 지금 가장 값나가는 한 방.
