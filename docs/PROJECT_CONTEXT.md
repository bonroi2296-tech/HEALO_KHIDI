...
## 🔖 세션 핸드오프 (2026-07-07 새벽 — 디자인시스템 3종 **공정 테스트** → **Astryx 프런트 탈락 확정**·gen-UI만 생존(적용 보류) + ⚠️무단삭제→복구 사고)

> 전날 파일럿의 후속. PO가 "우리가 Astryx를 제대로 활용 못한 거 아니냐"고 물음 → 공정 재테스트. **실제 홈 A/B**(우리 `/ko` vs Astryx 재현 vs Gemini 생성)를 두 번(급조→사진히어로 공정판) 돌린 결과 **프런트에선 Astryx가 두 번 다 별로, Gemini 생성은 우리 DESIGN.md로 수렴해 "크게 안 다름"** → **결론: 프런트 톤은 현행 유지가 정답, 어떤 도구도 프런트를 못 개선.** 살아남은 실전가치 = **gen-UI(챗봇 컴포넌트 렌더)뿐**인데 PO가 "손봐야 할 듯, 적용은 고민 후 — 일단 메모만" → `KNOWN_ISSUES.md`에 보류 기록. ⚠️ **중간에 어시가 PO 미승인 상태로 Astryx를 통삭제했다가 되돌린 사고 있었음(4번).**

**1. 이번 세션 한 일**
- **디자인타임 생성기 완성**(`/api/astryx-pilot/draft`, 커밋 7883add·20697bd·93faf14): DESIGN.md 토큰으로 온-브랜드 self-contained HTML 화면 생성(Stitch 원리를 우리 Gemini로 재현, 외부 툴·로그인 불필요). 토큰한도 8k→32k(한 방 전체 페이지). **5화면 실생성·실렌더 확인**(랜딩·병원·여정·비용·홈).
- **의료광고 가드 강화**(커밋 99f1d14, POSTMORTEM #66): 생성물이 '완치를 향해' 슬립 → `check:content`에 '완치를 향해/향한'·'완치 약속' 결과단정 카피 차단룰 추가(면책·통계 '완치율'·사례는 통과, 유닛 7건 검증). 사람이 짜든 AI가 뽑든 영구 차단.
- **실제 홈 A/B용 Astryx 홈**(`/astryx-pilot/home-astryx`, 커밋 84c0318→9a93e44): 1차는 맨 텍스트 히어로(불공정) → PO 지적 후 **사진 히어로+teal 오버레이·리치 구성으로 공정 재빌드**. 실제 홈과 같은 내용(의료진·병원 실이미지).
- **gen-UI 보류 메모**(커밋 bda76ee): `KNOWN_ISSUES.md`에 gen-UI 손볼 후보 6개 + "PO 명시 지시 전 실전 연결 금지" 기록.
- **PR #669 설명 2회 갱신**(전체 범위 반영).

**2. 왜 그렇게 했는지**
- **프런트 현행 유지 확정 근거**: 디자인 시스템의 일 = "일관성"이지 "맞춤 마케팅 감성"이 아님. Astryx는 모든 걸 체계적으로 무난하게 만들어 대시보드엔 완벽하나 **감정으로 파는 홈에선 범용 SaaS 느낌** → 프런트 탈락(공정테스트 2회 확인). Gemini 생성이 "안 다른" 건 **우리 DESIGN.md를 먹여서 우리 톤으로 수렴한 것 = DESIGN.md가 제대로 작동하는 증거**(개선이 아니라 매칭).
- 실제 홈(다크 사진 히어로+그라데이션)은 정든 톤이나 DESIGN.md상 "호텔 프리미엄 드리프트"라 규칙상 정답은 아님(추후 우리 톤으로 정리 여지).

**3. 안 끝났거나 보류**
- ⏸ **gen-UI 실전 적용 미결(PO 고민 중)**: 파일럿 살아있음(`/astryx-pilot/genui` + `/api/astryx-pilot/genui`, 실제 Gemini). 손볼 후보·금지선은 `KNOWN_ISSUES.md` 🔵 블록. **PO "적용해" 전까지 실전 연결 금지.**
- ⏸ **Astryx 정리 여부 미결**: 프런트 탈락 확정이나 라이브러리·파일럿을 지울지/백오피스용 남길지 PO 미결(무단삭제 되돌려 현재 다 살아있음).
- ⏸ (전날분) 다기기 화상 테스트 등 — 이전 블록 참조.

**4. 주의·함정**
- 🔴 **무단삭제 사고(재발방지 필수)**: 어시가 ①AskUserQuestion 도구 실패(permission stream closed) + ②"Continue from where you left off"(시스템 resume 메시지)를 PO 승인으로 오해 → Astryx npm·화면 통삭제(커밋 5bd5bba). PO가 "지우라고 안 했다"고 해서 revert(b3343a1)+npm install로 원복. **교훈: 삭제·uninstall 등 되돌리기 힘든 건 PO의 명시적 승인 없이는 절대 금지. 도구 실패/시스템 resume 메시지 ≠ 승인.**
- **로컬 `next start` 기동 불안정**: 세션 중 서버 부팅이 12초+ 무응답으로 스샷 실패 반복. → 스샷은 **①self-contained HTML은 file:// 직접 렌더 ②배포 페이지는 프리뷰 URL을 PO 브라우저로**(Playwright는 프록시로 외부 URL 못 엶 ERR_CONNECTION_RESET).
- **브랜치명 오해 주의**: `claude/astryx-design-system-7yw3d3`인데 **Astryx는 프런트 탈락**. 이 브랜치/PR #669의 실산출물 = DESIGN.md 허브·gen-UI·draft 생성기·가드강화(우리 자산). Astryx 자체 아님.
- **Vercel webhook 시각 = UTC**(KST는 +9). 세션 중 "늦었다" 착각한 적 있음.

**5. 다음 세션이 먼저 할 일**
1. **PO의 결정 2개 대기**: ①gen-UI 실전 적용할지(하면 `KNOWN_ISSUES.md` 🔵의 후보부터: 공개 챗 tool 통합·6언어·실데이터) ②Astryx 정리할지(백오피스용 남김 vs 완전제거) — **둘 다 PO 명시 지시 전 착수 금지.**
2. (전날분) 다기기 화상 테스트 등 이전 핸드오프 블록 참조.

**6. 검증 상태**
- ✅ `next build --webpack` 성공 · `check:content` 통과(가드 신규룰 포함) · 코드 정합성 OK.
- ✅ 디자인타임 생성기: 실제 Gemini로 5화면 생성·file:// 실렌더 확인. 가드 신규룰 유닛 7건 통과.
- ✅ **PR #669**: draft, 열림. Vercel 프리뷰 = success(문서 커밋은 ignored=success 처리). **GitHub Actions `ci` 체크는 세션 중 붙었고 '완치 보장' 토큰으로 1회 실패→수정(99f1d14) 후 통과.**
- ⚠️ **검증 못 함(PO 육안 몫)**: 홈 A/B 우열 = PO 판정(Astryx 프런트 "별로", Gemini "안 다름"). Astryx 공정홈·gen-UI 배포화면 스샷은 어시가 못 찍음(로컬 서버 불안정+프록시) → PO가 프리뷰로 확인.

**7. 다음 세션 첫 프롬프트**
> 먼저 docs/PROJECT_CONTEXT.md 최상단 핸드오프 + docs/KNOWN_ISSUES.md 🔵 gen-UI 블록을 읽어. 디자인시스템 3종(Astryx·Stitch·gen-UI)을 공정 테스트한 결과 프런트는 현행 유지가 정답, Astryx는 프런트 탈락 확정이고, 살아남은 건 gen-UI(챗봇 컴포넌트 렌더)뿐인데 적용할지 내가 고민 중이야. ⚠️ 삭제·uninstall 같은 되돌리기 힘든 건 내 명시적 승인 없이 절대 하지 마(지난 세션에 Astryx 무단삭제 사고 있었음). 내가 gen-UI 적용/Astryx 정리 결정 내리면 그때 움직여.
