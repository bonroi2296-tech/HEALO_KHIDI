...
> **📌 중간 저장 (2026-07-24 17시대, 로그인 장애 세션 — ⚠️Supabase 인스턴스 무응답 = 프로덕션 로그인 전면 중단)** — PO "로그인이 '로그인 중…'에서 멈춘다, 누가 또 고장냈냐" → **실측 결과 코드 무관, Supabase 인스턴스가 통째로 무응답**. 근거: GoTrue `POST /token` → **504 request_timeout**("context deadline exceeded", 13.2초) · `/rest/v1/*` → 90초 뒤 522 · `/auth/v1/health` → 무응답(17:12까지 계속) · MCP SQL 2회 connection timeout · 반면 **DB 안 타는 `/storage/v1/*`는 0.06초 정상**(= CF·네트워크 아님, **Postgres 붙는 서비스만 죽음**). Supabase 전사 status는 정상(우리 인스턴스만), 컨트롤플레인은 `ACTIVE_HEALTHY`로 **거짓 안심**(제어 API만 봄). **PO 조치 = [Supabase 대시보드](https://supabase.com/dashboard/project/hvwwlkawaxabhtumjhrg) → Project Settings → Compute and Disk → Restart project** + Reports→Database에서 디스크·CPU 확인(원인 규명). ⚠️**멈춘 진짜 원인(디스크풀/CPU/커넥션)은 DB에 못 붙어서 밖에서 확인 불가 — 추측 금지**. 코드 측 결함은 별건으로 수리: 무응답 시 화면이 영원히 로딩에 갇히던 것 → `withAuthTimeout`(20초) 6곳 + 가드 §20 + 반성문 #116 (브랜치 `work/login-fix`).
