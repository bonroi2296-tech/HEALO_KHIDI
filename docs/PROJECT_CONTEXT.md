...
> **📌 중간 저장 (2026-07-24 저녁 — 실회의 2건 후속 «다음 주에 이어서» 메모, PO 요청)** — PO: *"오늘 회의 2건 했어. 이거 관련해서도 얘기할 거 있는데 일단 정리해봐, 다음 주에 다시 하게 메모 남겨줘."* → **PO가 말할 회의 내용(안건·상대·결정)은 아직 안 들었다. 다음 주 세션은 그걸 먼저 PO에게 물어 이 블록을 채워라.** 지금까지 확인된 **사실만**:
> - **회의①** 방 `23275b3c`(18:00 예정) — 실제 **17:53~18:19(26분)**, 게스트 3명(`healwith_moon`·`Assel`·`Жанат`). 같은 방에서 낮 **14:10~14:44(34분)** moon+Assel 세션도 있었음.
> - **회의②** 방 `61d68d3a`(18:30 예정, 이날 새로 생성) — `Assel` 18:38~18:54, `Radmila` 19:01~19:07.
> - **자막 파이프라인은 실동작함**: 회의① ru→ko **181줄**, 회의② ru→ko 18줄·ko→ru 4줄(양방향). 단 회의① 실구간엔 **ko→ru 0줄**(원인 미확정 — 핸드오프 6번 참조).
> - 🔴 **그날 발견한 큰 것: 이 회의들이 KHIDI 실적에 한 건도 안 잡힌다.** 사전상담 집계 조건은 `completed` + `inquiry_id` 연결인데 **전체 42세션이 전부 문의 미연결, completed는 1건뿐** → K-02 자동집계 구조적 0. 배선 버그는 아니고(코드·드롭다운 실측) **«문의 선택 안 하고 생성 + 완료 안 누름»** 운영 흐름 문제. 상세·고칠 방향 = `docs/KNOWN_ISSUES.md` 최상단. **8/27 중간평가 점수와 직결이라 다음 주 1순위 후보.**
