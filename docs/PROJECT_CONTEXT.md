...
## 🔖 세션 핸드오프 (2026-07-15 — 실환자 운영 SOP(v0.4) + 보험사 GOP 정산 리스크 + 보험사 미팅 질문 체크리스트)

**1. 이번 세션 한 일** (전부 PR [#776](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/776) draft, 문서만·실서비스 무관)
- **`coo/04-operations/PATIENT_JOURNEY_SOP.md` (v0.4)** 신설: 실환자 여정 4단계(입국前~픽업~접수·진료·협진·입원·퇴원~귀국·사후관리) + **§6 "삐끗할 때·안전" 10개 섹션**(응급·야간 에스컬레이션 / 수용·거절 판정 / 보증금·노쇼·견적변동 / 국외이전 동의 / 할랄·종교·성별 / 지연 플랜B / 컴플레인 / KHIDI 증빙 자동수집 / 1인 백업 / **§6.10 보험사 GOP 정산 리스크**). 상단에 **「대표원장님과 함께 보는 미팅 문서」 프레임** + §4 면력 담당자 8창구 미팅 중 직접 기입 표.
- **`coo/04-operations/의료용어집_러한_v0.md`** — 코디 통역 러↔한 암 의료용어집 시드(원어민+의료진 검수 前 실사용 금지 배너).
- **`coo/04-operations/보험사_미팅_질문체크리스트.md` (v0.1)** — 보험사에 **"너희 정책 어떠니? 이거 해줄 수 있니?"** 완곡 질문 6축(GOP·결제·청구·보장·관계·내부판단) + PO 선택 5질문 승격(예상환자수·의료사고커버·취소노쇼·한국조건·최소볼륨).
- **`scripts/md2docx.cjs`** — md→한글워드(.docx) 변환기(맑은고딕). SOP·용어집 docx도 `coo/04-operations/`에 저장.
- 상호링크: `KHIDI_역할_프로세스_기획.md` §6·`coo/README.md` 갱신.
...
**5. 다음 세션이 먼저 할 일**
1. ⚠️ **직전 미검증분**: SOP·체크리스트 내용은 실환자·실미팅 前이라 **실무 정합 검증 불가** = 첫 환자/미팅 결과로만 확정. PO가 대표님·이대IPD·보험사 미팅 다녀왔으면 그 결과로 🔴 칸 채워 **SOP v1**으로 굳힘.
...
**7. 다음 세션 첫 프롬프트**
> docs/PROJECT_CONTEXT.md 최상단 읽어. 2026-07-15에 만든 실환자 운영 SOP(coo/04-operations/PATIENT_JOURNEY_SOP.md)·보험사 미팅 질문 체크리스트가 PO 교정 대기 중이야. PO가 대표님·이대서울 IPD·보험사 미팅 다녀왔으면 그 결과로 🔴 미지정 칸(면력 담당자 8창구·응급연락·보험 정책·GOP 체크리스트 등) 채워 SOP v1로 굳혀. 문서 필요하면 scripts/md2docx.cjs로 docx 뽑아 SendUserFile로 보내(다운로드가 PO PC 저장 — 이 세션은 클라우드라 직접 저장 불가). 그다음 content-clip-sweep 야간 cron·앱스토어 결제 확인.
