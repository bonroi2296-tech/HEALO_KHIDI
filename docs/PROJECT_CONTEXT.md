...
- **조치를 안 한 이유** — PO가 "다음 주(2026-07-27~)에 마저 하게 메모 잘 해놓고 핸드오프해"라고 명시적으로 이월 지시. 진단 결과만 남기고 손 안 댐.

**3. 안 끝났거나 보류**

- 🔴 **E2E 전역 동시실행 상한 추가 = 미조치(2026-07-27 주 최우선).** `#967`(로그인 횟수 감축)이 머지됐고 `#969`(좀비탭)가 대기 중이지만, **"22갈래가 동시에 같은 DB로 들어가는 구조" 자체는 그대로**다. 아무도 안 건드리는 유일한 구멍.
