...
- **[PR #969](https://github.com/bonroi2296-tech/HEALO_KHIDI/pull/969) — 방치된 상담방 탭이 영원히 DB를 두드리던 것** (미머지, 전 검사 초록): 유휴 연결종료 워치독(끊긴 뒤 60초 무재연결 → 폴링 정지) + `document.hidden`이면 폴링 tick 스킵(채팅·번역·자료·대기열 4곳) + 공유자료 8초→20초 + 반성문 #119.
