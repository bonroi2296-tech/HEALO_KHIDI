...
>
> **📌 중간 저장 (2026-07-24, E2E 안정화 세션 — PR 스모크 간헐 실패 원인 확정)** — ①**중요 발견: 06~07시 UTC 실패의 진범 = 공유 Supabase(프로덕션 겸용) 포화.** 실패 아티팩트 trace로 실측: REST 단순조회 10~25초·로그인 토큰 무응답, 서로 다른 러너 2개가 같은 분에 동시에 겪음 = 프로젝트 전체 지연(**그 분간 프로덕션 실사용자도 느렸음**). 부하원 = 스모크 1회당 UI 로그인 10회+(스펙 beforeEach마다) × 동시 E2E job 3~4개(PR 폭주+재실행). 컴파일·per-IP rate limit·cron 가설은 반증으로 기각. ②수리 = 로그인 역할당 1회(setup 프로젝트 세션 저장→쿠키 주입, 스펙 무수정) + webServer 240s + `[e2e-ui-login]` CI 가드. 반성문 #116. ③**구조 리스크(PO 판단 대상): CI와 프로덕션이 같은 Supabase인 한 PR 폭주→프로덕션 지연 전이 가능** → CI 전용 무료 프로젝트 분리가 근본 차단(비용 0, 시드 이관 필요). ⚠️함정: E2E 스모크는 `next build`가 아니라 **`next dev --webpack`(콜드 컴파일)** 대상 — "빌드 서버 검사"로 착각하지 말 것.
>
