...
## 🔖 세션 핸드오프 (2026-06-17) — 면력 사진 self-host + 법률 번역 + 하네스 개선
...
- (진행 중, 이 핸드오프 커밋과 함께) docs 단일 SoR 일원화 + `/bug` 비활성.
...
**안 끝났거나 보류 (PO 결정 대기):**
- **specialty 페이지**(치과·성형·피부 = 옛 종합의료관광 잔재): 유지 vs 내림 미결.
- **경쟁사(CloudHospital) 개선안** 순서 미결.
- **도메인 `healwith.co.kr` 등록 + 메일함**(admin@healwith.co.kr 수신): PO 행정작업(미완 — 메일함 없어 지금 그 주소 수신 안 됨).
- **메신저 4채널 URL**(WhatsApp·Telegram·WeChat·LINE): PO 가입 후 env 등록 필요.
- **이대서울 대표사진**: 위키미디어에 없음 → PO 실사진 제공 필요. 갤러리 2~5·이대서울 사진은 출처 불명(권리 확인 필요).
- **§15 정확한 조문**: 변호사/법령정보센터 확정 필요.
- **법률 번역 현지 변호사 최종검토**(특히 RU·KZ).
...
**다음 세션이 먼저 할 일:**
1. PO 결정 받기: specialty 페이지·경쟁사 순서·사진 권리.
2. 도메인/메일 풀리면 `docs/DOMAIN_CUTOVER_healwith.md` 실행.
3. KHIDI 중간평가(8/27) 상시 기준 — `docs/KHIDI_중간보고_베이스.md`.

**검증 상태:** PR #50~#56 전부 CI(ci·smoke·Vercel) 초록 + 머지 완료. `check:content`·`next build --webpack` 통과 확인. **라이브 클릭 검증(실기기 화상상담·문의폼 등)은 PO 몫 — 미검증.**
