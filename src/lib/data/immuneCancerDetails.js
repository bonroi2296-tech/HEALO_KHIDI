...
export const CANCER_DETAILS = {
  ...
  digest: {
    ...
    //       → 원문 표기를 «그대로» 옮긴다. 「~로 표기합니다」 같은 껍데기는 붙이지 마라
    //          (2026-09-01 PO: *"그거 왜 그케 써야 하는건데?"*). 누가 한 말인지는 바로 아래
    //          출처 줄이 이미 말하고 있어서, 문장 안에 또 넣으면 같은 말을 두 번 하는 것이고
    //          헤드라인이 공지문처럼 읽힌다. **이 칸은 원문 문장만, 출처는 출처 줄에.**
    //    ⚠️ 숫자·의학적 타당성은 우리가 판단하지 않는다.
    //    ✅ 2026-09-01 PO 결정 ①: **면력 관련 의학 정보의 기준은 immunehospital.com 표기다.**
    //       그래서 위 「원문 그대로 옮긴 문장」이 곧 정본이고, 별도 회신을 기다릴 것이 없다.
    //       바꿀 일이 생기면 «원문이 바뀌었을 때»뿐 — 그때도 우리가 해석해 고치지 말고
    //       바뀐 원문 표기를 그대로 옮기고 확인일을 갱신해라(출처 줄의 «2026-04 확인» 부분).
    //    ✅ 2026-09-01 PO 결정 ②: 출처 줄에 **「healwith 가 독자적으로 검증한 수치가 아닙니다」류의
    //       면책 문장을 붙이지 않는다** (PO: *"이런 쓸데 없는 말은 하지 마"*). 되살리지 마라.
    //       남기는 것은 «누가 한 말인지 + 어디서 왔는지»뿐 — 병원 이름·확인일·참고논문·원문 링크.
    stats: {
      survivalImprovement: { ko: "말기 위암 수술 후 한방+항암 병행치료 시 생존율 54%", en: "54% survival with combined Korean medicine and chemotherapy after surgery for advanced gastric cancer", ru: "Выживаемость 54% при сочетании корейской медицины и химиотерапии после операции по поводу рака желудка поздней стадии", kz: "Асқынған асқазан обыры бойынша операциядан кейін корей медицинасы мен химиотерапияны қатар қолданғанда өмір сүру — 54%", zh: "晚期胃癌术后韩方与化疗并行治疗，生存率54%", ja: "進行胃がんの術後に韓方と抗がん剤を併用した場合、生存率54%" },
      survivalImprovementSource: { ko: "출처: 면력한방병원 공식 사이트 (2026-04 확인). 해당 페이지가 밝힌 참고논문 — Rao X.Q. et al. (1994), CJITWM 14(6):366.", en: "Source: Immune Hospital official website (retrieved April 2026). Reference cited on that page — Rao X.Q. et al. (1994), CJITWM 14(6):366.", ru: "Источник: официальный сайт Иммунной клиники (проверено в апреле 2026). Работа, указанная на той странице, — Rao X.Q. et al. (1994), CJITWM 14(6):366.", kz: "Дереккөз: Иммундық клиниканың ресми сайты (2026 жылғы сәуірде тексерілді). Сол беттегі сілтеме — Rao X.Q. et al. (1994), CJITWM 14(6):366.", zh: "来源：免疫医院官方网站（2026年4月确认）。该页面所引文献 — Rao X.Q. et al. (1994), CJITWM 14(6):366。", ja: "出典：免疫病院公式サイト（2026年4月確認）。同ページが挙げる引用文献 — Rao X.Q. et al. (1994), CJITWM 14(6):366。" },
