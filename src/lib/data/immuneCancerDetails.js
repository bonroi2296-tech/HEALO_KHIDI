...
export const CANCER_DETAILS = {
  ...
  digest: {
    ...
    stats: {
      survivalImprovement: { ko: "말기 위암 환자 수술 후 한방 병행치료 시 생존율 54% 개선", en: "54% improvement in survival when combining Korean medicine with treatment after surgery for advanced gastric cancer patients", ru: "Улучшение выживаемости на 54% при сочетании корейской медицины с лечением после операции у пациентов с поздним раком желудка", kz: "Асқынған асқазан обыры пациенттерінде операциядан кейін корей медицинасын қатар қолданғанда өмір сүру 54%-ға жақсарады", zh: "晚期胃癌患者术后联合韩医治疗，生存率改善54%", ja: "進行胃がん患者で術後に韓方併用治療を行うと生存率が54%改善" },
    },
  },
...
export const CANCER_FAQ = {
  female: [
    ...
    { q: { ko: "통역 서비스가 제공되나요?", en: "Are interpreter services available?", ru: "Есть ли услуги переводчика?", kz: "Аударма қызметі бар ма?", zh: "提供翻译服务吗？", ja: "通訳サービスはありますか？" }, a: { ko: "healwith 코디네이터가 러시아어·카자흐어 통역을 지원합니다. 진료 동행도 가능합니다.", en: "healwith coordinators provide Russian and Kazakh interpretation, including accompaniment to appointments.", ru: "Координатор healwith обеспечивает перевод на русский и казахский языки, включая сопровождение на приём.", kz: "healwith үйлестірушісі орыс және қазақ тілдеріне аударма жасайды, қабылдауға еріп баруды қоса.", zh: "healwith协调员提供俄语、哈萨克语翻译，并可陪同就诊。", ja: "healwithコーディネーターがロシア語・カザフ語の通訳を行います。診療への同行も可能です。" } },
