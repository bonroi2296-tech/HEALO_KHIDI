 ...
 *   상태»였다 — 상담 단계에서 24일·32일 무동작. 코디 목록엔 「⏰ N일째 정체」 배지가 «이미» 있는데
 *   (caseDelayDays, 상담 7일 기준) 그 배지를 달고도 몇 주가 갔다 = «보러 가야 보이는» 표시만으론
 *   안 움직인다. 침묵 감지 크론(detect-silent-patients)은 치료 «후» 환자의 증상 보고만 본다.
 *   그래서 «보러 가지 않아도 울리는» 길(매일 종 알림)을 따로 둔다. 이 모듈은 그 판정의 순수 함수다.
 *   배지와 다른 점: 여기선 후속글·케이스이력·상담세션까지 «움직임»으로 세므로 더 보수적이다.
 *
