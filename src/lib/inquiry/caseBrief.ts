...
function buildPrompt(lang: BriefLang, withImaging = false): string {
  ...
  return [
    ...
    // 🔴 2026-09-10 추가. 「환자에게 보낼 서류」 칸에는 «병원이 보내온 회신»이 들어온다(문의 #316 이대
    //   러시아어 견적서). 그 파일 이름 앞에 「[병원 회신]」이 붙어 온다 → 모델이 그것을 «환자가 한 말»과
    //   섞으면 누가 한 말인지 뒤집힌다. request 칸은 환자가 말한 것만 담는 자리라 특히 위험하다.
    `HOSPITAL REPLIES: some attached documents are named with a leading marker meaning "hospital reply" (Korean: 「[병원 회신]」). Those are documents the HOSPITAL sent back to us — a quote, a second opinion, an admission plan. They are NOT the patient's words. Never let their content leak into \`request\` (which holds only what the patient stated). Put what the hospital proposed into \`points\`, and say plainly that it came from the hospital (the ${L} equivalent of "the hospital replied: ..."), naming the hospital only if the document states it.`,
    // ⚠️ 돈 숫자는 AI 가 옮기다 틀리면 환자에게 잘못된 금액이 나간다. 요약은 «있다는 사실»까지만.
    `MONEY (hard): if a hospital reply contains prices, do NOT restate the figures as fact in the brief. State only that a quote arrived and what it covers (surgery / tests / room), then add one short clause telling the reader to confirm the amounts in the original document (the ${L} equivalent of "check the amounts in the original"). Exception: you may give ONE overall range if the document itself presents it as a range, and you must mark the currency exactly as written. Never convert currencies.`,
