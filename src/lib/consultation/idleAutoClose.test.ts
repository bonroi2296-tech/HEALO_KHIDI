 ...
 *
 * 왜 테스트가 필요한가: 이 로직은 «5분/10분 뒤에 일어나는 일»이라 사람이 손으로 확인하려면
...
describe("자리 비움 자동 종료", () => {
  ...
  it("통화 중 혼자: 10분 뒤 묻고, 2분 더 무응답이면 끊는다", () => {
    const onAsk = vi.fn(), onClose = vi.fn();
  ...
  it("«네, 있어요»를 누르면 처음부터 다시 세고, 눌러둔 동안엔 안 끊긴다", () => {
    ...
    vi.advanceTimersByTime(10 * 60 * 1000);   // 1회차 질문
    expect(onAsk).toHaveBeenCalledTimes(1);
    w.stay();                                  // 사용자가 «있어요» 누름
    vi.advanceTimersByTime(2 * 60 * 1000);
    ...
    vi.advanceTimersByTime(8 * 60 * 1000);     // 누른 시점부터 10분
  ...
  it("대기 중이던 사람이 입장 승인되면 통화 규칙(10분)으로 갈아탄다", () => {
    const onAsk = vi.fn(), onClose = vi.fn();
    const w = createIdleWatcher("waiting", { onAsk, onClose });

    vi.advanceTimersByTime(4 * 60 * 1000);     // 아직 5분 전
    w.setMode("inRoom");                        // 승인되어 입장
    vi.advanceTimersByTime(5 * 60 * 1000);     // 대기 규칙이었다면 이미 물었을 시점
    expect(onAsk).not.toHaveBeenCalled();

    vi.advanceTimersByTime(5 * 60 * 1000);     // 입장 시점부터 10분
    expect(onAsk).toHaveBeenCalledTimes(1);
  });
});
