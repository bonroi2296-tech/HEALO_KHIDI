...
const CURE_CLAIM: RulePattern[] = [
  // ko: 완치 보장/가능/확실/시켜 · (완전히) 암을 치료/제거 · 암이 없어짐/사라짐/근치 · 반드시 낫 · 100% 완치
  //
  // ⚠️ 2026-08-03 전면 재작성(반성문 #167). 옛 패턴은 `암(?:을|이|은)?\s*(?:완전히\s*)?(?:치료|제거…)` 로
  //   **「완전히」가 선택(?)** 이라, "대장암 치료"·"항암 치료"·"암을 제거하는 수술" 같은 **가장 흔한 정상
  //   의학 용어**를 완치 주장으로 잡았다(실DB 437건 중 131건=30% 오탐, 그중 진짜 완치 주장 0건).
  //   비스트리밍 경로는 그 정상 답변을 통째로 코디 이관 문구로 갈아치우고 있었다.
  //
  ...
  //   ② 한국어는 부사를 앞으로 뺀다("완전히 암을 제거합니다") → **부사 앞/뒤 두 방향 다** 잡는다.
  ...
  { flag: "cure_claim", re: /완치\s*(?:를)?\s*(?:보장|확실|가능|됩니다|된다|시켜|시킵)/ },
  // 총체성 부사 + 치료/제거 — 부사가 대상 뒤에 오는 어순
  { flag: "cure_claim", re: /(?:암|암세포|종양)(?:을|를|이|가|은|는|도)?\s*(?:완전히|싹|깨끗이)\s*(?:치료|제거|없애|사라지)/ },
  // 같은 주장, 부사가 앞으로 빠진 어순("완전히 암을 제거합니다")
  { flag: "cure_claim", re: /(?:완전히|싹|깨끗이)\s*(?:암|암세포|종양)(?:을|를|이|가|은|는|도)?\s*(?:치료|제거|없애|사라지)/ },
  // 없애/없어지/사라지/근치는 부사 없이도 완치 뉘앙스(축약형 포함).
  //   단 "근치적 절제술·근치술·근치 수술·근치 요법·근치율"은 **정식 술기·통계 용어**라 제외.
  { flag: "cure_claim", re: /(?:암|암세포|종양)(?:을|를|이|가|은|는|도)?\s*(?:(?:완전히|모두|싹|깨끗이|전부)\s*)?(?:없[애앱앨앴앤]|없어[지집졌질]|사라[지집졌질]|근치(?!적|술|율|\s*(?:절제|수술|요법|치료|방사선)))/ },
  { flag: "cure_claim", re: /(?:반드시|무조건|틀림없이)\s*(?:낫|완치|치료)/ },
  { flag: "cure_claim", re: /100\s*%\s*(?:완치|치료|회복|제거)/ },
