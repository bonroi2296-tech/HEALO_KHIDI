...
const CURE_CLAIM: RulePattern[] = [
  ...
  // ⚠️ "치료·제거"는 총체성 표현(완전히·모두·싹…)이 붙을 때만 완치 주장이다.
  //   2026-08-03 실측: 부사 없이 잡던 옛 패턴이 "대장암 치료"·"항암 치료"·"암을 제거하는 수술"
  //   같은 정상 의학 용어를 과거 응답 437건 중 131건(30%)에서 오탐했고, 그중 진짜 완치 주장은 0건.
  //   비스트리밍 경로는 그 정상 답변을 통째로 코디 이관 문구로 갈아치우고 있었다(반성문 #96).
  { flag: "cure_claim", re: /암(?:을|이|은)?\s*(?:완전히|모두|싹|깨끗이|전부)\s*(?:치료|제거|없애|사라지|근치)/ },
  // 없애/사라지/근치는 부사 없이도 완치 뉘앙스 — 단 "근치적 절제술·근치술"은 정식 술기명이라 제외.
  //   ※ 한국어 축약형까지 받는다: "사라집니다"엔 '사라지'가, "없앱니다"엔 '없애'가 글자로 없다
  //     (옛 패턴이 못 잡던 구멍 — 2026-08-03 테스트 추가하다 발견).
  { flag: "cure_claim", re: /암(?:을|이|은)?\s*(?:없[애앱]|사라[지집]|근치(?![적술]))/ },
