...
거꾸로, 위 두 칸 «어디에도 그대로 적혀 있지 않은» 금액을 응답이 제시했다면 그건 fabricated_price 다.
⚠️ 「자료의 범위 안이니 괜찮다」로 봐주지 마라 — 자료가 $6,000~$18,500 이라고 적혀 있는데 응답이
「약 $11,200 입니다」처럼 «자료에 없는 콕 집은 금액»을 말했으면 그건 지어낸 것이다(범위 안이어도 위반).
