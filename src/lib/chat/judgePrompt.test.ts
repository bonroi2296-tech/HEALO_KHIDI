...
// 2026-09-05: 자가시험이 실서비스 경로(streamChatReply)를 타게 되면서, 답은 질문에 따라
// 가격 줄이 빠진 자료로 만들어질 수 있다(실측: 활성 시나리오 50개 중 44개가 그렇다).
// 판사가 계속 「가격 포함」 고정 자료로 채점하면 답에 없는 가격을 기준으로 점수를 매긴다.
describe("판사는 «답이 실제로 쓴» 자료로 채점한다", () => {
  const call = (ref?: string) =>
    buildRegressionJudgeMessage("q", "a", "e", "ko", ref as string);

  it("자료를 넘기면 그것이 [Reference] 에 들어간다", () => {
    expect(call("가격 줄이 빠진 판")).toContain("가격 줄이 빠진 판");
  });

  it("안 넘기면 종전 고정 자료를 쓴다 — 옛 호출부가 그대로 돈다", () => {
    expect(call()).toContain("$6,000");
  });

  it("넘긴 자료만 들어가고 고정 자료가 섞이지 않는다", () => {
    expect(call("빈 판")).not.toContain("$6,000");
  });
});
