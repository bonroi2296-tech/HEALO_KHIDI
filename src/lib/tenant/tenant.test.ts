...
describe("판의 콘텐츠 위생 — 사람 눈 대신 기계가 잡는다", () => {
  ...
  it("⚖️ 치료 효과를 단정하는 문구가 없다 (의료법 제56조)", async () => {
    ...
    const 금지 = [
      ...
      "100% 안전", "치료경험담", "효과가 입증",
      // 2026-07-29 추가 — 병원 사이트를 더 크롤하다 **원문에서 새로 발견한** 표현들.
      // 출처: `/pages/cancer/home.php` (「항암치료율 향상」·「항암부작용 감소」·「암세포 증식 억제」)
      //      `/pages/rehab/postop.php` (「효과를 극대화」·「회복을 촉진」) — 옮길 뻔한 문장들이라 미리 막는다.
      // 지금 우리 글에는 하나도 없다(전부 0건) — 즉 이건 «있는 걸 잡는 검사»가 아니라 «들어오는 걸 막는 문»이다.
      "극대화", "항암치료율", "부작용 감소", "부작용을 줄", "재발을 막", "재발 억제",
      "면역력을 높", "면역력 증진", "회복을 촉진",
    ];
