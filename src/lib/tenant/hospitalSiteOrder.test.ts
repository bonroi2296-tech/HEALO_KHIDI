...
describe("가로로 넘기는 줄 — 「더 있다」고 티만 내지 않는다", () => {
  /* 2026-07-30 PO 지적: *"이거 뭐 우측에 영상 더 보일것처럼 안보여주는건 뭐임?"*
     이 줄은 스크롤바를 일부러 숨긴다(휴대폰엔 맞는 선택). 그런데 화살표가 없어서
     **마우스 쓰는 사람에겐 넘길 방법이 하나도 없었다** — 오른쪽 카드만 반쯤 잘려 보이는 상태.
     「덜 만든 화면」이 아니라 「고장난 화면」으로 읽힌다.
     ⚠️ 스크롤바를 다시 숨기면서 화살표를 지우면 그 상태로 되돌아간다 → 둘을 한 검사로 묶는다. */
  const 줄 = fs.readFileSync("src/components/hospital-template/motion.jsx", "utf8");

  it("스크롤바를 숨기면 반드시 화살표가 있어야 한다", () => {
    const 스크롤바숨김 = 줄.includes("[scrollbar-width:none]");
    if (!스크롤바숨김) return; // 스크롤바를 보여주면 화살표는 선택
    expect(줄, "스크롤바를 숨겼는데 「다음」 화살표가 없다 — 넘길 방법이 없어진다").toContain('aria-label="다음"');
    expect(줄, "스크롤바를 숨겼는데 「이전」 화살표가 없다").toContain('aria-label="이전"');
  });

  it("화살표는 «넘칠 때만» 나온다 — 눌러도 안 되는 단추를 남기지 않는다", () => {
    // 카드가 다 보이는데 화살표가 있으면 그것도 거짓말이다. 조건부 렌더인지 본다.
    expect(줄, "「다음」 화살표가 조건 없이 항상 그려진다").toMatch(/넘침\.오른쪽 && \(/);
    expect(줄, "「이전」 화살표가 조건 없이 항상 그려진다").toMatch(/넘침\.왼쪽 && \(/);
  });

  it("휴대폰에서는 화살표를 안 그린다 (손가락으로 넘기는 게 자연스럽다)", () => {
    expect(줄, "화살표에 `hidden md:flex` 가 없다 — 좁은 화면에서 화면을 가린다").toMatch(/hidden md:flex[^`]*rounded-full/);
  });
});
