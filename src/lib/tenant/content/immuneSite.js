...
export const IMMUNE_SITE = {
  ...
  programs: [
    {
      /* 2026-07-30 최종 — 신촌 **1인 수액실**(링거 폴대가 보인다). 카드 글이 「정맥·근육 주사」다.
         출처: 드라이브 `신촌_수액실/저화질(웹:블로그용)/4V6A0426.jpg`(원본 13MB / 웹용 229KB).
         ⚠️ 사람이 안 나온다 = 환자 초상권 문제 없음. */
      image: "/immune/facility/facility-iv-room-1.jpg",
    ...
    },
    {
      ...
      /* 2026-07-30 최종 — 성동점 **주사실**. 카드 첫 항목이 「pDRN 신경주사」라 주사를 놓는 자리가 맞다.
         출처: 성동 지점 사이트 시설안내(`sd.immunehospital.com`) alt=「주사실」.
         ⚠️ 냉각치료(CRYO)·자가재생(GFC) 각각의 사진은 여전히 없다 — 이 사진은 「주사를 놓는 공간」까지만 보여준다. */
      image: "/immune/facility/facility-injection-room-1.jpg",
    ...
    {
      /* 2026-07-30 최종 — **녹용공진단 제품 사진**. 카드가 「공진단 (녹용·원방)」을 그대로 적고 있어
         글과 사진이 같은 것을 가리킨다. 출처: 드라이브 `공진단/240809_녹용공진단/녹용공진단.jpg`(1.98MB).
         ⚠️ 원본은 분홍 배경 + 보자기 선물포장이라 판의 톤과 튀었다 → **제품(상자+환) 쪽으로 잘라** 썼다.
         ⚠️ 이건 「제품 사진」이지 「조제 장면」이 아니다. 탕전실·약재 사진이 생기면 그쪽이 더 낫다. */
      image: "/immune/program/program-gongjindan-1.jpg",
  ...
  menu: {
    ...
    labels: {
      ...
      // 2026-07-31 신설 — 가격을 표 «밖» 문단에서 표 «안» 한 줄로 옮기면서 붙는 이름표.
      // 밖에 떠 있을 땐 카드마다 줄 수가 달라 버튼 줄이 어긋났다.
      price: { ko: "참고가", en: "Guide price", ru: "Ориентировочная цена", kz: "Бағдарлы баға", zh: "参考价", ja: "参考価格" },
