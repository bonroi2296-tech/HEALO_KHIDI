 ...
 *
 * 교정 근거(2026-07-28, 같은 날 두 번 실측 — 값이 흔들린다):
 *   ① 대시보드 «83시간 = $14.13» ÷ 벽시계 991분  = $0.01426/분
 *   ② 대시보드 «97시간 = $17.16» ÷ 벽시계 1,106분 = $0.01552/분
 * 둘의 중간값을 쓴다. 흔들리는 이유는 (a)대시보드 시간이 시간 단위로 반올림되고
 * (b)«벽시계 1분당 CPU 분» 비율이 빌드마다 다르기 때문(5.02 → 5.26 으로 관측).
 *
 * ⚠️ 그래서 **정확도는 ±10% 수준이다. «오차 1%» 같은 말을 쓰지 마라** — 2026-07-28 에
 *    「빌드비 추정치」를 「크레딧 총 소진액(빌드+나머지)」과 맞대보고 1% 라고 잘못 발표해
 *    그 문구가 어드민 설명서까지 나갔다. 비교는 **반드시 같은 항목끼리**(빌드 vs 빌드).
 * → 콘솔 실값과 계속 벌어지면 위 방식으로 다시 교정할 것.
 */
export const EST_USD_PER_BUILD_MINUTE = 0.0149;
