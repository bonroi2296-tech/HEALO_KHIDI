...
export const RATE_LIMITS = {
  ...
  //    정작 「보내기」가 429 를 맞았다. 업로드 서명 통과 같은 크기로 맞춰 둔다.
  //    ⚠️ 2026-09-08 상향(20→120): 서류 1장에 1회이고 접수 한 건의 첨부 상한이 100장이다.
  //    실제 암환자 의무기록은 한 벌이 40~50장(유방암 케이스 44장) — 20 은 실제 환자와 안 맞았다.
  //    올리기 창구가 먼저 막혀 드러나지 않았을 뿐 여기도 같은 벽이었다.
  //    💰 요금은 분당 상한이 아니라 aiGuard 의 «일일» 상한 셋이 지킨다(IP당 soft 50·hard 400,
  //    전역 2000, 수동 차단목록). 그래서 여기를 올려도 요금 위험은 늘지 않는다.
  //    ⚠️ 다만 IP당 일일 400 은 «한 사람이 하루에 서류 400장»이다 — 코디가 한 IP 에서 환자
  //    여러 명 것을 몰아 올리면 닿을 수 있다(44장 × 9명 = 396). 그때는 그 값을 올려라.
  DOC_CLASSIFY: {
    windowMs: 60 * 1000,
    maxRequests: 120,
