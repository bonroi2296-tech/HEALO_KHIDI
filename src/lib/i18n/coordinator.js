...
const CT = {
  ...
  ibCloseNoteHint: { ko: "위 메모 칸에 이유를 적고 누르면 함께 남습니다.", en: "Write the reason in the note above; it is saved together.", ru: "Укажите причину в поле заметки выше — она сохранится вместе.", kz: "Себебін жоғарыдағы жазба өрісіне жазыңыз — бірге сақталады.", zh: "请先在上方备注栏写明原因，会一并保存。", ja: "上のメモ欄に理由を書いてから押すと一緒に保存されます。" },
  ibCloseConfirm: { ko: "이 문의를 종료합니다. 결과는 «이탈»로 기록되고 진행 단계는 «보류»가 됩니다. 식은 문의 알림에서 빠지며 언제든 되돌릴 수 있습니다. 계속할까요?", en: "Close this inquiry? The outcome is recorded as \"lost\" and the stage becomes \"On hold\". It leaves the cold-lead alerts and can be reopened anytime.", ru: "Закрыть заявку? Результат будет записан как «отказ», этап станет «Приостановлено». Заявка уйдёт из напоминаний об остывших заявках; её можно вернуть в любой момент.", kz: "Өтінімді жабасыз ба? Нәтиже «бас тартты» деп жазылады, кезең «Кейінге қалдырылды» болады. Суыған өтінім ескертулерінен шығады; кез келген уақытта қайта ашуға болады.", zh: "确定结束此咨询？结果将记为“流失”，阶段改为“暂缓”。它将不再出现在冷却咨询提醒中，随时可以恢复。", ja: "この問い合わせを終了しますか？結果は「離脱」として記録され、段階は「保留」になります。放置アラートから外れ、いつでも再開できます。" },
