...
const CT = {
  ...
  ceScr_therapy: { ko: "치료 안내 상세 · 치료법 카드(모든 암종 화면 공통)", en: "Treatment detail · therapy cards (shared by every cancer page)", ru: "Страница лечения · карточки методов (общие для всех видов рака)", kz: "Емдеу беті · емдеу әдісі карталары (барлық обыр беттеріне ортақ)", zh: "治疗详情 · 疗法卡片（各癌种页面共用）", ja: "治療詳細 · 治療法カード（全がん種ページ共通）" },
  ...
  // 이름표 옆에 붙는 비고(노란 배지)
  ceNote_hospitalMetaOnly: { ko: "검색 결과·미리보기 카드용 소개입니다 — 병원 상세 본문은 어드민 「병원 번역」에서 고칩니다", en: "Used for search snippets and link previews — the hospital page body is edited in Admin → Hospital translations", ru: "Используется для сниппетов поиска и превью ссылок — текст страницы больницы правится в Админ → «Перевод больниц»", kz: "Іздеу үзінділері мен сілтеме алдын ала қарауына арналған — аурухана бетінің мәтіні Әкімші → «Аурухана аудармасы» бөлімінде түзетіледі", zh: "用于搜索摘要和链接预览 — 医院详情正文在管理后台「医院翻译」中修改", ja: "検索スニペット・リンクプレビュー用の紹介文です — 病院詳細の本文は管理者「病院翻訳」で修正します" },
