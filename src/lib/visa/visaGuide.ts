...
const VISA_DATA: Record<VisaType, VisaInfo> = {
  'C-3-3': {
    ...
    fee: 'US$20~80',
    // ⚠️ 이 문구는 «국적과 무관하게» 모든 환자에게 뜬다(VISA_DATA 는 국적별이 아니다).
    //    무비자 일수 같은 국적별 숫자를 여기 적으면 비자필요국(몽골·우즈벡 등)·일본(90일)에
    //    엉뚱한 안내가 나가고, /patient/rebooking 은 국적 맥락 없이 이것만 띄운다.
    //    → 국적별 숫자는 COUNTRY_ENTRY[국적].note 에만 둔다.
    notes: {
      ko: '⚠️ 무비자로 입국한 뒤에는 치료가 길어져도 체류자격 변경이 원칙적으로 허용되지 않습니다. 장기 치료가 예상되면 처음부터 이 비자로 신청하세요. 수수료·처리기간·재정능력 기준은 관할 공관마다 다르니 아래 대사관 공지를 확인하세요.',
      en: '⚠️ If you enter visa-free, changing your status later is generally not permitted even if treatment runs long. If long treatment is expected, apply for this visa from the start. Fees, processing times, and financial requirements differ by mission — check the embassy notice below.',
      ru: '⚠️ При безвизовом въезде смена статуса пребывания, как правило, не разрешается, даже если лечение затянулось. Если ожидается длительное лечение, оформляйте эту визу заранее. Сборы, сроки и финансовые требования различаются по консульствам — см. объявление посольства ниже.',
      zh: '⚠️ 以免签入境后，即使治疗延长，原则上也不允许变更停留资格。如预计长期治疗，请从一开始就申请此签证。费用、办理时间和财力要求因领馆而异，请查阅下方使馆公告。',
      ja: '⚠️ ビザ免除で入国した場合、治療が長引いても在留資格の変更は原則認められません。長期治療が見込まれる場合は最初からこのビザで申請してください。手数料・処理期間・財政要件は公館ごとに異なります — 下記の大使館公示をご確認ください。',
      kz: '⚠️ Визасыз кірген жағдайда емделу ұзарса да, болу мәртебесін өзгертуге негізінен рұқсат етілмейді. Ұзақ емделу күтілсе, осы визаны бастапқыда рәсімдеңіз. Алымдар, өңдеу мерзімі және қаржылық талаптар мекемеге қарай әртүрлі — төмендегі елшілік хабарландыруын қараңыз.',
    },
  },
  'G-1-10': {
    visaType: 'G-1-10',
    ...
    fee: 'US$50~80',
    // 위 C-3-3 주석과 같은 이유로 국적별 숫자는 넣지 않는다.
    // 「최대 체류 365일」 칸과 어긋나지 않게: 365 = 상한(중증), 일반은 6개월 범위.
    notes: {
      ko: '체류기간은 치료 내용에 따라 정해지고 국내에서 연장할 수 있습니다 — 일반은 6개월 범위, 중증 환자는 최대 1년 범위(만료 4개월 전부터 신청). ⚠️ 무비자로 입국한 뒤에는 이 자격으로 바꾸는 것이 원칙적으로 허용되지 않으니, 장기 치료가 예상되면 처음부터 신청하세요.',
      en: 'Your stay is set by the treatment and can be extended in Korea — normally within 6 months, up to 1 year for severe cases (apply from 4 months before expiry). ⚠️ After entering visa-free, switching to this status is generally not permitted, so apply from the start if long treatment is expected.',
      ru: 'Срок пребывания определяется лечением и может быть продлён в Корее — обычно в пределах 6 месяцев, для тяжёлых случаев до 1 года (подача за 4 месяца до окончания). ⚠️ После безвизового въезда переход на этот статус, как правило, не разрешается — при длительном лечении оформляйте визу заранее.',
      zh: '停留期限根据治疗内容确定，可在韩国境内延长 — 一般在6个月范围内，重症最长1年（到期前4个月起可申请）。⚠️ 免签入境后原则上不允许变更为该资格，如预计长期治疗，请从一开始就申请。',
      ja: '滞在期間は治療内容に応じて決まり、韓国国内で延長できます — 通常は6か月の範囲、重症の場合は最長1年（満了4か月前から申請可）。⚠️ ビザ免除で入国した後にこの資格へ変更することは原則認められないため、長期治療が見込まれる場合は最初から申請してください。',
      kz: 'Болу мерзімі емдеуге қарай белгіленеді және Кореяда ұзартылады — әдетте 6 ай шегінде, ауыр науқастарға ең көбі 1 жыл (мерзім бітуден 4 ай бұрын өтініш беруге болады). ⚠️ Визасыз кіргеннен кейін осы мәртебеге ауысуға негізінен рұқсат етілмейді, сондықтан ұзақ емделу күтілсе бастапқыда рәсімдеңіз.',
    },
  },
};
...
// ⚠️ 이 날짜는 «데이터 전체»를 실검증한 날이다. 일부만 확인하고 올리지 마라
//    (올리는 순간 CI 의 재검증 강제가 그만큼 뒤로 밀린다).
//    2026-08-07 재확인한 것: C-3-3·G-1-10 의 처리기간·수수료·체류자격 변경 규칙,
//    카자흐(30일)·러시아(60일) 무비자. 나머지 8개 국적(mn·uz·kg·tj·az·zh·ja·en)은
//    이번에 안 봤으므로 날짜를 올리지 않는다.
export const VISA_DATA_LAST_VERIFIED = '2026-07-01';
