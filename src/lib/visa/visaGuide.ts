...
const G1_ADDITIONAL_DOCUMENTS: VisaDocument[] = [
  ...
  },
  {
    ...
    description: {
      ...
      ja: '母国医療機関の診断書または紹介状（公証翻訳が必要）',
      kz: 'Туған еліндегі медициналық мекеменің диагнозы немесе жолдамасы (нотариалды аударма қажет)',
    },
    required: true,
  },
];
...
const COUNTRY_ENTRY: Record<string, CountryEntry> = {
  ru: {
    ...
    summary: {
      ...
      kz: 'Ресей азаматтары туристік мақсатта 60 күнге дейін визасыз кіре алады. Емделу мақсатында аурухана шақыру хатымен C-3-3 медициналық визасын алу ұсынылады.',
    },
    note: {
      ko: 'K-ETA(전자여행허가)를 사전에 받아야 합니다(한시 면제 22개국에 해당 없음). 91일 이상 치료에는 G-1-10 비자가 필요합니다.',
      en: 'You must obtain K-ETA in advance (Russia is not among the 22 temporarily exempted countries). For treatment over 91 days, the G-1-10 visa is required.',
      ru: 'Необходимо заранее получить K-ETA (Россия не входит в число 22 временно освобождённых стран). Для лечения свыше 91 дня нужна виза G-1-10.',
      zh: '须提前申请K-ETA（俄罗斯不在22个临时免除国之列）。治疗超过91天需办理G-1-10签证。',
      ja: '事前にK-ETA（電子旅行許可）の取得が必要です（ロシアは一時免除の22か国に含まれません）。91日以上の治療にはG-1-10ビザが必要です。',
      kz: 'Алдын ала K-ETA алу қажет (Ресей уақытша босатылған 22 елдің қатарына кірмейді). 91 күннен асатын емделуге G-1-10 визасы қажет.',
    },
    embassy: {
      name: {
  ...
  kz: {
    ...
    visaFreeDays: 30,
    summary: {
      ko: '카자흐스탄 국민은 관광 목적 30일 무비자 입국이 가능합니다(한-카자흐 무비자 협정). 다만 치료 목적이면 병원 초청장 기반 C-3-3 의료비자를 권장합니다.',
      en: 'Kazakhstani citizens may enter visa-free for up to 30 days (Korea–Kazakhstan visa-waiver agreement). For treatment, a C-3-3 medical visa with a hospital invitation is recommended.',
      ru: 'Граждане Казахстана могут въезжать без визы на срок до 30 дней (соглашение о безвизовом режиме между Кореей и Казахстаном). Для лечения рекомендуется медицинская виза C-3-3 с приглашением больницы.',
      zh: '哈萨克斯坦公民可免签入境最多30天（韩国-哈萨克斯坦免签协议）。如为治疗目的，建议持医院邀请函办理C-3-3医疗签证。',
      ja: 'カザフスタン国民は観光目的で最大30日間ビザ免除で入国できます（韓国・カザフスタン間のビザ免除協定）。治療目的の場合は病院の招待状によるC-3-3医療ビザを推奨します。',
    ...
    note: {
      ko: 'K-ETA(전자여행허가)를 사전에 받아야 합니다(한시 면제 대상국 아님). 91일 이상 치료에는 G-1-10 비자가 필요합니다. 한국어·영어가 아닌 서류는 공증 번역이 필요합니다.',
      en: 'You must obtain K-ETA in advance (Kazakhstan is not on the temporary exemption list). For treatment over 91 days, the G-1-10 visa is required. Documents not in Korean or English need notarized translation.',
      ru: 'Необходимо заранее получить K-ETA (Казахстан не входит в список временно освобождённых стран). Для лечения свыше 91 дня нужна виза G-1-10. Документы не на корейском или английском требуют нотариального перевода.',
      zh: '须提前申请K-ETA（哈萨克斯坦不在临时免除名单内）。治疗超过91天需办理G-1-10签证。非韩语或英语的文件需公证翻译。',
      ja: '事前にK-ETA（電子旅行許可）の取得が必要です（一時免除の対象国ではありません）。91日以上の治療にはG-1-10ビザが必要です。韓国語・英語以外の書類は公証翻訳が必要です。',
      kz: 'Алдын ала K-ETA (электрондық саяхат рұқсаты) алу қажет (Қазақстан уақытша босатылған елдер тізімінде жоқ). 91 күннен асатын емделуге G-1-10 визасы қажет. Корей не ағылшын тілінде емес құжаттарға нотариалды аударма керек.',
    },
    embassy: {
      name: {
  ...
  zh: {
    ...
    summary: {
      ...
      ru: 'Гражданам Китая нужна виза для индивидуального въезда. (Для тургрупп от 3 человек — безвизовый въезд на 15 дней, временно до июня 2026 г.) Для лечения оформите визу C-3-3.',
      ...
      kz: 'Қытай азаматтарына жеке кіруге виза қажет. (3 және одан көп адамнан тұратын топтық турларға 2026 жылдың маусымына дейін 15 күн визасыз.) Емделуге C-3-3 визасын рәсімдеңіз.',
    },
    note: {
      ko: '병원 초청장(또는 진료예약 확인서)이 필수입니다. 한국어·영어가 아닌 서류는 공증 번역이 필요합니다.',
      en: 'A hospital invitation (or appointment confirmation) is required. Documents not in Korean or English need notarized translation.',
