 ...
 *   ①식은 문의 크론이 매일 알림을 올리고 ②전환 집계엔 «대기»로 남는다.
 *   → 어드민 점수판과 코디 화면이 «같은 함수»로 결과를 바꾼다(두 벌로 갈리면 이력·집계가 어긋난다).
 *
 * 규칙
 * - admitted: 단계를 «입국·치료»로 전진(advanceCaseStatus, 뒤로는 안 감; «보류»에서도 전진한다).
 * - lost: 단계는 그대로 두되(어드민 점수판) 또는 «보류»로 내림(코디 「종료」 단추, holdOnLost). 이력에 «🚫 이탈 처리».
 *   ⚠️ 이미 입국·치료 이후(treatment·follow_up·completed)인 케이스는 코디 종료 단추로 못 닫는다 — «안 온 것»이 아니다.
 * - null(되돌리기): 결과만 비우고 단계는 안 건드린다(«보류»였다면 코디가 단계를 다시 고른다). 이력에 «↩️».
 * - 같은 결과를 다시 보내면(되돌리기 두 번 누름·점수판 재클릭) 이력·단계는 안 건드린다 — 메모만 갱신.
 * - 없는 문의는 not_found — Supabase 는 0행 갱신을 오류로 안 돌려줘서 따로 본다.
...
export interface SetInquiryOutcomeResult {
  ok: boolean;
  error?: "invalid_outcome" | "not_found" | "already_arrived" | "update_failed";
  /** 처리 뒤 진행 단계(모르면 null). */
  caseStatus: string | null;
  /** 종료로 «보류»를 쓰면서 남긴 단계 메모 — 화면이 메모 칸을 서버와 맞추는 데 쓴다. */
  caseStatusNote?: string | null;
  /** 같은 결과를 다시 보내 이력·단계를 안 건드렸나. */
  unchanged?: boolean;
}

/** 입국 이후 단계 — «안 온다»가 성립하지 않는다. */
export const ARRIVED_CASE_STATUSES: ReadonlySet<string> = new Set(["treatment", "follow_up", "completed"]);
...
export async function setInquiryOutcome(db: any, input: SetInquiryOutcomeInput): Promise<SetInquiryOutcomeResult> {
  ...
  // 바꾸기 «전» 상태 — 존재 확인·되돌리기 이력 문구(«유치 취소» vs «종료 취소»)·보류 판단에 쓴다.
  const { data: before } = await db.from("inquiries").select("case_status, outcome").eq("id", inquiryId).maybeSingle();
  if (!before) return { ok: false, error: "not_found", caseStatus: null };
  ...
  if (outcome === "lost" && input.holdOnLost && caseStatus && ARRIVED_CASE_STATUSES.has(caseStatus)) {
    return { ok: false, error: "already_arrived", caseStatus };
  }

  if (prevOutcome === outcome) {
    if (outcome === null) return { ok: true, caseStatus, unchanged: true };
    const { error } = await db
      .from("inquiries")
      .update({ outcome_note: note, outcome_updated_at: now, outcome_updated_by: userId })
      .eq("id", inquiryId);
    if (error) {
      console.error("[inquiryOutcome] note update error:", error.message);
      return { ok: false, error: "update_failed", caseStatus };
    }
    return { ok: true, caseStatus, unchanged: true };
  }
  ...
  let caseStatusNote: string | null | undefined;
  try {
    if (outcome === "admitted") {
    ...
    if (outcome === "lost" && input.holdOnLost && caseStatus !== "on_hold") {
      caseStatusNote = note ?? "종료(안 옴)";
      const { error: hErr } = await db
        .from("inquiries")
        .update({ case_status: "on_hold", case_status_note: caseStatusNote, case_status_updated_at: now })
        .eq("id", inquiryId);
      if (hErr) {
        console.error("[inquiryOutcome] on_hold error:", hErr.message);
        caseStatusNote = undefined;
      } else {
        caseStatus = "on_hold";
      }
    }
  ...
  return { ok: true, caseStatus, caseStatusNote };
}
