...
/**
 * 설문 발송 '침묵' 감지 — 사후관리 설문이 있어야 하는데 안 나가는 상태를 잡는다.
 *
 * 왜(POSTMORTEM #81, 🔁 #70 부류): K-03(만족도) 설문이 '아무도 안 만드는 신호'
 * (consultation_sessions.completed=영구 0)에 물려 조용히 0건이었다. #70 재발방지에서
 * "설문 발송률 모니터"를 후속과제로 미뤄둔 게 바로 이 감지기 — 발송 메커니즘만 고치던
 * 과거 수정(#12·13·19·70)들은 "표본 자체가 0"인 상황을 못 봤다. 그래서 데이터로 잡는다.
 *
 * 동작: 사후관리 진입(inquiries.followup_started_at) 8일↑(D+7 +1일 유예) 지난 비테스트
 * 케이스인데 1주차 설문(fu_week_1)이 안 나간 게 있으면 Sentry 경보. 절대 throw 안 함.
 */
export async function alertIfSurveysStale(): Promise<{ stale: boolean; overdue: number }> {
  try {
    ...
    const { data: cases } = await supabaseAdmin
      ...
      .or("is_test.is.null,is_test.eq.false") // null=테스트아님(cron 과 동일 판정)
    ...
    const sentIds = new Set(((sent as any[]) || []).map((r) => r.inquiry_id));
