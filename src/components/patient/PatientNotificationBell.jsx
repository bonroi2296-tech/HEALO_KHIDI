...
/**
 * 환자용 인앱 알림 벨 — 우상단 고정.
 * notifications 테이블에 RLS(본인 select/update 허용)가 걸려 있어 별도 서버 API 없이
 * 브라우저 Supabase 클라이언트로 직접 조회·읽음처리한다. 30초 폴링.
 * 발신은 서버의 sendInAppNotification(@/lib/notifications/inApp) 이 담당.
 */
...
export default function PatientNotificationBell() {
  ...
  return (
    <div ref={panelRef} className="fixed top-2.5 right-3 z-[60]">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label={l(T.title)}
        className="relative p-2 rounded-full bg-white/90 backdrop-blur border border-gray-200 shadow-sm hover:bg-gray-50 transition"
      >
      ...
      {open && (
        <div className="absolute top-11 right-0 w-80 max-w-[88vw] bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
