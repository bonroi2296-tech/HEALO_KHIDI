...
const nextConfig = {
  ...
  async redirects() {
    ...
    return [
      ...
      // SNS 프로필 링크 창구(2026-09-11). 인스타·틱톡은 앱 안 브라우저가 출처를 지워서
      // 유입이 전부 (direct) 로 섞인다 — GA4 90일 유입 13개 경로에 인스타·틱톡이 «한 건도» 없었다.
      // ⚠️ 「그냥 /ru 를 쓰면 안 되나」의 답: 안 된다. /ru 는 러시아어 홈 «자체»라 검색·직접입력·SNS 가
      //    같은 칸에 쌓인다. 구분하려면 SNS 전용 입구가 따로 있어야 한다.
      // 프로필에 노출되는 주소에는 utm 이 «안 보인다»(환자 눈에 피싱처럼 보이면 안 된다 — PO 지적).
      // 꼬리표는 여기서 붙인다. 채널별로 가른 건 코드 3줄 차이로 「어느 판이 환자를 데려오나」를 알 수 있어서다.
      // permanent=false(307) — 목적지를 바꿀 수 있어야 하고 브라우저가 영구 캐시하면 안 된다.
      // 늘릴 때: 뒤 한 글자만 더한다(예: 현장 배포물 /ruk). utm_source 는 GA4 에서 그대로 채널 이름이 된다.
      ...[
        ["i", "instagram"],
        ["f", "facebook"],
        ["t", "tiktok"],
        ...
        source: `/ru${ch}`,
        destination: `/ru?utm_source=${src}&utm_medium=social&utm_campaign=profile`,
        permanent: false,
      })),
    ];
  },
