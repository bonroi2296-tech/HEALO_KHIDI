...
> **2026-07-06 확장:** DESIGN.md는 이제 "AI가 신규 UI 짤 때 읽는 가이드"를 넘어, **디자인타임·컴포넌트·런타임 세 층의 도구가 모두 참조하는 단일 진실(SoR)**이다. 아래 토큰(§4)이 뇌, 각 도구는 그 뇌를 각자 방식으로 소비한다. (배경: Astryx·Google Stitch·제너레이티브 UI 3개 리서치 → "모듈+규칙을 설계하는 아키텍트" 방향 채택. 상세는 `docs/PROJECT_CONTEXT.md` 핸드오프.)

| 층 | 도구 | 이 문서를 어떻게 읽나 | 상태 |
|---|---|---|---|
| **디자인타임 생성** | Google Stitch | 이 파일을 **import**하거나 라이브 사이트 URL에서 추출 → 화면 초안 생성. ⚠️ Stitch는 강제력이 약해(매 생성 재지정 필요) **초안 전용**, 확정은 우리 손 + §6 자가검열. | 선택적 액셀러레이터 |
| **컴포넌트(모듈)** | 프런트=우리 Tailwind / 백오피스=Astryx | 프런트는 §4 토큰 그대로. 백오피스 Astryx는 아래 **「Astryx teal 토큰 매핑」** CSS를 덮어 브랜드 통일. | 하이브리드(백오피스 8/27 평가 후) |
| **런타임 생성** | Vercel AI SDK gen-UI (`ai` v6, 이미 설치) | 챗봇이 아래 **「런타임 gen-UI 화이트리스트」** 안의 검증 컴포넌트만 상황맞춤 렌더. 자유생성 금지. | 저리스크·우선 |

**허브 규칙**: 색·간격·톤 값을 바꾸면 **여기(§4) 한 곳만** 바꾸고 세 도구가 다 따라오게 한다. 도구별로 값을 따로 두지 마라(드리프트 원인).
...
## 🧩 Astryx teal 토큰 매핑 (백오피스 컴포넌트용)

> **언제:** 백오피스(어드민·코디 등 정보밀도 화면)를 Astryx 컴포넌트로 만들 때. 프런트(환자앞단)는 이 매핑 불필요 — 우리 Tailwind 그대로.
> **원리:** Astryx neutral 테마의 강조색(진회색)을 §4 브랜드 teal로 **토큰만** 덮는다(테마=겉모습만). 값은 §4 hex 그대로 = 드리프트 없음. (파일럿 실검증: `/astryx-pilot*`)

```css
/* Astryx 스코프 안에서 이 블록을 theme.css 뒤에 로드 */
[data-astryx-theme="neutral"] {
  color-scheme: light;                            /* 기기 다크모드여도 밝은 브랜드 톤 고정 */
  --color-accent:       light-dark(#0d9488, #2dd4bf);  /* primary  = teal-600 / teal-400 */
  --color-accent-muted: light-dark(#f0fdfa, #134e4a);  /* subtle   = teal-50 / teal-900 */
  --color-text-accent:  light-dark(#0f766e, #5eead4);  /* accent 텍스트 = teal-700 / teal-300 */
  --color-icon-accent:  light-dark(#0d9488, #2dd4bf);
  --color-on-accent:    #ffffff;                       /* teal 위 글자 */
}
```

⚠️ **정식 도입 시 통합 비용(파일럿이 잡은 함정):** 우리 Tailwind 3.4는 `@tailwind base`(unlayered)라 preflight가 Astryx의 `@layer` 컴포넌트 스타일을 이겨 **버튼이 투명해진다.** → 글로벌 Tailwind를 `@layer`로 재편해야 함(8/27 평가 후 실전 투입 시 1회성 작업). 파일럿에선 astryx.css를 de-layer한 사본으로 우회.
...
- **2026-07-06**: **허브화** — DESIGN.md를 "세 도구가 읽는 단일 계약서"로 확장(상단 「이 문서의 지위」표). 계기: Astryx·Google Stitch·제너레이티브 UI 3개 리서치 → "모듈+규칙 아키텍트" 방향. 추가: ①§4 토큰에 hex 병기(Tailwind 안 쓰는 소비자용) ②「Astryx teal 토큰 매핑」(백오피스 컴포넌트, @layer 통합비용 경고 포함) ③「런타임 gen-UI 화이트리스트+가드레일」(챗봇 = Vercel AI SDK, 자유생성 금지·검증 컴포넌트만·의료 가드). 기존 톤 규칙·금지목록은 그대로(추가만).
