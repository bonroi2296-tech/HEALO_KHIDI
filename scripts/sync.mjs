 ...
 *   · **본판(main)에서는 자동 저장·올리기를 안 함 — 내려받기만 함**
 *     (main 푸시 = 검사 없이 실서비스 배포. 워크트리 하나가 main 을 잡고 있는
 *      환경에서 남의 미저장 변경이 실려 나가는 사고를 막는다.)
 *
...
// 본판(main)은 푸시하는 순간 검사 없이 실서비스로 배포된다 → 여기선 내려받기만 허용.
// (워크트리 여러 개 환경에선 main 폴더에 남의 미저장 변경이 남아 있을 수 있다.)
const PROTECTED_BRANCHES = new Set(["main", "master"]);
const isProtected = PROTECTED_BRANCHES.has(branch);
if (isProtected) {
  say(DIM("  (본판이라 내려받기만 한다 — 자동 저장·올리기는 하지 않는다)"));
}
...
if (dirtyLines.length) {
  ...
  // 본판 안전장치: main 에 저장 안 된 변경이 있으면 손대지 않고 멈춘다.
  // 자동 저장하면 검사 없이 실서비스로 나가고, 남의 작업 중간을 잘라 올리게 된다.
  if (isProtected) {
    say();
    bad(`본판(${branch})에 저장 안 된 변경이 있어서 멈춘다.`);
    say(DIM("  본판에 바로 올리면 자동 검사 없이 실서비스로 배포된다."));
    say(DIM("  이 변경은 다른 작업(세션)이 만지던 것일 수도 있어서 손대지 않았다."));
    say(DIM("  파일은 그대로 두고, 클로드한테 '본판에 안 저장된 변경 있다' 라고 말해라."));
    say();
    process.exit(3);
  }
} else {
...
// 본판 안전장치 2: main 에 아직 안 올린 저장이 있으면 올리지 않고 멈춘다.
// (합치기 신청서(PR)·자동 검사를 건너뛰고 실서비스로 나가는 걸 막는다.)
if (isProtected && ahead > 0) {
  bad(`본판(${branch})에 아직 안 올린 저장 ${ahead}개가 있어서 멈춘다.`);
  say(DIM("  본판에 바로 올리면 자동 검사 없이 실서비스로 배포된다."));
  say(DIM("  클로드한테 '본판에 안 올린 저장 있다' 라고 말해라 — 작업본으로 옮겨 신청서로 올린다."));
  say();
  process.exit(3);
}
