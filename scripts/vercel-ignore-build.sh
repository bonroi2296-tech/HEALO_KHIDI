...
#           (자동저장 백업 커밋은 [preview] 가 없어 여기서 스킵된다 — 옛 「규칙 0」은 삭제, 이유는 규칙 2 에)
...
#
# 자동저장(백업) 커밋 — Stop 훅(auto-commit-push.sh)이 "chore: 작업 자동 저장 (시각)" 으로 만드는 미완성
# 스냅샷 — 도 여기서 스킵된다: 제목에 [preview] 가 없기 때문이다. 예전엔 이걸 «규칙 0»으로 스크립트 맨 앞에서
# 따로 걸렀는데, 2026-09-07 실사고(#1671)로 지웠다: 커밋 1개짜리 신청서를 스쿼시하면 깃허브가 «그 커밋 제목»을
# 본판 제목으로 쓰고, 그 결과 본판 머리가 자동저장 제목이 된 채 창구가 열리면 규칙 0 이 프로덕션 판정보다
# 먼저 exit 0 → 그날 실서비스 배포가 조용히 취소될 판이었다. 프로덕션은 규칙 1 이, 프리뷰는 이 블록이
# 이미 판정하므로 규칙 0 은 «아무것도 지키지 않는 코드»였다(규칙 0 을 지운 사본으로 자체검사 22건 전부 통과).
# 창구가 연 배포는 제목이 뭐든 짓는다 — 자체검사 「자동저장 제목이 머리 커밋일 때」 4건이 그 구멍을 지킨다.
#
...
#
#   ⭐ 기본이 0(완전 차단)인 이유 — PO 지시: «프리뷰도 내가 필요하면 요청할테니깐 그건 막아버려».
#   화면 확인은 프리뷰가 아니라 **로컬 + 화면 사진**이 기본이다. 「로그인해야 보이는 화면
#   (코디·관리자)은 로컬에서 못 본다」가 프리뷰의 마지막 명분이었는데 2026-07-31 실측으로
#   깨졌다 — 로컬 dev 서버에 브라우저로 로그인해서 코디 대시보드까지 그대로 열고 사진을 찍었다.
#
#   ⚠️ 어시가 판단해서 여는 통로가 아니다. **PO 가 요청했을 때만** Vercel 프로젝트 환경변수
#      PREVIEW_DAILY_LIMIT 을 1 로 올려 짓고, 짓고 나면 되돌린다(절차: docs/rules/DEPLOY.md).
#      커밋 제목만으로는 열 수 없다 — 그래야 「막아버려」가 실제로 막힌 상태로 유지된다.
PREVIEW_DAILY_LIMIT="${PREVIEW_DAILY_LIMIT:-0}"

if [ "$VERCEL_ENV" = "preview" ]; then
  case "$commit_subject" in
    *"[preview]"*)
      # 상한 0 = 완전 차단(기본). [preview] 를 달아도 안 짓는다.
      if [ "$PREVIEW_DAILY_LIMIT" -le 0 ]; then
        echo "🛑 프리뷰는 기본 차단이다 (2026-07-31 PO 지시 — 필요하면 PO 가 직접 요청한다)."
        echo "   화면 확인은 로컬 dev 서버 + 화면 사진으로 한다. 로그인해야 보이는 화면도 로컬에서 열린다."
        echo "   PO 요청으로 열려면: Vercel 환경변수 PREVIEW_DAILY_LIMIT=1 → 짓고 → 되돌린다."
        exit 0
      fi
