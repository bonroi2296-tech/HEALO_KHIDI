...
async function upload(buf, name, type) {
  const sign = await post({ name, type, size: buf.length });
  ...
  const confirm = await post({ confirmPath: sign.json.path, type });
...
const over = await post({ name: "huge.pdf", type: "application/pdf", size: 60 * 1024 * 1024 });
check("50MB 초과 거부", over.json.error === "file_too_large", over.json.error || "");

// 4) 허용 안 된 타입
const badType = await post({ name: "x.exe", type: "application/x-msdownload", size: 1000 });
check("허용 외 타입 거부", badType.json.error === "invalid_file_type", badType.json.error || "");

// 5) 남의 경로 confirm 시도 — 임의 객체 삭제 통로가 되면 안 된다
const evil = await post({ confirmPath: "inquiry/../../documents/anything.pdf", type: "application/pdf" });
