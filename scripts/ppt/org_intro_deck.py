...
# ── 연락처 번호 ────────────────────────────────────────────────────────
# 2026-09-11 PO 확인: 대표번호 = **070-7500-7795**.
#   ⚠️ 한때 070-7500-«1075» 로 잘못 알던 적이 있다 — 뒷자리가 휴대폰(+82-10-4772-«1075»)
#      꼬리와 같아서 섞이기 쉽다. 정본은 개인정보처리방침·이용약관(6개 언어)에 박힌 7795 다.
#
# 🔁 **2026-09-11 결정 뒤집힘: 휴대폰도 넣는다.** 2026-09-10 엔 「개인번호는 안 넣는다」였는데
#    ①070 은 인터넷전화라 **WhatsApp 가입이 안 되고** 카자흐 실무 채널이 왓츠앱이다
#    ②이 번호는 **이미 공개돼 있다** — 개인정보처리방침에 「+82-10-4772-1075 (국제)」로
#      6개 언어 실서비스에 떠 있고(2026-09-11 `/ru/privacy` 실측), 사이트 왓츠앱 단추도
#      같은 번호다(`src/lib/tenant/index.js` 의 `wa.me/821047721075`).
#    즉 제안서에 적는다고 «새로» 노출되는 것이 없다.
...
if MOBILE:
    # 해외에서 거는 쪽은 이 줄을 본다 — 070 은 국제전화가 번거롭고 왓츠앱이 안 된다.
    _rows.append(["휴대폰 · WhatsApp", MOBILE])
_rows += [
