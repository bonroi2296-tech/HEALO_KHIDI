...
quad(X0 + CW + GAP, Y0, 'W', '약점', 'Weakness', [
    ...
    ('유치 실적 미발생', '사업 5개월간 실환자 유치 0건 · 문의 대다수가 국내 치료 적응증 미충족'),
...
quad(X0, Y0 + CH + GAP, 'O', '기회', 'Opportunity', [
    ...
    ('러시아어권 전체로 확장', '같은 화면 · 같은 통역으로 러시아 · 우즈베크 · 키르기스까지'),
], strong=True)

quad(X0 + CW + GAP, Y0 + CH + GAP, 'T', '위협', 'Threat', [
    ('송금 · 결제 경로 제약', '금융 제재로 직접 송금이 막혀 병원 창구 대납 등 우회 경로가 필요하다'),
    ('중증 환자의 이송 위험', '전신상태가 나쁘면 이동 자체가 불가 · 현지 의료진이 이송 불가로 판단하는 사례'),
...
d.text((X0 + 34, BY + 54),
       '방한 「전」에 한국 대학병원 소견부터 받아보는 경로를 표준으로 만든다   ·   브로커 소개가 아니라 의료진 판단으로 시작하는 유치 모델',
