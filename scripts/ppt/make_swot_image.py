...
quad(X0 + CW + GAP, Y0, 'W', '약점', 'Weakness', [
    ('현지 인지도 부족', '우리 이름을 아는 환자가 아직 없어 소개를 거쳐야 문의가 닿는다'),
    ('유치 실적 미발생', '실환자 유치 0건 · 문의 대다수가 한국에 와도 치료가 어려운 상태였다'),
    ('사전 견적을 못 준다', '검사 전에는 치료 일정과 비용을 확정할 수 없어 환자가 결정을 미룬다'),
])
...
quad(X0 + CW + GAP, Y0 + CH + GAP, 'T', '위협', 'Threat', [
    ('환율 · 비용 부담', '환율이 불리해지면 한국 치료비 부담이 커져 결정이 미뤄진다'),
    ('국내 병원 수용 여력', '대학병원은 내국인 진료가 우선이라 방한 일정을 잡기 어렵다'),
