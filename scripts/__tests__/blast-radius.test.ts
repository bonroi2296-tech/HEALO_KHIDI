...
describe("영향 반경 지도", () => {
  ...
  it("«없는» 파일에 「전 화면 영향」이라고 단정하지 않는다", () => {
    // 경로 생김새만 보고 판정하면 오타·이미 지운 파일에도 그대로 경보가 뜬다(실측으로 걸렸다).
    // 거짓 경보는 진짜 경보를 묻히게 한다 — 「전 화면」은 제일 센 말이라 더 그렇다.
    const r = run(["--files", "src/lib/i18n/이런파일은없다.js"]);
    expect(r.globalHits.length).toBe(0);
    expect(r.impactedRoutes.length).toBe(0);
  });
