...
const FORBIDDEN = [
  ...
  // 2026-07-06 추가: 디자인타임 AI 생성 초안이 '완치를 향해'를 슬립 → '완치 보장'만 막던 가드 구멍 발견.
  //   완치를 지향·약속하는 마케팅 카피도 결과 단정 뉘앙스라 차단(사람이 짜든 AI가 뽑든 영구 차단).
  { re: /완치\s*(?:를|을)\s*향[해한]/, allow: /[\\/]lib[\\/]chat[\\/]|ai-status|playbook/, msg: "의료광고 금지문구 '완치를 향해/향한' — 완치 지향 카피도 결과 단정(의료광고법). '치료·회복 지원' 등 사실 기반으로." },
  { re: /완치\s*(?:를|을)?\s*약속(?!\s*(?:하지\s*않|안|못|없|드리지\s*않))/, allow: /[\\/]lib[\\/]chat[\\/]|ai-status|playbook/, msg: "의료광고 금지문구 '완치 약속' — 결과 약속 금지. 면책이면 '…약속하지 않습니다' 형태로." },
